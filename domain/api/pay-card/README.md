...
The package owns the Pay Card API wire contracts, RTK Query endpoints, and transforms. Runtime
configuration is injected through the Redux thunk `extraArgument` with `payCardApiExtra`, so the
domain package does not depend on app environment or configuration modules.
