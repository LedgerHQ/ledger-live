...
provider: Desktop and Mobile gate it behind the `lwdAuth`/`lwmAuth` feature
flags, which are off by default, and wallet-cli always requests a token. HTTP
status errors are deliberately left as RTK Query errors so the adapter's 401
refresh-and-retry can fire; mapping them to a caller-facing outcome is the
caller's job.
