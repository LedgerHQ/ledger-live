 ...
 * string for the same reason the custodial wallets' does: a decimal that survives the wire should
 * not be rounded into a number on the way in.
 */
export const PayCardRewardWalletSchema = z.object({
  id: z.string().min(1),
  balance: z.string().min(1),
  currency: z.string().min(1),
...
export const PayCardRewardWalletResponseSchema = PayCardRewardWalletSchema;
