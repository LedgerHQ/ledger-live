...
| -------- | ------ | ---- | ------- |
| `exchangeAuthorizationCode` | POST | `/v1/auth/oauth2/token` | Exchange the authorization code for a session. No argument, and it answers with a handle |
| `refreshSession` | POST | `/v1/auth/oauth2/token` | Same endpoint, `refresh_token` grant. No argument, and it answers with a handle |
