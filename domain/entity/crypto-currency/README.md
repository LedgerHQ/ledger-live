...
No Redux slice, no selectors — the currency list is fully static after boot. Each app maintains its own `SUPPORTED_CURRENCY_IDS` constant; consumers resolve currencies directly from the registry.
...
**Static registry, no store.** The supported currency list is set once at app startup and never changes at runtime. Putting it in Redux would imply reactivity that doesn't exist. Each app owns a `SUPPORTED_CURRENCY_IDS` constant; consumers resolve currencies directly from `CRYPTO_CURRENCIES_REGISTRY`. This replaces the `setSupportedCurrencies` / `listSupportedCurrencies` singleton from `ledger-wallet-framework`.
