...
- Expose **selectors** and a **rate-limit state** used by the sync middleware
...
`getLastFailureTime / setLastFailureTime / clearLastFailureTime` maintain a module-level timestamp used by `@domain/api-push-devices` to enforce a 5-minute back-off after a sync failure. Stored in-process only (not persisted).
...
src/
  ...
  data/
    ...
    rateLimitState.ts In-process rate-limit timestamp
    index.ts
