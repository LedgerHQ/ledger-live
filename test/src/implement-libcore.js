...
const NJSThreadDispatcher = new lib.NJSThreadDispatcher({
  ...
  newLock: () => {
    console.warn("libcore NJSThreadDispatcher: newLock: Not implemented");
  }
});
...
const NJSLogPrinter = new lib.NJSLogPrinter({
  context: {},
  printError: message => log("Error", message),
  printInfo: message => log("Info", message),
  printDebug: message => log("Debug", message),
  printWarning: message => log("Warning", message),
  printApdu: message => log("Apdu", message),
  printCriticalError: message => log("CriticalError", message),
...
const loadCore = (): Promise<Core> => {
  ...
  class Currency {
    ...
    getBitcoinLikeNetworkParameters() {
      return new BitcoinLikeNetworkParameters(
        this.bitcoinLikeNetworkParameters
      );
    }
  ...
  function wrapMethod(className, methodName, wrap, paramsWrap) {
    ...
    Clz.prototype[methodName] = async function() {
      const args = paramsWrap
        ? [...arguments].map((value, i) =>
            paramsWrap[i] ? paramsWrap[i](value) : value
          )
        : arguments;
  ...
  statics.Amount.fromHex = (...args) =>
    new statics.Amount(...args).fromHex(...args);
