...
const BackgroundRunnerService = async ({ deviceId }: { deviceId: string }) => {
  const emitEvent = e => store.dispatch(addBackgroundEvent(e));

  const deviceInfo = await withDevice(deviceId)(t =>
    from(getDeviceInfo(t)),
  ).toPromise();
  emitEvent({ type: "deviceInfo", deviceInfo });

  const latestFirmware = await manager.getLatestFirmwareForDevice(deviceInfo);
  emitEvent({ type: "latestFirmware", latestFirmware });
  log(latestFirmware);
  ...
  const onNext = event => {
    emitEvent(event); // Forward them up so UI JS can update itself.
    ...
    if (displayedOnDevice) {
      ...
      NativeModules.BackgroundRunner.update(
        Math.round(progress.toFixed(4) * 100),
      );
    }
  };
