...
const makeProps = (overrides: Partial<FlagDetailsDialogProps> = {}): FlagDetailsDialogProps => ({
  setOverride: jest.fn(),
  display: baseDisplay,
  onClose: jest.fn(),
...
describe("FlagDetailsDialog", () => {
  it("labels the dialog with the selected flag id", () => {
    render(<FlagDetailsDialog {...makeProps()} />);
    expect(screen.getByRole("dialog", { name: "mockFeature" })).toBeInTheDocument();
  });
  ...
  it("calls onClose when the footer Cancel button is clicked", async () => {
    ...
    render(<FlagDetailsDialog {...makeProps({ onClose })} />);
  ...
  it("applies the edited value via setOverride when Apply is clicked", async () => {
    ...
    const user = userEvent.setup();
    render(<FlagDetailsDialog {...makeProps({ setOverride })} />);
    fireEvent.change(screen.getByRole("textbox"), { target: { value: '{ "enabled": true }' } });
