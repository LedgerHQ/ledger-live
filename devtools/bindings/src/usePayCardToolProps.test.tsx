...
describe("usePayCardToolProps", () => {
  ...
  it("hands the tool a row per joined wallet, priced by the host", async () => {
    const store = buildStore();
    await act(() => seedWallets(store));
    const { result } = renderHook(
      () => usePayCardToolProps({ resolveCounterValue: () => 12_540 }),
      {
        wrapper: withStore(store),
      },
    );
  ...
  it("leaves the Ledger currency off a row the catalog does not cover", async () => {
    ...
    expect(unmapped?.counterValueRaw).toBeNull();
  });
