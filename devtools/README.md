...
Use the `.web` or `.native` suffix on every component file that has platform-specific content.
...
The registry's loader is `() => import("@devtools/<id>")`. Web bundlers resolve to `"main"` (`index.ts`); native bundlers resolve to `"react-native"` (`index.native.ts`). Both must default-export the component. Use bare specifiers — `moduleSuffixes` in each platform tsconfig resolves the correct file at build time.
...
```ts
// devtools/registry/src/metadata/platform/my-tool.ts
...
import { featureFlags, type FeatureFlagsToolProps } from "./metadata/platform/feature-flags";
