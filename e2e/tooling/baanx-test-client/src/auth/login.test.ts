...
describe("loginToBaanx — mapped HTTP failures", () => {
  ...
  it("keeps the reason when the rejection comes from another realm, as a real fetch does", async () => {
    // The case above cannot catch this: its `new Error` is built in the test's own realm, so
    // `instanceof Error` is true for it. Real `fetch` under Jest rejects with a TypeError built in
    // Node's realm, where `instanceof Error` is false — which silently reduced every genuine
    // transport failure to "unknown transport failure". node:vm reproduces that exactly.
    const rejection = runInNewContext(
      `const e = new TypeError("fetch failed");
       e.cause = new Error("connect ECONNREFUSED 127.0.0.1:443");
       e;`,
    ) as { message: string };
    ...
    // The half that actually says what went wrong; undici puts it on `cause`.
    expect(error.message).toContain("ECONNREFUSED");
  });
