...
describe("resolveBaanxAuthConfig", () => {
  ...
  describe("missing configuration", () => {
    ...
    it("never puts a secret value in the message", () => {
      ...
      // Asserted outside the catch, and only after checking something was thrown:
      // inside it, a build that stopped throwing would pass with no assertions.
      expect(message).not.toBeNull();
      expect(message).not.toContain(RFC6238_SECRET);
      expect(message).not.toContain("env-password");
    });
  });
