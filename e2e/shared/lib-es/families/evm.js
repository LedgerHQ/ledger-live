import { containsSubstringInEvent, fetchCurrentScreenTexts, waitForReviewTransaction, pressUntilTextFound, waitFor, } from "../speculos";
import { getSpeculosModel, isTouchDevice } from "../speculosAppVersion";
import { longPressAndRelease, pressAndRelease, swipeRight, } from "../deviceInteraction/TouchDeviceSimulator";
...
function shouldSkipRecipientDisplayValidation(tx) {
    return (tx.accountToCredit.currency.id === Currency.POL.id ||
...
export const sendEvmButtonDevice = withDeviceController(({ getButtonsController }) => async (tx) => {
    const buttons = getButtonsController();
    const events = await pressUntilTextFound(DeviceLabels.SIGN_TRANSACTION);
    validateTransactionData(tx, events);
    await buttons.both();
});
export const sendEvmNanoS = withDeviceController(({ getButtonsController }) => async (tx) => {
    const buttons = getButtonsController();
...
export const approveTokenButtonDevice = withDeviceController(({ getButtonsController }) => async () => {
...
export const signTypedMessageButtonDevice = withDeviceController(({ getButtonsController }) => async () => {
    await waitFor(DeviceLabels.REVIEW_TYPED_MESSAGE);
