...
function buildAcquirePayload(deviceParams, runId, seed) {
    ...
    const libraries = dependencies?.map(dep => ({
        name: dep.name,
        path: `/apps/${conventionalAppSubpath(model, firmware, dep.name, dep.appVersion ?? appVersion)}`,
    })) ?? undefined;
    return {
        coin_app: appName,
...
export async function waitForSpeculosReady(deviceId, { interval = 2_000, timeout = 150_000 } = {}) {
...
export function getSpeculinhoRunIdFromError(error) {
...
export async function createSpeculosDeviceCI(deviceParams) {
    ...
    for (let attempt = 0; attempt < 5; attempt++) {
        ...
        let res;
        try {
            res = await axios.post(`${speculinhoUrl}/acquire`, payload, {
                headers: { "Content-Type": "application/json" },
                validateStatus: () => true,
            });
        }
        catch (error) {
            throw new Error(`[speculosCI] Speculinho /acquire transport error (${speculinhoUrl}): ${sanitizeError(error)}`);
        }
        if (res.status === 202) {
...
export async function releaseSpeculosDeviceCI(runId) {
    ...
    try {
        await axios.post(`${speculinhoUrl}/release`, { run_id: runId.toString() }, {
            headers: { "Content-Type": "application/json" },
 ...
 */
export async function fetchSpeculinhoLogs(runId) {
    const speculinhoUrl = getSpeculinhoBaseUrl();
    if (!speculinhoUrl) {
 ...
 */
export async function fetchSpeculinhoStatus(runId) {
    const speculinhoUrl = getSpeculinhoBaseUrl();
    if (!speculinhoUrl) {
