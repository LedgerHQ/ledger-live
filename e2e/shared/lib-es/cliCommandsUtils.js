...
import { runCliGetAddress, runCliGetTokenAllowance, runCliLiveData, runCliTokenApproval, } from "./runCli";
...
import { applyGeneratedUserdata, getGeneratedAddress, hasGeneratedUserdata, } from "./generatedUserdata";
import { getCachedAddress, isUtxoBasedCurrency } from "./addressCache";
import { getCachedUtxoAddress } from "./utxoAddressCache";
export const getAccountAddress = async (account) => {
...
export const liveDataCommand = (account, options) => {
 ...
 */
export const addEmptyAccountCommand = (account, options) => async (userdataPath) => {
    if (!userdataPath) {
    ...
    const seedIdPath = getSeedIdentifierDerivation(cryptoCurrency, derivationMode);
...
export const liveDataWithAddressCommand = (account, options) => {
    const cmd = async (userdataPath) => {
    ...
    cmd.canUseGeneratedUserdata = () => hasGeneratedUserdata(account) && !isUtxoBasedCurrency(account.currency.id);
    return cmd;
};
export const liveDataWithParentAddressCommand = (liveDataAccount, accountToAssign) => {
    const cmd = async (userdataPath) => {
    ...
    cmd.canUseGeneratedUserdata = () => hasGeneratedUserdata(liveDataAccount) &&
...
export const liveDataWithRecipientAddressCommand = (tx, options) => {
...
export function parseTokenAllowanceCliOutput(output) {
 ...
 */
export const isTokenAllowanceSufficientCommand = async (account, spenderAddress, minAmount) => {
 ...
 */
export const getTokenAllowanceCommand = async (account, spenderAddress) => {
    const ownerAddress = account.parentAccount?.address ?? account.address;
    if (!ownerAddress)
        throw new Error("Token allowance check requires the main account address");
    const output = await runCliGetTokenAllowance({
 ...
 */
export const approveTokenCommand = async (account, spender, approveAmount) => {
...
export const revokeTokenCommand = async (account, spender) => {
    const original = setDisableTransactionBroadcastEnv("0");
    const result = runCliTokenApproval({
        currency: account.currency.speculosApp.name,
        index: account.index,
...
export function setDisableTransactionBroadcastEnv(value) {
