...
exports.pickRotatingProvider = pickRotatingProvider;
...
function isQuoteErrorItem(item) {
    return typeof item === "object" && item !== null && "parameter" in item;
}
async function getMinimumSwapAmount(accountFrom, accountTo, providersWhitelist) {
    try {
        ...
        if (!Array.isArray(data)) {
            ...
            return await (0, currencyUtils_1.getAmountFromUSD)(accountFrom.currency.id, FALLBACK_TARGET_USD);
        }
        ...
        console.warn(`No minAmount from quote API for ${accountFrom.currency.id} → ${accountTo.currency.id}, ` +
...
async function keepRunningProviders(eligibleProviders, accountFrom, accountTo) {
    const addressFrom = accountFrom.address || accountFrom.parentAccount?.address;
    if (!addressFrom) {
        console.warn("No address available from accounts when checking provider health.");
        return eligibleProviders;
    }
    const amountFrom = await getMinimumSwapAmount(accountFrom, accountTo, eligibleProviders.map(p => p.name));
    if (amountFrom == null) {
        return eligibleProviders;
    }
    try {
        ...
        const { data } = await (0, axios_1.default)(requestConfig);
        if (!Array.isArray(data)) {
            console.warn("Unexpected quote API response while checking provider health; keeping full providers list.");
            return eligibleProviders;
        }
        ...
        console.warn("Error checking swap provider health:", (0, index_1.sanitizeError)(error));
        return eligibleProviders;
    }
}
async function pickRotatingProvider(eligibleProviders, accountFrom, accountTo) {
    ...
    if (runningProviders.length === 0) {
        throw new Error(`[Providers Health Check] - no running swap provider for ` +
    ...
    if (override) {
        ...
        if (!match) {
            throw new Error(`[Providers Health Check] - ❌ "${override}" did not match any of the healthy providers: ` +
    ...
    if (dropped.length > 0) {
        console.warn(`[Providers Health Check] - Some quotes are down for ${accountFrom.currency.name} → ` +
    ...
    for (let offset = 0; offset < eligibleProviders.length; offset++) {
        ...
        if (isRunning.includes(candidate.name)) {
            if (offset > 0) {
                console.warn(`[Providers Health Check] - ❌ Scheduled provider "${eligibleProviders[scheduledIndex].name}" ` +
                    `is down; using "${candidate.name}" instead`);
            }
            return candidate;
        }
    }
    throw new Error(`[Providers Health Check] - No healthy provider found for ` +
        `${accountFrom.currency.name} → ${accountTo.currency.name}`);
}
