...
exports.parseExtraFeatureFlags = parseExtraFeatureFlags;
function isOptionalFeatureMap(value) {
 ...
 */
function parseExtraFeatureFlags(rawExtraFeatureFlags) {
    const normalizedValue = rawExtraFeatureFlags?.trim();
    if (!normalizedValue)
        return {};
    const helpText = "Expected E2E_FEATURE_FLAGS_JSON to be a JSON object. For example: " +
    ...
    try {
        ...
        throw new Error(`Invalid E2E_FEATURE_FLAGS_JSON: ${error instanceof Error ? error.message : String(error)}. ${helpText}`);
    }
    if (!isOptionalFeatureMap(parsed)) {
        throw new Error(`Invalid E2E_FEATURE_FLAGS_JSON: E2E_FEATURE_FLAGS_JSON must be a JSON object mapping feature flag keys to configuration objects. ${helpText}`);
    }
    return parsed;
}
//# sourceMappingURL=featureFlagsJsonUtils.js.map