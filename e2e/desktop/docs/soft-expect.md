...
[Playwright soft assertions](https://playwright.dev/docs/test-assertions#soft-assertions) record a failure without stopping the test. The test continues, all soft failures are reported together at the end, and the run still fails if any soft check failed.
