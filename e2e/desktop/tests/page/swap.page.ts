...
export class SwapPage extends AppPage {
  ...
  private readonly maxSpendableToggle = this.page.getByTestId("swap-max-spendable-toggle");
  private readonly fromAccountCoinSelector = "from-account-coin-selector";
  private readonly fromAccountAmoutInput = "from-account-amount-input";
  private readonly toAccountCoinSelector = "to-account-coin-selector";
  private readonly errorSpan = (text: RegExp | string) =>
    `span[color*="error"]:has-text("${text}")`;
  private readonly numberOfQuotes = "number-of-quotes";
  private readonly originCurrencyAmount = this.page.getByTestId("origin-currency-amount-value");
  private readonly destinationCurrencyDropdown = this.page.getByTestId(
    "destination-currency-dropdown",
  );
  private readonly destinationCurrencyAmount = this.page.getByTestId("destination-currency-amount");
  private readonly feesValue = this.page.getByTestId("fees-value");

  // Exchange Button Component
  private readonly exchangeButton = this.page.getByTestId("exchange-button");

  // Exchange Drawer Components
  readonly swapId = this.page.getByTestId("swap-id");

  private readonly chooseAssetDrawer = new ChooseAssetDrawer(this.page);
