...
export class MarketPage extends AppPage {
  ...
  private starredOptionFilter = this.page.getByRole("option", {
    name: "Starred Assets",
  });
  ...
  // Wait for one of the two mutually-exclusive markers (legacy search input vs disco category tab)
  // so the variant check can't race the initial load and pick the wrong branch.
  async isLegacyMarketList(): Promise<boolean> {
    await this.searchInput.or(this.allCategoryTab).first().waitFor({ state: "visible" });
