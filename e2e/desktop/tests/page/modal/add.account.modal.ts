...
export class AddAccountModal extends Modal {
  private readonly selectAccount = this.page.locator("text=Choose a crypto asset"); // FIXME: I need an id
  ...
  private readonly deselectAllButton = this.page.getByText("Deselect all");
  private readonly checkbox = this.page.getByTestId("accountRow-checkbox").first();
  private readonly accountsList = this.page.getByTestId("add-accounts-step-import-accounts-list");
  private readonly stopButton = this.page.getByTestId("add-accounts-import-stop-button");
  private readonly doneButton = this.page.getByTestId("add-accounts-finish-close-button");
  private readonly infoBox = this.page.getByTestId("add-token-infoBox");
  private readonly addSubAccountButton = this.page.getByTestId("modal-continue-button");
  private readonly successAddLabel = this.page.locator("text=Account added successfully");
  private readonly selectAccountInList = (Currency: Currency) =>
    this.page.getByRole("option", {
      name: `${Currency.name} (${Currency.ticker})`,
      exact: true,
    });
  private readonly selectTokenNetwork = (SubAccount: Account) =>
    this.page
      .getByRole("option", {
