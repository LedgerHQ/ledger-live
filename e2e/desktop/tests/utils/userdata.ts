...
/**
 * Waits up to `timeoutMs` for app.json to contain a data.identities.userId that differs from
 * `previousUserId` — i.e. regenerated by the Reset App reboot. Transient absence or partial writes
 * during the reset are tolerated; unexpected filesystem errors are rethrown so they fail fast.
 */
export async function waitForIdentitiesRegenerated(
  userdataFile: string,
  ...
  while (Date.now() < deadline) {
    try {
      const parsed: { data?: { identities?: { userId?: string } } } =
        await getUserdata(userdataFile);
      lastUserId = parsed?.data?.identities?.userId;
      if (
        typeof lastUserId === "string" &&
        lastUserId.length > 0 &&
      ...
      // app.json is briefly absent or half-written while hardReset() and the reboot rewrite it
  ...
  throw new Error(
    `app.json identities.userId was not regenerated (still "${lastUserId ?? "absent"}") within ${timeoutMs}ms.`,
  );
}
