...
export default class SendPage {
  ...
  private async expectRecipientMessageDescription(message: string) {
    const text = await getTextOfElement(this.recipientErrorDescriptionId);
    console.log("text", text);
  ...
  private async expectSenderMessageError(title: string, description: string) {
    const text = await getTextOfElement(this.senderErrorTitleId);
    console.log("text", text);
    const text2 = await getTextOfElement(this.senderErrorDescriptionId);
