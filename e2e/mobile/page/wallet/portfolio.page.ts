...
export default class PortfolioPage {
  ...
  assetItemCurrencyTickerId = (currencyName: string) =>
    `${this.assetItemId(currencyName)}-currency-ticker`;
  ...
  async checkSelectAssetPage() {
    ...
    for (let i = 0; i < assetsCount; i++) {
      ...
      await detoxExpect(this.bigCurrencySubTitle(currencyId)).toBeVisible();
      await detoxExpect(this.bigCurrencySubTitle(currencyId)).toHaveText(currencyObj.ticker);
    }
  }
}
