...
export default class AddAccountDrawer extends CommonPage {
  ...
  async expectAccountDiscovery(accountName: string, currencyId: string, accountRowIndex = 0) {
    await detoxExpect(this.accountItem(this.accountIdAccountDrawer(currencyId))).toBeVisible();
    const accountId = (await getIdByRegexp(this.accountItemRegExp(), accountRowIndex)).replace(
      this.accountItemId,
      "",
    );
