...
export async function launchSpeculos(appName: string) {
  ...
  // Keep offset within a safe range to avoid exceeding 65535 when mapped in transport
  // Transport caps internally, but we keep it sane at source as well.
  const rawOffset = Math.max(0, freeApiPort - 30000) + jestWorkerId;
  const safeOffset = rawOffset % 30000; // 30000 base + 30000 max => <= 60000
  setEnv("SPECULOS_PID_OFFSET", safeOffset);
