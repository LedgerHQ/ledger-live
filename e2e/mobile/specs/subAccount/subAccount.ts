...
async function checkOperationInfos(
  ...
  await app.operationDetails.checkRecipientAddress(
    postTransaction ? transaction.accountToCredit : transaction.accountToCredit.parentAccount!,
  );
  ...
  describe("Add subAccount without parent", () => {
    ...
    it(`Add Sub Account without parent (${asset.currency.speculosApp.name}) - ${asset.currency.ticker}`, async () => {
      ...
      const accountId = await app.addAccount.addAccountAtIndex(
        asset?.parentAccount === undefined ? asset.accountName : asset.parentAccount.accountName,
        asset?.parentAccount === undefined
