...
`ref` is `{ accountId, currencyId, address, derivationMode }` — four strings, derivable from
anything the app already holds, including a Ledger Sync descriptor that was never synced. The
`accountId` is an `AccountId`, which refuses the `+` a token account id carries: a token-account ref
is unrepresentable, and a token balance is read from `subAccountBalances` instead.
...
```ts
const rows = useSelector(accountBalancesSlice.selectors.selectAccountBalanceRows);
```
...
context. Freshness and de-duplication are two guards in the body of a plain thunk — not RTK's
`createAsyncThunk` `condition` — reading state that is already in the store.
...
- **`account-data/` sits in live-common, which is maintenance-only.** Raised in review and worth
  keeping open rather than settling by assertion, because the code there is not one thing:
  - `legacy-mapping/` and the full-sync halves (`syncAccountOnce`, `syncAccountBalanceRows`,
    `syncAccountOperations`) **die with the legacy account model.** A new package for them would be
    a permanent-looking home for code whose purpose is to be deleted, and it would still depend on
    live-common for the types and the bridge it wraps.
  - `getAccountBalanceRows` and `getAccountOperationPage` **do not.** They are the new world's
    readers over `CoinModuleApi`, and they outlive `Account` entirely. Those are the ones that
    deserve a package of their own — and the reason they have not moved yet is that this is an
    exploration whose deliverable is a finding, not a package layout.

  The split to aim for, when this stops being an exploration: granular readers in their own
  `libs/*`, full-sync glue left in live-common to be deleted with it.
