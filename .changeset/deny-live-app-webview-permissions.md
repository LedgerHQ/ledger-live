...
Live Apps no longer run in the host's session at all. A manifest without a `cacheBustingId` used to leave the `partition` attribute unset, putting the guest in the renderer's own session, where `setDevicePermissionHandler` grants any Ledger HID device: `navigator.hid.getDevices()` and `HIDDevice.open()` consult that handler without passing through the `hid` check or `select-hid-device`, so neither new guard applied to them. Those apps now share a `persist:live-apps` partition, and apps pinning a `cacheBustingId` keep their own as before.
