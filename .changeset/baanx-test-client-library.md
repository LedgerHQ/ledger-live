...
Includes an opt-in live integration test (`test-integ`) that logs in against the sandbox; it is excluded from the hermetic unit run and skips when credentials are not configured.
