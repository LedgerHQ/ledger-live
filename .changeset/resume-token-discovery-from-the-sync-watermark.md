...
from its stored token sub-accounts) and the lower bound of the scan (`scanAssetsMinHeight`, the same watermark
...
The completeness `scanAssetsMinHeight` requires is held by the existing `syncHash`, which covers the
