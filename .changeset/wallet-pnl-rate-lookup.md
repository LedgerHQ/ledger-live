...
No caller changes: the exported signatures keep their arity and the state argument is assignable
as before, so the 29 files consuming the package are untouched.
...
snapshot; they now live behind `@domain/entity-market-countervalues/mock`. They stay reachable
from `@ledgerhq/wallet-pnl/scenarios`, which the web-tools pnl-calculator imports.
