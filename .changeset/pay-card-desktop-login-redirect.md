...
The desktop login block also renders the centred design: a title, a description, the login action, and a second action under it that starts the login for a card holder who has an account already. Both actions take the Lumen `Button`, the first on `base` and the second on `no-background`, which are the appearances the design names. The link shows only while the intro has not been seen, because the login action is the login after that. Mobile keeps its inline row. Both read the same copy keys.
