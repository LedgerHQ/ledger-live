...
unreachable `Expand`, `Balance.web` and `CardOnboarding.web` components, the `metadata` and
web-only `hooks` barrels, and the named exports that duplicated a default export.
`@devtools/protocols` now enumerates its two subpath exports instead of a `./*` wildcard.
