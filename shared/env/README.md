...
In this PR, `@shared/env` is a typed re-export of `@ledgerhq/live-env`:
...
```ts
import { getEnv, setEnv, setEnvUnsafe, EnvName, changes } from "@shared/env";
```
...
- Published `libs/*` packages: must use `@ledgerhq/live-env` directly — `@shared/env` is not published to npm.
