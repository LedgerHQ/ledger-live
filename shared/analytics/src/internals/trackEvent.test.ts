...
describe("trackEvent", () => {
  describe("enrichment", () => {
    it("sends with sync extras", async () => {
      ...
      await trackEvent("track", "Sync Event", {}, false);
    ...
    it("sends after async extras resolve", async () => {
      ...
      const pending = trackEvent("track", "Async Event", {}, false);
    ...
    it("calls the extra props function with no arguments", async () => {
      ...
      await trackEvent("track", "Stateful", {}, false);
    ...
    it("lets extra props win over caller props", async () => {
      ...
      await trackEvent("track", "Collision", { platform: "caller-supplied" }, false);
    ...
    it("uses mandatory extra props for mandatory events", async () => {
      ...
      await trackEvent("track", "Mandatory Event", { flow: "onboarding" }, true);
    ...
    it("reports failed_enrichment when sync extras throw", async () => {
      ...
      await trackEvent("track", "Unenrichable", { foo: "bar" }, false);
  ...
  describe("property filter", () => {
    it("filters caller props before sending", async () => {
      ...
      await trackEvent("track", "Tracking Event", { sensitive: "data to filter", theme: "light" }, false);
    ...
    it("filters enriched props before sending", async () => {
      ...
      await trackEvent("track", "Enriched Event", { theme: "light" }, false);
    ...
    it("publishes filtered payloads to analyticsEvents$ on success", async () => {
      ...
      await trackEvent("track", "Subject Event", { theme: "light" }, false);
    ...
    it("reports failed_filter when the property filter throws", async () => {
      ...
      await trackEvent("track", "Filtered Event", { theme: "light" }, false);
