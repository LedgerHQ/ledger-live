...
This package provides the **analytics methods** for Ledger Wallet apps to use.

Each app brings its own **analytics client** (e.g. Segment), **logger**, **extra properties** and **consent state**.
This package provides the methods to do that setup, and the tracking functions for tracking analytics events.

## How it works

1. App calls `track` wtih event name, props and options

→ gather route data (for trackPage and trackScreen only)
→ check consent (uses a registered tracking selector)
...
| `setIsTrackingEnabledSelector(fn)`  | Consent gate                         | **While unset, tracking is always enabled** — the CLI relies on this |
...
setStore(store);
setIsTrackingEnabledSelector((state) =>
  trackingEnabledSelector(state as AppState)
);
