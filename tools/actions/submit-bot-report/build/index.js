(() => {
  var __webpack_modules__ = {
    ...
    910: (e, p, d) => {
      ...
      const R = function isDomainOrSubdomain(e, p) {
  ...
  (() => {
    ...
    async function main() {
      ...
      const t = d.getInput("slackChannel");
      const s = d.getInput("slackApiToken");
      const o = l.resolve(d.getInput("path"));
      const n = m.promises.readFile(l.join(o, "full-report.md"), "utf-8");
      const u = m.promises.readFile(
        l.join(o, "slack-comment-template.md"),
        "utf-8"
      );
      const v = `https://api.github.com/repos/LedgerHQ/ledger-live/commits/${i}/comments`;
      console.log("sending to " + v);
      const c = await p()(v, {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({ body: await n }),
      })
        .then(handleErrors)
        .then((e) => e.json());
      if (s && c) {
        const e = await u;
        const d = e.replace("{{url}}", c.html_url);
        await p()("https://slack.com/api/chat.postMessage", {
          ...
          body: JSON.stringify({ text: d, channel: t || "ledger-live-bot" }),
        }).then(handleErrors);
      }
    }
    main().catch((e) => {
