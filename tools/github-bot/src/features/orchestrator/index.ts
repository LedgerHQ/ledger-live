...
export function orchestrator(app: Probot) {
  ...
  app.on("workflow_run.requested", async context => {
    ...
    if (matchedWorkflow) {
      context.log.info(`[Orchestrator](workflow_run.requested) ${payload.workflow_run.name}`);
  ...
  app.on("workflow_run", async context => {
    ...
    if (matchedWorkflow) {
      context.log.info(`[Orchestrator](workflow_run.in_progress) ${payload.workflow_run.name}`);
      const workflowUrl = payload.workflow_run.html_url;
      const summaryPrefix = matchedWorkflow.description
        ? `#### ${matchedWorkflow.description}\n\n`
        : "";
      ...
      const [tips, workflowRun] = await Promise.all([tipsPromise, workflowRunPromise]);
  ...
  app.on("workflow_run.completed", async context => {
    ...
    if (workflowFile === "gate.yml") {
      ...
      context.log.info(`[Orchestrator](workflow_run.completed) ${payload.workflow_run.name}`);
      ...
      context.log.info(
        `[Orchestrator](workflow_run.completed) Linked artifacts ${artifacts.map(a =>
          JSON.stringify({
            name: a.name,
            id: a.id,
      ...
      const [affected, metadata] = (await Promise.all(
        ["affected.json", "pr_metadata.json"].map(
          async (
            fileName: string,
          ): Promise<Record<string, { path: string }> | PullRequestMetadata | undefined> => {
            const artifactId = artifacts.find(artifact => artifact.name === fileName)?.id;
            ...
            try {
              const raw = await downloadArtifact(octokit, owner, repo, artifactId);
      ...
      const localRef = `forked/${checkSuite.head_sha}`;
  ...
  app.on(["check_run.created"], async context => {
    ...
    if (matchedWorkflow) {
      ...
      const result = await updateWatcherCheckRun(octokit, owner, repo, payload.check_run.head_sha);
      if (result) {
        context.log.info(
  ...
  app.on("check_run.requested_action", async context => {
    ...
    const workflowRun = data.workflow_runs.reduce<WorkflowRunPayload["workflow_run"] | undefined>(
      (wf, curr) => {
