...
test("losing the implementation we converge on is reported", () => {
  const known = {
    "@noble/curves": { target: true, reason: "the target" },
    elliptic: { tolerated: true, reason: "legacy", exit: "LIVE-37448" },
  };
  const watched = new Map([["elliptic", "app > elliptic"]]);
  assert.deepEqual(findViolations(watched, [], known), [
    "@noble/curves is the implementation we converge on, and it is not in the tree",
  ]);
});
