...
| `ledger-sync enroll`                  | Enroll/restore Ledger Sync on this machine (device required). **Separate trust model from `ring` and `agent-intent`** — see the [ledger-wallet-cli skill](../../.agents/skills/ledger-wallet-cli/SKILL.md#ledger-sync).                    |
