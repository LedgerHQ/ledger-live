import { Challenge, crypto } from "@ledgerhq/hw-ledger-key-ring-protocol";
import type { Route } from "./mock-server";

const CHALLENGE_TLV =
  "";
const [CHALLENGE] = Challenge.fromBytes(crypto.from_hex(CHALLENGE_TLV));
const LKRP_TOKEN = makeJwt({ sub: "lkrp", exp: 4102444800 });
export const KEYCLOAK_TOKEN = makeJwt({ sub: "wallet-cli", exp: 4102444800 });

/** Body of the LKRP challenge response: `credential.publicKey` is the key that signed it. */
export type SignedChallenge = {
  signature: { credential: { publicKey: string }; attestation?: string };
};

type AuthRouteOptions = {
  onChallengeRequest?: (request: Request) => void;
  onAuthenticateRequest?: (signed: SignedChallenge) => void;
};

/** Keycloak + LKRP OIDC routes that let `AuthSDK` obtain {@link KEYCLOAK_TOKEN}. */
export function makeAuthRoutes({
  onChallengeRequest,
  onAuthenticateRequest,
}: AuthRouteOptions = {}): Route[] {
  return [
    {
      method: "GET",
      match: /\/protocol\/openid-connect\/auth(\?|$)/,
      response: null,
      status: 302,
      headers: {
        Location: "/auth/challenge",
        "Set-Cookie": "AUTH_SESSION_ID=wallet-cli-test; Path=/; HttpOnly",
      },
    },
    {
      method: "GET",
      match: "/auth/challenge",
      response: { tlv: CHALLENGE_TLV, json: CHALLENGE.toJSON() },
      onRequest: onChallengeRequest,
    },
    {
      method: "POST",
      match: "/openid/v1/authenticate",
      response: "wallet-cli-auth-code",
      onRequest: async request => {
        onAuthenticateRequest?.((await request.json()) as SignedChallenge);
      },
    },
    {
      method: "POST",
      match: "/openid/v1/token",
      response: { access_token: LKRP_TOKEN, token_type: "Bearer" },
    },
    {
      method: "POST",
      match: "/openid/v1/exchange",
      response: { access_token: KEYCLOAK_TOKEN, token_type: "Bearer" },
    },
  ];
}

function makeJwt(payload: Record<string, unknown>): string {
  const prefix = Buffer.from(JSON.stringify({ alg: "none", typ: "JWT" })).toString("base64url");
  const body = Buffer.from(JSON.stringify(payload)).toString("base64url");
  return `${prefix}.${body}.signature`;
}
