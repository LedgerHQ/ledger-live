...
const App = () => {
  ...
  const [derivationsPerCurrency, setDerivationsPerCurrency] = useState<
    { currency: { name: string; id: string }; derivations: { name: string; path: string }[] }[]
  >([]);
      ...
      currencies.map(currency => {
        ...
        return {
          ...
          derivations: getDerivationModesForCurrency(currency).map(derivationMode => {
            ...
            return {
              name: derivationMode,
              path: runDerivationScheme(deriationScheme, currency, {
