...
module.exports = function ({ types: t }) {
  ...
  function extractImportPath(node) {
    ...
    if (
      !t.isIdentifier(node.callee, { name: "register" }) &&
      !t.isIdentifier(node.callee, { name: "lazyLoad" })
    ) {
      return null;
    }

    if (node.arguments.length !== 1) {
      // const callName = node.callee?.name || "register";
      // console.log(
      //   `[Babel Plugin] extractImportPath: ${callName} call has ${node.arguments.length} arguments, expected 1`,
      // );
      return null;
    }
    ...
    if (!t.isObjectExpression(node.arguments[0])) {
      // const callName = node.callee?.name || "register";
      // console.log(
      //   `[Babel Plugin] extractImportPath: ${callName} argument is not an object expression`,
      // );
      return null;
    }
    ...
    visitor: {
      ...
      VariableDeclarator(path, state) {
        ...
        if (importPathValue) {
          ...
          // const callName = node.init.callee?.name || "register";
          // console.log(
          //   `[Babel Plugin] Transforming VariableDeclarator: ${varName} = ${callName}(...) -> import * as ${moduleName} from "${importPathValue}"; const ${varName} = ${moduleName}.${varName} || ${moduleName}.default`,
          // );
      ...
      CallExpression(path, state) {
        ...
        if (
          ...
          // console.log(`[Babel Plugin] Visiting CallExpression: ${node.callee.name}(...)`);
        }
        ...
        if (importPathValue) {
          ...
          // const callName = node.callee?.name || "register";
          // console.log(
          //   `[Babel Plugin] Transforming CallExpression: ${callName}(...) -> _ImportedComponent_X`,
          // );
