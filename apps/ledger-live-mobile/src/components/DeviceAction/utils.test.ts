...
describe("utils", () => {
  ...
  describe("getFlowName", () => {
    ...
    it("map for receiveFlow and swapFlow", () => {
      expect(getFlowName(HOOKS_TRACKING_LOCATIONS.receiveFlow, {})).toBe("receive");
      expect(getFlowName(HOOKS_TRACKING_LOCATIONS.swapFlow, {})).toBe("swap");
    });

    it("sendFlow + transaction.mode === 'send' -> 'send'", () => {
      const req = { transaction: { mode: "send" } };
      expect(getFlowName(HOOKS_TRACKING_LOCATIONS.sendFlow, req)).toBe("send");
    });
    ...
    it("sendFlow + transaction.mode !== 'send' -> 'staking'", () => {
      ...
      expect(getFlowName(HOOKS_TRACKING_LOCATIONS.sendFlow, req1)).toBe("staking");
      expect(getFlowName(HOOKS_TRACKING_LOCATIONS.sendFlow, req2)).toBe("staking");
      expect(getFlowName(HOOKS_TRACKING_LOCATIONS.sendFlow, req3)).toBe("staking");
    });
