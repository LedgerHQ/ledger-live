...
export default function StakingSummary({ navigation, route }: Props) {
  ...
  const onChangeAmount = () => {
    navigation.navigate(ScreenName.SuiStakingAmount, {
      ...
      nextScreen: ScreenName.SuiStakingValidator,
    });
  };

  const onContinue = useCallback(async () => {
    navigation.navigate(ScreenName.SuiStakingSelectDevice, {
    ...
    <SafeAreaView style={[styles.root, { backgroundColor: colors.background }]}>
        ...
        <DelegatingContainer
          left={
            <View style={styles.delegatingAccount}>
              <Circle size={64} bg={rgba(color, 0.2)}>
                <CurrencyIcon size={64} currency={currency} />
              </Circle>
          ...
          right={
            <Touchable event="StakingFlowSummaryChangeCircleBtn" onPress={onChangeValidator}>
              <Circle size={64} style={[styles.validatorCircle, { borderColor: colors.primary }]}>
                <Animated.View
                  style={{
                    transform: [
                      {
                ...
                >
                  <ValidatorImage url={chosenValidator?.imageUrl} name={chosenValidator?.name} />
                </Animated.View>
                <ChangeValidator />
              </Circle>
            </Touchable>
      ...
      <View style={styles.footer}>
        ...
        <Button
          ...
          testID="sui-summary-continue-button"
        />
      </View>
    </SafeAreaView>
  );
...
function SummaryWords({
  ...
  readonly onChangeValidator: () => void;
  ...
  return (
    <>
      <Line>
        <Words>
          <Trans i18nKey="sui.staking.iStake" />
        </Words>
        <Touchable onPress={onChangeAmount}>
          <Selectable name={formattedAmount} testID="sui-delegation-summary-amount" />
        </Touchable>
      </Line>
      <Line>
        <Words>
        ...
        <Touchable onPress={onChangeValidator}>
          <Selectable
            name={validator?.suiAddress ?? "-"}
            testID="sui-delegation-summary-validator"
          />
        </Touchable>
      </Line>
    </>
...
const Selectable = ({ name, testID }: { readonly name: string; readonly testID: string }) => {
  const { colors } = useTheme();
  return (
