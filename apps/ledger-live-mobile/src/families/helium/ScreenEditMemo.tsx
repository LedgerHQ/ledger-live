...
function HeliumEditMemo({ navigation, route }: Props) {
  const { colors } = useTheme();
  const { t } = useTranslation();
  const { model } = route.params.transaction;

  invariant(model.mode === "send", "must be a send tx");

  const [memo, setMemo] = useState(model.memo);
...
export { HeliumEditMemo as component, options };
