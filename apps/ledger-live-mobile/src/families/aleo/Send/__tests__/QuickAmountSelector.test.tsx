...
describe("QuickAmountSelector", () => {
  ...
  it("renders null for a token account of another family", () => {
    const nonAleoAccount = {
      ...ALEO_TOKEN_ACCOUNT_1,
      token: { ...ALEO_TOKEN_ACCOUNT_1.token, parentCurrencyId: "bitcoin" },
    };

    const { toJSON } = renderSelector(nonAleoAccount, makePrivateTransaction());
