...
export default function ViewKeyApproveScreen({ route, navigation }: Props) {
  ...
  const { accountsToAdd: allAccountsToAdd, currency, device, onCloseNavigation } = route.params;

  // Stable memos prevent the device transport session from resetting mid-approval when
  // existingAccounts changes (freshAddress is stable even when scanned account id differs).
  const existingAddresses = useMemo(
    () => new Set(existingAccounts.map(a => a.freshAddress)),
    [existingAccounts],
  );
  const accountsToAdd = useMemo(
    () => allAccountsToAdd.filter(a => !existingAddresses.has(a.freshAddress)),
    [allAccountsToAdd, existingAddresses],
  );
  ...
  const onResult = useCallback(() => {
    ...
    if (accountsWithViewKeys.length === 0) {
      if (accountsToAdd.length > 0 && rejectedAccountIds.size === accountsToAdd.length) {
  ...
  return (
    <SafeAreaView style={[styles.root, { backgroundColor: colors.background.main }]}>
      <DeviceActionDefaultRendering
        ...
        onResult={onResult}
      />
      {hookState.sharePending || payload !== null ? (
