...
export default function UnstakingDrawer({
  ...
  // Aleo tracks a single unbonding position, so anything in flight blocks the claim too.
  const canClaim = unbonding.isClaimable && !position.hasPendingUnbondingChange;
    ...
    () => [
      {
        ...
        Icon: (props: IconProps) => (
          <Circle {...props} bg={canClaim ? rgba(colors.green, 0.2) : colors.lightFog}>
            <ClaimRewardIcon color={canClaim ? colors.green : colors.grey} />
          </Circle>
        ),
