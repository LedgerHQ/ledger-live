 ...
 * change would otherwise cost a bridge call. Stays `null` until the first estimate lands, and on a
 * failed one — the figure is a hint and the bridge validates the amount regardless, so a rejected
 * estimate leaves the hint unrendered rather than faulting the screen.
 *
