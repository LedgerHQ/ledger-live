...
import DelegationLabelRight from "../../cosmos/Delegations/LabelRight";
...
function Delegations({ account }: Props) {
  ...
  const onDelegate = useCallback(() => {
    onNavigate({
      route: NavigatorName.OsmosisDelegationFlow,
      screen: delegations.length > 0 ? ScreenName.OsmosisDelegationValidator : ScreenName.OsmosisDelegationStarted,
    });
  }, [onNavigate, delegations]);

  const onRedelegate = useCallback(() => {
    onNavigate({
      route: NavigatorName.OsmosisRedelegationFlow,
      screen: ScreenName.OsmosisRedelegationValidator,
      params: {
  ...
  const onCollectRewards = useCallback(() => {
    onNavigate({
      route: NavigatorName.OsmosisClaimRewardsFlow,
      screen: delegation
  ...
  return (
    <View style={styles.root}>
      <DelegationDrawer
        ...
        ValidatorImage={({ size }) => (
          <ValidatorImage
            isLedger={(delegation || undelegation)?.validatorAddress === LEDGER_OSMOSIS_VALIDATOR_ADDRESS}
            name={
        ...
        amount={(delegation || undelegation)?.amount ?? new BigNumber(0)}
        data={data}
        actions={actions}
      />
      {totalRewardsAvailable.gt(0) && (
        <>
          ...
          >
            ...
            <Button
              event="Osmosis AccountClaimRewardsBtn Click"
              onPress={onCollectRewards}
              type="primary"
