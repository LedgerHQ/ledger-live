...
export default function SelectValidator({ navigation, route }: Props) {
  ...
  const {
    ...
    loading,
    error,
  } = useEvmStakingValidators(account.currency.id, searchQuery);
  const validators = useMemo(() => {
    const query = searchQuery.toLowerCase().trim();
    const isDetox = Config.DETOX === "true" || Config.DETOX === "1";
    const e2eValidators = isDetox ? account.stakingResources?.validators ?? [] : [];
  ...
  }, [account.stakingResources?.validators, fetchedValidators, searchQuery]);
