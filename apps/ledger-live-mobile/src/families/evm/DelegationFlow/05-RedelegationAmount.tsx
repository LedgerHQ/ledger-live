...
export default function RedelegationAmount() {
  const { t } = useTranslation();
  const navigation =
    useNavigation<
      StackNavigatorNavigation<EvmDelegationFlowParamList, ScreenName.EvmRedelegationAmount>
    >();
  const route =
    useRoute<StackNavigatorRoute<EvmDelegationFlowParamList, ScreenName.EvmRedelegationAmount>>();
  ...
  const max = useMemo(
    () =>
      account.stakingResources.delegations.find(
        delegation => delegation.validatorAddress === evmTransaction.valAddress,
      )?.amount ?? new BigNumber(0),
    [account.stakingResources.delegations, evmTransaction.valAddress],
  );

  const totalFees = useMemo(
    () => ("totalFees" in status ? status.totalFees : status.estimatedFees) ?? new BigNumber(0),
    [status],
  );
  const amount = evmTransaction.amount ?? new BigNumber(0);
  const remaining = max.minus(amount);
  ...
  const canContinue =
    !bridgePending && !bridgeError && !hasErrors && amount.gt(0) && max.gte(amount);
  ...
  const onContinue = useCallback(() => {
    navigation.navigate(ScreenName.EvmRedelegationSelectDevice, {
      ...route.params,
      transaction,
          ...
          <View style={styles.container}>
            <View style={styles.main}>
              <CurrencyInput
                ...
                hasError={hasErrors || max.lt(amount)}
                testID="evm-redelegation-amount-input"
              />
              <TouchableOpacity
                style={styles.maxButton}
                onPress={onUseMax}
                testID="evm-redelegation-use-max"
              >
            ...
            <View style={styles.footer}>
              ...
              <SummaryRow title={<Trans i18nKey="account.delegatedAssets" />}>
                <LText semiBold>
                  <CurrencyUnitValue unit={unit} value={max} showCode />
                </LText>
              </SummaryRow>
              ...
              <Button
                ...
                event="Evm RedelegationAmountContinueBtn"
                onPress={onContinue}
                title={<Trans i18nKey="common.continue" />}
                type="primary"
                testID="evm-redelegation-amount-continue"
              />
            </View>
          </View>
        </TouchableWithoutFeedback>
