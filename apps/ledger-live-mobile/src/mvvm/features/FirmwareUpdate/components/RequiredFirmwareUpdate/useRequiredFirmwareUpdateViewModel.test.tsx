...
/** No seenDevices entry matches the flow's device (Nano X), so the VM finds no
 * deviceInfo at all. */
const stateWithNoMatchingDevice = (state: State): State => ({
  ...state,
  settings: {
    ...state.settings,
    seenDevices: [{ modelId: DeviceModelId.stax, deviceInfo: otherDeviceInfo } as never],
  },
});
...
describe("useRequiredFirmwareUpdateViewModel", () => {
  ...
  describe("derived view state", () => {
    ...
    it("is not USB-capable when no seen device matches the flow's device model", () => {
      const nav = makeNavigation([ScreenName.FirmwareUpdate]);
      const { result } = renderVM(nav, stateWithNoMatchingDevice);
  ...
  describe("onPressCta", () => {
    ...
    it("passes an onBackFromUpdate that goes back on the resolved navigator", () => {
      const nav = makeNavigation([ScreenName.FirmwareUpdate]);
      const { result } = renderVM(nav, stateWithDeviceInfo);

      act(() => result.current.onPressCta());

      const { onBackFromUpdate } = nav.navigate.mock.calls[0][1];
      onBackFromUpdate();
