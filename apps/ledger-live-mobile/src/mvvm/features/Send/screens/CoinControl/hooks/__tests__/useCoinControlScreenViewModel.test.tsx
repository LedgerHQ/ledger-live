...
export const { getMainAccount, getAccountCurrency } = jest.requireMock(
...
export function defaultOverrideInitialState(state: State): State {
  return {
    ...state,
    settings: {
      ...state.settings,
...
export const mockOnAmountChange = jest.fn();
...
export const { useTranslatedBridgeError } = jest.requireMock(
  "../../../Recipient/hooks/useTranslatedBridgeError",
);
export const { getAccountBridge } = jest.requireMock("@ledgerhq/live-common/bridge/impl");
...
export const { Linking } = jest.requireMock("react-native");
...
describe("useCoinControlScreenViewModel", () => {
  ...
  it("should open coin control URL when onLearnMoreClick is called", () => {
    ...
    expect(Linking.openURL).toHaveBeenCalledWith(urls.coinControl);
  });
