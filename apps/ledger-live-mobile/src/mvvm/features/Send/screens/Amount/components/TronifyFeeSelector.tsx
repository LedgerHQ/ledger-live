...
export function TronifyFeeSelector({
  bottomSheetRef,
  viewModel,
  onClose,
}: TronifyFeeSelectorProps) {
  ...
  return (
    <BottomSheet ref={bottomSheetRef} enableDynamicSizing snapPoints={null}>
      <BottomSheetView style={styles.content}>
        <BottomSheetHeader title={t("send.newSendFlow.feeSelector.title")} density="compact" />
        ...
        <Pressable style={styles.option} onPress={handleSelectTronify}>
          <View style={styles.optionLeft}>
            ...
            <View style={styles.feesRow}>
              {viewModel.originalFeeFormatted ? (
                <Text typography="body3" lx={{ color: "muted" }} style={styles.strikethrough}>
                  {viewModel.originalFeeFormatted}
                </Text>
              ) : null}
            ...
            {viewModel.savingsFiatFormatted ? (
              <Text typography="body3" lx={{ color: "success" }} style={styles.savingsText}>
        ...
        {viewModel.insufficientBalance && viewModel.selected ? (
          <Text typography="body3" lx={{ color: "error" }} style={styles.insufficientWarning}>
