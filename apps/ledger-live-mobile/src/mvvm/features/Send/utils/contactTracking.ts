...
export type AddContactDrawerTrackingPage =
  | "add contact options"
  | "add contact"
  | "select existing contact"
  | "name address"
  | "address signing device";
...
export function getSendFlowTrackingPage(
  currentStep: SendFlowStep | undefined,
  isSelectingContactAddress = false,
  addContactDrawerPage?: AddContactDrawerTrackingPage,
): string {
  if (addContactDrawerPage) return addContactDrawerPage;
  if (isSelectingContactAddress) return "select contact address";
  if (!currentStep) return "step recipient";
