...
describe("useSwapTransactionStatusViewModel", () => {
  beforeEach(() => {
    ...
    mockedUseSelector.mockImplementation(selector => {
      if (selector === flattenAccountsSelector) return [sendAccount, receiveAccount];
  ...
  it("should use the parent account name and icon currency for token receive accounts", () => {
    ...
    mockedUseSelector.mockImplementation(selector => {
      if (selector === flattenAccountsSelector)
        return [sendAccount, receiveParentAccount, receiveTokenAccount];
      if (selector === localeSelector) return "en-US";
      return undefined;
    });
