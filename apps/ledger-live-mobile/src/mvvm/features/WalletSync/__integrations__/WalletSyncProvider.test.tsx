  ...
  return render(
    ...
    </WalletSyncProvider>,
    {
      overrideInitialState: (state: State) => ({
        ...state,
        featureFlags: { ...state.featureFlags, remoteFlagsReady },
      }),
    },
  );
}
...
describe("WalletSyncProvider", () => {
  ...
  it("should not watch wallet sync while the remote flags have not resolved", () => {
    ...
    // must not run while the flags still hold their compiled defaults. On mobile this is
    // reachable on any cold start slower than `WaitForAppReady`'s one-second bypass.
