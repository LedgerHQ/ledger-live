...
export function useAppProtectionPromptViewModel(): AppProtectionPromptViewModel {
  ...
  useEffect(() => {
    ...
    settle(false);
  }, [isAskingForPassword, isPasswordFlowMounted, isProtected, settle]);
  ...
  }, [isAskingForPassword, isProtected, settle]);
  ...
  const onContinue = useCallback(() => settle(true), [settle]);
  const onSuccessClose = useCallback(() => settle(false), [settle]);
