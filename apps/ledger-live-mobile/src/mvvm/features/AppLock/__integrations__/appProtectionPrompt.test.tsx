...
describe("asking from anywhere for the app to be protected", () => {
  ...
  // The lock takes the screen, and a sheet the queue is still holding behind another one closes
  // without telling its consumer, so the request has to be answered here or never.
  it("holds the caller's action when the app locks while it is waiting", async () => {
    const { store, user } = render(<Harness />, { overrideInitialState: unprotected });
