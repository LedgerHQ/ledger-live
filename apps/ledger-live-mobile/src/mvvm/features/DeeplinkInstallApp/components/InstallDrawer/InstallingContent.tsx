...
function InstallingContentView({
  showAllowManager,
  listingApps,
  progress,
  productName,
  ...
  return (
    ...
    >
      ...
      <Text
        ...
        lx={{ color: "base", textAlign: "center", marginTop: "s16" }}
      >
        {t("deeplinkInstallApp.installing.title", { appName: appDisplayName })}
      </Text>
    </Box>
  );
}
...
export function InstallingContent({ device, appConfig, onSuccess, onError }: Props) {
  const viewModel = useInstallingContentViewModel({ device, appConfig, onSuccess, onError });

  return (
    <InstallingContentView {...viewModel} device={device} appDisplayName={appConfig.displayName} />
  );
}
