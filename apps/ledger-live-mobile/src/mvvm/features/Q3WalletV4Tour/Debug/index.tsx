...
function Q3WalletV4TourScreenDebug() {
  ...
  const isQ3ATourEnabled = releaseTour?.enabled === true && releaseTour.params?.variant === "q3_a";
  const { isDrawerOpen, handleOpenDrawer, handleCloseDrawer, closeDrawer, onSlideChange } =
    useQ3WalletV4TourDrawerViewModel();
  const canOpenDrawer = isQ3ATourEnabled && !hasSeenQ3WalletV4Tour;
  ...
  let openDrawerLabel = "Open Drawer (enable Q3 A release tour)";
  if (canOpenDrawer) {
    openDrawerLabel = "Open Drawer";
  } else if (isQ3ATourEnabled) {
      ...
      <ScrollView contentContainerStyle={{ flexGrow: 1 }}>
        <Box lx={{ padding: "s16", rowGap: "s24" }}>
          <Text typography="body2" lx={{ color: "muted" }}>
            Test the Q3 Wallet V4 Tour drawer, gated by releaseTour variant q3_a.
          </Text>
