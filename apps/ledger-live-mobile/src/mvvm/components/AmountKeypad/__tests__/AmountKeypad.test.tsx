...
describe("AmountKeypad", () => {
  it("emits the digit that was pressed", () => {
    const { onKeyPress } = renderKeypad();

    fireEvent.press(screen.getByTestId("amount-key-7"));

    expect(onKeyPress).toHaveBeenCalledWith("7");
  });

  it("emits the decimal separator under its own test id", () => {
    const { onKeyPress } = renderKeypad();

    fireEvent.press(screen.getByTestId("amount-key-decimal"));

    expect(onKeyPress).toHaveBeenCalledWith(".");
  });

  it("emits the delete key so the caller can trim the amount", () => {
    const { onKeyPress } = renderKeypad();
  ...
  it("ignores presses while disabled", () => {
    const { onKeyPress } = renderKeypad({ disabled: true });

    fireEvent.press(screen.getByTestId("amount-key-1"));
