...
const EMPTY: DefaultAssets = { cryptos: [], stablecoins: [] };
  ...
  const defaults = useMemo<DefaultAssets>(() => {
    ...
    for (const id of assetsData.currenciesOrder.metaCurrencyIds) {
      ...
      if (stablecoinTickers.has(ticker)) {
        ...
        if (cryptos.length < maxCryptos) cryptos.push(item);
      }
    }

    return { cryptos, stablecoins };
  }, [assetsData, stablecoinTickers, maxCryptos, maxStablecoins]);
