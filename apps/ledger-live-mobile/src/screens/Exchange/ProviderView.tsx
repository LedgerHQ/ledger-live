...
export default function ProviderView({ route }: Props) {
  ...
  const manifest = useRemoteLiveAppManifest((provider as RampLiveAppCatalogEntry).appId);
  const { colors } = useTheme();
  const language = useSelector(languageSelector);
  const cryptoCurrency = provider.cryptoCurrencies.find(
    crypto => crypto.id === trade.cryptoCurrencyId,
  );
  const inputs = mapQueryParamsForProvider(provider as RampLiveAppCatalogEntry, {
    accountId,
    accountAddress,
    ...
    theme: colors.fog,
    language,
    fiatAmount: typeof trade.fiatAmount === "undefined" ? undefined : String(trade.fiatAmount),
    cryptoAmount:
      typeof trade.cryptoAmount === "undefined" ? undefined : String(trade.cryptoAmount),
  });
  return (
    <>
      <TrackScreen
      ...
      />
      {manifest ? (
        <WebPlatformPlayer manifest={manifest as LiveAppManifest} inputs={inputs} />
      ) : null}
    </>
  );
}
