...
export default function SendSelectRecipient({ route }: Props) {
  ...
  const CustomRecipientAlert =
    specific && "StepRecipientCustomAlert" in (specific as object)
      ? // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (specific as { StepRecipientCustomAlert?: React.ComponentType<any> }).StepRecipientCustomAlert
      : null;
  const customSendRecipientCanNext =
    specific && "sendRecipientCanNext" in (specific as object)
      ? (specific as { sendRecipientCanNext?: (...args: unknown[]) => boolean }).sendRecipientCanNext
      : null;
  const SendRecipientFields =
    specific && "SendRecipientFields" in (specific as object)
      ? // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (specific as { SendRecipientFields?: React.ComponentType<any> }).SendRecipientFields
      : null;
