...
let notificationPermissionRefreshPromise: ReturnType<
  typeof getNotificationPermissionStatus
> | null = null;

const getHasEnabledOsNotifications = async (store: AppStore): Promise<boolean> => {
  const permissionStatus = notificationsPermissionStatusSelector(store.getState());
  if (permissionStatus !== undefined) {
    return permissionStatus === AuthorizationStatus.AUTHORIZED;
  }

  try {
    // Share one native permission read while Redux is still missing the value during startup.
    if (!notificationPermissionRefreshPromise) {
      notificationPermissionRefreshPromise = getNotificationPermissionStatus()
        .then(status => {
          store.dispatch(setNotificationPermissionStatus(status));
          return status;
        })
        .finally(() => {
    ...
    const refreshedPermissionStatus = await notificationPermissionRefreshPromise;
    return refreshedPermissionStatus === AuthorizationStatus.AUTHORIZED;
  } catch {
    return false;
  }
};
...
const extraProperties = async (store: AppStore) => {
  ...
  const hasEnabledOsNotifications = await getHasEnabledOsNotifications(store);
