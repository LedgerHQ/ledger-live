...
export function usePostOnboardingActionPress(props: PostOnboardingActionRowProps) {
  ...
  const [computedIsActionCompleted, setComputedIsActionCompleted] = useState(false);
  const [computedIsCompletionLoading, setComputedIsCompletionLoading] = useState(true);

  useEffect(() => {
    if (completionStatus) return;

    let cancelled = false;
    setComputedIsCompletionLoading(true);
    isPostOnboardingHubActionFulfilled(
      ...
      .then(result => {
        if (cancelled) return;
        setComputedIsActionCompleted(result);
        setComputedIsCompletionLoading(false);
      })
      .catch(() => {
        if (cancelled) return;
        setComputedIsActionCompleted(false);
        setComputedIsCompletionLoading(false);
      });
    return () => {
      cancelled = true;
    };
  }, [completed, getIsAlreadyCompletedByState, getIsAlreadyCompleted, context, completionStatus]);

  const isActionCompleted = completionStatus?.isCompleted ?? computedIsActionCompleted;
  const isCompletionLoading = completionStatus?.isLoading ?? computedIsCompletionLoading;
