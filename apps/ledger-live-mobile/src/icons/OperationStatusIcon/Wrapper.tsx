  ...
  const inferColor = useCallback(
    (type: OperationType) => {
      switch (type) {
        case "IN":
        case "NFT_IN":
          return colors.success.c50;

        case "FREEZE":
          return colors.primary.c80;

        case "REWARD":
          return colors.warning.c80;

        default:
          return colors.neutral.c70;
      }
    },
    [colors],
  );
  const color = failed ? colors.error.c60 : inferColor(type);
  return (
      ...
      <G fill={color}>
        ...
        {!confirmed ? (
          <G>
            ...
            />
            {!failed ? (
              <G id="clock" fill={colors.neutral.c70} transform="translate(14.000000, 14.000000)">
                <Circle id="clockBg" fill={colors.background.drawer} cx="6" cy="6" r="6" />
