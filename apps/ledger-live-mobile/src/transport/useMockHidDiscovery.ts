...
import { buildUsbCompatDeviceId, HIDDiscoveryState } from "@ledgerhq/live-dmk-mobile";
 ...
 *
 * The `deviceId` format generated by `buildUsbCompatDeviceId(JSON.stringify(payload))`
 * must match what the e2e test expects:
 * - `common.page.ts` builds test ID from the same compat id
    ...
    const subscription = e2eBridgeClient
      ...
      .subscribe(msg => {
        if (msg.type === "addUSB") {
          ...
          const deviceId = buildUsbCompatDeviceId(JSON.stringify(descriptor));
          const device = {
            deviceId,
