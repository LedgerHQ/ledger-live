...
export async function serializeAccountsState(state: State): Promise<{
  active: {
    data: AccountRaw;
    version: number;
  }[];
  ...
  const active = await Promise.all(
    state.accounts.active.flatMap(account => {
      const accountUserData = accountUserDataExportSelector(state.wallet, { account });
      return accountUserData != null ? [accountModel.encode([account, accountUserData])] : [];
    }),
  );
  return { active };
}
