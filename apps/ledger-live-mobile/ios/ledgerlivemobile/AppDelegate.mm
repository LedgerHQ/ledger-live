...
- (BOOL)application:(UIApplication *)application
  ...
  // Retrieve the correct GoogleService-Info.plist file name for a given
  // environment
  NSString *googleServiceInfoEnvName =
      [RNCConfig envFor:@"GOOGLE_SERVICE_INFO_NAME"];
  ...
  NSString *filePath =
      [[NSBundle mainBundle] pathForResource:googleServiceInfoName
                                      ofType:@"plist"];
  ...
  NSString *brazeCustomEndpointFromEnv =
      [RNCConfig envFor:@"BRAZE_CUSTOM_ENDPOINT"];
  BRZConfiguration *configuration =
      [[BRZConfiguration alloc] initWithApiKey:brazeApiKeyFromEnv
                                      endpoint:brazeCustomEndpointFromEnv];
  ...
  if ([[NSUserDefaults standardUserDefaults]
          objectForKey:iOSPushAutoEnabledKey]) {
    pushAutoEnabled = [[NSUserDefaults standardUserDefaults]
        boolForKey:iOSPushAutoEnabledKey];
  }
  if (pushAutoEnabled) {
  ...
  [[BrazeReactUtils sharedInstance]
      populateInitialUrlFromLaunchOptions:launchOptions];

  BOOL appLaunched = [super application:application
          didFinishLaunchingWithOptions:launchOptions];
  // This condition is needed to prevent a crash since upgrading to React Native
  // 0.75.4 It may be fixed in a later version of React Native
  ...
  BOOL isRunningDetox =
      [[[NSProcessInfo processInfo] arguments] containsObject:@"-IS_TEST"];
...
- (void)registerForPushNotifications {
  ...
  // Authorization is requested later in the JavaScript layer via
...
- (void)userNotificationCenter:(UNUserNotificationCenter *)center
  ...
  [[BrazeReactUtils sharedInstance]
      populateInitialUrlForCategories:response.notification.request.content
                                          .userInfo];
  BOOL processedByBraze =
      AppDelegate.braze != nil &&
      [AppDelegate.braze.notifications
...
- (void)showOverlay {
  ...
  UIImageView *logoView = [[UIImageView alloc]
      initWithImage:[UIImage imageNamed:@"Blurry_nocache1"]];
  ...
  [logoView setContentHuggingPriority:251
                              forAxis:UILayoutConstraintAxisHorizontal];
  [logoView setContentHuggingPriority:251
                              forAxis:UILayoutConstraintAxisVertical];
  logoView.frame = CGRectMake(0, 0, 128, 128);

  logoView.center = CGPointMake(self.window.frame.size.width / 2,
