...
export const ElementHelper = {
  ...
  async getWebElementsText(id: string) {
    ...
    while (true) {
      try {
        texts.push(await ElementHelper.getWebElementByTestId(id).atIndex(i).getText());
        i++;
      } catch {
        break;
      }
  ...
  async tapWebElementByTestId(id: string, index = 0) {
    await ElementHelper.getWebElementByTestId(id).atIndex(index).tap();
  },

  async typeTextByWebTestId(id: string, text: string) {
    await ElementHelper.getWebElementByTestId(id).runScript(
      (el, text) => {
  ...
  async tapById(id: string | RegExp, index = 0) {
    return await ElementHelper.getElementById(id, index).tap();
  },

  async tapByText(text: string | RegExp, index = 0) {
    return await ElementHelper.getElementByText(text, index).tap();
  },
  ...
  async typeTextById(id: string | RegExp, text: string, closeKeyboard = true, focus = true) {
    await ElementHelper.typeTextByElement(
      ElementHelper.getElementById(id),
      text,
      closeKeyboard,
      focus,
    );
  ...
  async typeTextByElement(
    ...
    if (focus) await ElementHelper.tapByElement(elem);
  ...
  async scrollToText(
    ...
    await ElementHelper.performScroll(by.text(text), scrollViewId, pixels, direction);
  },
  ...
  async scrollToId(
    ...
    await ElementHelper.performScroll(by.id(id), scrollViewId, pixels, direction);
  },

  async getTextOfElement(id: string | RegExp, index = 0) {
    const attributes = await ElementHelper.getElementById(id, index).getAttributes();
