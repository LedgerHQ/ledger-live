...
import { AccountType } from "@ledgerhq/live-common/e2e/enum/AccountType";
...
test.describe.only("Send token - E2E", () => {
  ...
  test(
    `Send from ${transaction.accountToDebit.accountName} to ${transaction.accountToCredit.accountName} - ${transaction.accountToDebit.currency.name} - E2E test`,
    {
      annotation: {
        type: "TMS",
        description: "B2CQA-3055, B2CQA-3057",
      },
    },
    async ({ app }) => {
      await addTmsLink(getDescription(test.info().annotations, "TMS").split(", "));
      ...
      if (!getEnv("DISABLE_TRANSACTION_BROADCAST")) {
        ...
        await app.account.navigateToTokenInAccount(transaction.accountToDebit);
        //checker ici le graph qui monte quand j'envoie des fonds
...
const transactionsAddressInvalid = [
  ...
  {
    transaction: new Transaction(Account.SOL_GIGA_1, Account.SOL_WIF_1, "0.1", undefined),
    expectedErrorMessage: "This associated token account holds another token",
    xrayTicket: "B2CQA-todo",
  },
  {
    transaction: new Transaction(Account.SOL_1, Account.SOL_GIGA_2, "0.1", undefined), //Todo: Verifier si je laisse bien ici car c'est le parent qui envoie
...
for (const transaction of transactionsAddressInvalid) {
  test.describe("Send token - invalid address input", () => {
    ...
    test(
      `Send from ${transaction.transaction.accountToDebit.accountName} to ${transaction.transaction.accountToCredit.accountName} - ${transaction.transaction.accountToDebit.currency.name} - invalid address input`,
      {
        annotation: {
          type: "TMS",
      ...
      async ({ app }) => {
        ...
        await app.send.selectCurrency(transaction.transaction);

        if (
          transaction.transaction.accountToCredit.accountType === AccountType.SPL &&
          transaction.transaction.accountToCredit.ataAddress
        ) {
          await app.send.fillRecipient(transaction.transaction.accountToCredit.ataAddress);
        } else {
