...
const tooLowAmountForQuoteSwaps = [
  ...
  {
    swap: new Swap(Account.ETH_USDT_2, Account.BTC_NATIVE_SEGWIT_1, "5", Fee.MEDIUM),
    xrayTicket: "todo",
  ...
  },
  {
    ...
    xrayTicket: "todo",
    errorMessage: "Not enough balance, including network fees. ",
    ctaBanner: true,
    quotesVisible: false,
  },
];
...
for (const {
  ...
  test.describe.only("Swap - with too low amount (throwing UI errors)", () => {
    ...
    test(
      `Swap too low quote amounts from ${swap.accountToDebit.currency.name} to ${swap.accountToCredit.currency.name} - ${errorMessage}`,
      {
        annotation: {
          type: "TMS",
          description: xrayTicket,
      ...
      async ({ app, electronApp }) => {
        ...
        await app.swap.verifySwapAmountErrorMessageIsCorrect(electronApp, errorMessage);
        if (ctaBanner) {
