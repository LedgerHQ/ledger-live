...
test("Cosmos staking flows via portfolio, asset page and market page", async ({ page }) => {
  ...
  await test.step(
    ...
    async () => {
      ...
      await expect.soft(modal.container).toHaveScreenshot("modal-staking-cosmos-account.png");
      await modal.close();
      await expect.soft(page).toHaveScreenshot("non-cosmos-account-page.png");
      await accountPage.clickBannerCTA();
      await expect.soft(modal.container).toHaveScreenshot("modal-staking-cosmos-account-2.png");
      await modal.close();
    },
  );
