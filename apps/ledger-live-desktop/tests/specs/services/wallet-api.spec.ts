...
// Per-test page objects. Splitting the former mega-test into independent tests
// means each test gets a fresh Electron app + webview, so cross-step state
// contamination (and the manual resetWebview dance) is no longer needed between
// tests. We still reset between sub-flows that share a single test.
type WalletApiFixtures = {
  discoverPage: DiscoverPage;
  liveAppWebview: LiveAppWebview;
  drawer: Drawer;
  modal: Modal;
  layout: Layout;
...
const test = base.extend<WalletApiFixtures>({
  discoverPage: async ({ page }, use) => {
    await use(new DiscoverPage(page));
  },
  liveAppWebview: async ({ page, electronApp }, use) => {
    await use(new LiveAppWebview(page, electronApp));
  },
  ...
  deviceAction: async ({ page }, use) => {
    await use(new DeviceAction(page));
  },
});
...
test.beforeAll(async () => {
  ...
  // startDummyServer binds a random free port (port 0) and exposes it through
  // MOCK_REMOTE_LIVE_MANIFEST, so this is safe to run once per worker under
  // fullyParallel without port conflicts.
...
test.describe("Wallet API methods", () => {
  // Each test starts from a freshly launched app with the dummy live-app open.
  test.beforeEach(async ({ layout, discoverPage, drawer }) => {
    test.skip(!testServerIsRunning, "Dummy Wallet API live app server is not running");
  ...
  test("account operations @smoke", async ({ liveAppWebview, drawer, modal, deviceAction }) => {
  ...
  test("currency list @smoke", async ({ liveAppWebview }) => {
  ...
  test("storage @smoke", async ({ liveAppWebview }) => {
  ...
  test("ethereum signing @smoke", async ({ page, liveAppWebview, modal, deviceAction }) => {
    ...
    await test.step("transaction.signAndBroadcast", async () => {
      ...
      await expect(drawerContent.getByText("View in explorer")).toBeVisible();
      await expect(drawerContent.getByText("Confirmed")).toBeVisible();
  ...
  test("solana signing @smoke", async ({ page, liveAppWebview, modal, deviceAction }) => {
  ...
  test("xrp + sanctioned @smoke", async ({ page, liveAppWebview, modal, deviceAction }) => {
  ...
  test("wallet info + deeplink @smoke", async ({ liveAppWebview, discoverPage }) => {
