...
test("Updater", async ({ page }) => {
  ...
  await test.step("[idle] state should not be visible", async () => {
    ...
    await expect.soft(page).toHaveScreenshot("app-updater-idle.png", { mask: [page.locator('canvas')] });
  });

  await test.step("[checking] state should be visible", async () => {
    await appUpdater.setStatus("checking");
    await expect.soft(page).toHaveScreenshot("app-updater-layout.png", { mask: [page.locator('canvas')] });
  ...
  await test.step("[error] state should be visible", async () => {
    ...
    await expect.soft(page).toHaveScreenshot("app-updater-error-with-carousel.png", { mask: [page.locator('canvas')] });
  });
  ...
  await test.step("[error] state (any) should be visible, without the carousel", async () => {
    ...
    await expect.soft(page).toHaveScreenshot("app-updater-error-without-carousel.png", { mask: [page.locator('canvas')] });
  });
});
