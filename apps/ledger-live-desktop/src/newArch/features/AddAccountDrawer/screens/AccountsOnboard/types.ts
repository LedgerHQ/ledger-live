...
export interface OnboardingConfig extends BaseOnboardingConfig {
  ...
  /**
   * Family-specific function to determine if there are creatable accounts.
   * If not provided, falls back to checking if any account has `used: false`.
   */
  hasCreatableAccounts?: (accounts: Account[]) => boolean;
}
