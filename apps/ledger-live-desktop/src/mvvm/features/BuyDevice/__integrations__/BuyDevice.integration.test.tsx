...
describe("Buy Device Dialog", () => {
  it("displays dialog correctly", async () => {
    ...
    const button = await screen.getByRole("button", { name: "OPEN" });
    await user.click(button);

    expect(screen.getByText("Connect a Ledger device")).toBeVisible();

    const connectButton = await screen.getByRole("button", { name: "Connect" });
    expect(connectButton).toBeVisible();

    const buyDeviceButton = await screen.getByRole("button", { name: "Buy your Ledger device" });
