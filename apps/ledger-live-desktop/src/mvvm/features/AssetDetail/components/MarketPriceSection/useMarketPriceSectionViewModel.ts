...
export function useMarketPriceSectionViewModel({
  ...
  const marketId = distributionItem?.marketId
    ...
    : undefined;
  const currencyDataId = marketId ?? distributionItem?.currency.id ?? marketAssetId;
  ...
  if (hasVariationData) {
    ...
    variationText = `${variationSign}${counterValueFormatter({
      value: Math.abs(variationValue),
  ...
  return {
    ...
    dayLabel: t("market.range.day", "1 day"),
