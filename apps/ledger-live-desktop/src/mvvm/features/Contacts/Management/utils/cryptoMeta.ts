 ...
 * - Composite key per entry: `${chainId}:${normalizedAddressLower}`.
...
const entryKey = (addressHex: string, chainId: number) =>
...
export const getCryptoMeta = (
  ...
  addressHex: string,
  chainId: number,
): string | undefined => meta[entryKey(addressHex, chainId)];

/**
 * Persist a crypto-id annotation for the given address+chain pair.
...
export const setCryptoMeta = (
  ...
  const key = entryKey(addressHex, chainId);
  const next = { ...snapshot };
