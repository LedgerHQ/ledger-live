 ...
 * defaults say, so the watcher only mounts once `remoteFlagsReady` is set.
 *
 ...
 * user action. Every other consumer gets there through its own hook and is not covered; on this app
 * they all sit behind the Wallet Sync router. Do not read that as "safe because they come later":
 * what keeps them safe is that the values are already right, the cache prime landing before the
 * first render and the compiled `environment` defaulting to PROD. Closing the gap properly means
 * gating the SDK factory itself rather than each caller, which is tracked separately.
 */
