...
/**
 * `showDisplayPOC` feature flag reader.
 *
 * FF shape: `{ enabled: true, params: { families: ["bitcoin", "evm"] } }`.
 * Mirrors `useNewSendFlowFeature` so attendees can copy the pattern 1:1.
 */
export function useDisplayPOCFeature() {
  const feature = useFeature("showDisplayPOC");
