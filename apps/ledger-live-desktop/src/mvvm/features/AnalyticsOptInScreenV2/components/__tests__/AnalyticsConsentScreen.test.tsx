...
describe("AnalyticsConsentScreen", () => {
  ...
  it("should use the theme-specific illustration", () => {
    const { container: darkContainer } = render(
      <AnalyticsConsentScreen {...baseProps} theme="dark" />,
    );
    expect(darkContainer.querySelector("img")?.getAttribute("src")).toContain(
      "analyticsConsentIllustrationDark",
    );

    const { container: lightContainer } = render(
      <AnalyticsConsentScreen {...baseProps} theme="light" />,
    );
    expect(lightContainer.querySelector("img")?.getAttribute("src")).toContain(
      "analyticsConsentIllustrationLight",
    );
  });
