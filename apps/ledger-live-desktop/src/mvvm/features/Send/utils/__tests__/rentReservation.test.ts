...
describe("buildRentReservationOperation", () => {
  it("locks the reserved native amount as a pending FEES op on the main account", () => {
    const op = buildRentReservationOperation({
      mainAccountId: MAIN_ID,
      payerAddress: PAYER,
      paymentTxId: "txA-hash-1",
      reservedNativeAmount: 12_500_000n,
    });
  ...
  it("derives a distinct, non-zero, deterministic sequence per payment tx id", () => {
    const reserve = (paymentTxId: string) =>
      buildRentReservationOperation({
        ...
        payerAddress: PAYER,
        paymentTxId,
        reservedNativeAmount: 1_000_000n,
      });
  ...
  it("returns null for a negative reserved amount", () => {
    expect(
      buildRentReservationOperation({
        mainAccountId: MAIN_ID,
        payerAddress: PAYER,
        paymentTxId: "txA-hash-1",
        reservedNativeAmount: -1n,
      }),
    ).toBeNull();
  });
});
