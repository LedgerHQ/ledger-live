...
describe("Send Flow Integration", () => {
  ...
  describe("Sponsored send (TRON Tronify)", () => {
    ...
    // Known constant used to build the TX-A device signature the way coin-tron's combine() shapes it:
    // a 4-hex-digit length prefix, then the echoed raw_data_hex, then the signature.
    ...
    it("runs the sponsored happy path: rent signature -> polling -> transfer signature -> confirmation", async () => {
      ...
      await act(async () => {
        setMockOrchestrationState({ order: rentOrder, toSign: rawDataHex });
      });

      // The raw combined device signature reaches the rent-payment action verbatim — the family seam
      // (inside the now-mocked orchestration) is what recovers the signature and rebuilds the wire
      // payload, so this test asserts only the hand-off, not the seam's submitEnergyRentPayment call.
