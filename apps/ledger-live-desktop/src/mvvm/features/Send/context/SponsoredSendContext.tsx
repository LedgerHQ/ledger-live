...
export function SponsoredSendProvider({ children }: Readonly<{ children: ReactNode }>) {
  ...
  // Reserve the native rent the moment TX-A is (possibly) broadcast. The orchestration fires this from
  // inside its in-flight submit action, which keeps running past a reset/unmount, so the reservation
  // survives the dialog closing mid-submit. Keyed by paymentTxId (deduped through reservedTxIdsRef)
  // so a re-render or the fire on both submit-success and reconcile paths never double-locks, while
  // a retry under a fresh id gets its own reservation.
  ...
  const handleRentPaymentBroadcast = useCallback(
    ({
      ...
      if (!mainAccount || !paymentTxId || reservedTxIdsRef.current.has(paymentTxId)) return;
  ...
  // `sponsored` lives on the generic `GenericTransaction` but not on the flow's family `Transaction`
  // union, so the update is cast back to the input shape.
