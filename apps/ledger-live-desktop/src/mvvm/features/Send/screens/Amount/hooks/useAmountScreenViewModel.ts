...
export function useAmountScreenViewModel({
  ...
  const { selectedFeeOptionId, available, quote, intentReady, feeLoading, savingsFiatFormatted } =
    useSponsoredSend();
  ...
  // The sponsored-fee nudge is presentational; the Amount ViewModel owns its data + open handler.
    ...
    () => ({
      ...
      entryLabel: t(
        selectedFeeOptionId === "tronify"
          ? "newSendFlow.feePayment.tronify"
          : "newSendFlow.feePayment.regular",
      ),
      savingsFiatFormatted,
      onOpen: onOpenFeePayment,
    }),
    [available, selectedFeeOptionId, savingsFiatFormatted, onOpenFeePayment, t],
  );
