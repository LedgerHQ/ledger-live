...
export function useAddNewContactViewModel(): AddNewContactViewModel {
  ...
  }, [handleAddressPhaseBack, isAddressPhase, setHeaderState]);
  ...
  const handleSaveSuccess = useCallback(
    async (createdContact: Contact) => {
      ...
      try {
        ...
        // "cancelled" means the user already navigated away, so respect that choice.
  ...
  const contactAdapter = useAddContactDialogAdapter(handleSaveSuccess);
