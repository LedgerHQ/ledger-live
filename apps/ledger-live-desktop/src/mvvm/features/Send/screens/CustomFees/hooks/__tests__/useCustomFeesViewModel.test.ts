...
describe("useCustomFeesViewModel - EIP-1559 Validation", () => {
  it("should not expose gasLimit as a custom fee input", () => {
    ...
    expect(result.current.inputs.map(input => input.key)).toEqual([
  ...
  it("should not flag insufficient balance in max mode when fees fit the balance", () => {
    ...
    const transaction = {
      ...createMockTransaction(),
      useAllAmount: true,
      amount: new BigNumber("990000000000000000"),
    } as Transaction;
    const status = createMockStatus();
    const currency = createMockCurrency();
    const transactionActions = createMockTransactionActions();
    ...
    act(() => {
      result.current.onInputChange("maxFeePerGas", "1000");
    });
  ...
  it("should flag insufficient balance in max mode when fees exceed the balance", () => {
    const account = createMockAccount();
    const transaction = {
      ...createMockTransaction(),
      useAllAmount: true,
    ...
    act(() => {
      result.current.onInputChange("maxFeePerGas", "50000");
    });
  ...
  it("should reset amount to zero on confirm when max mode is active", () => {
    const account = createMockAccount();
    const transaction = {
      ...createMockTransaction(),
      useAllAmount: true,
      amount: new BigNumber("123000000000000000"),
    } as Transaction;
    const status = createMockStatus();
    const currency = createMockCurrency();
    const transactionActions = createMockTransactionActions();
    ...
    expect(transactionActions.updateTransaction).toHaveBeenCalledTimes(1);
    const updater = (transactionActions.updateTransaction as jest.Mock).mock.calls[0][0];
    const nextTx = updater(transaction);

    expect(nextTx.amount.toString()).toBe("0");
    expect(nextTx.useAllAmount).toBe(true);
