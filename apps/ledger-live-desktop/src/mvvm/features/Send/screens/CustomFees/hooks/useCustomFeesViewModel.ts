...
export function useCustomFeesViewModel({
  ...
  const estimatedFeesFromInputs = useMemo(() => {
    ...
    return null;
  }, [customFeeConfig, values, allInputsValid]);
  ...
  if (
    ...
    queueMicrotask(async () => {
      try {
        ...
        // On bridge error, keep the last successful estimate and flag insufficient balance
        // since high fee values that cause bridge failures typically exceed the balance
  ...
  const fiatValue = useMemo(() => {
    // When bridge estimation is needed but hasn't produced a meaningful result yet
