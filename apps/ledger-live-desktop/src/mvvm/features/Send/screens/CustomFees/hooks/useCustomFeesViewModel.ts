...
export function useCustomFeesViewModel({
  ...
  const estimatedFeesFromInputs = useMemo(() => {
    const allValid = activeInputs.every(input =>
      isValidNumberForInput(input.key, values[input.key] ?? ""),
    );
    if (!customFeeConfig || !allValid) return null;
  ...
  // allInputsValid is also needed by validation — compute it once here so both
  // useBridgeFeeEstimation and useCustomFeeValidation receive the same value.
  ...
  const fiatValue = useMemo(() => {
    if (bridgeEstimationKey && estimatedFees.lte(0)) return null;
    return formatCurrencyUnit(fiatUnit, estimatedFeesFiat, {
      showCode: true,
      disableRounding: true,
      locale,
    });
  }, [bridgeEstimationKey, estimatedFees, estimatedFeesFiat, fiatUnit, locale]);
  ...
  const handleConfirm = useCallback(() => {
    ...
    const patch = customFeeConfig.buildTransactionPatch(values);
    transactionActions.updateTransaction(tx => ({ ...tx, ...patch }));
    onConfirm();
