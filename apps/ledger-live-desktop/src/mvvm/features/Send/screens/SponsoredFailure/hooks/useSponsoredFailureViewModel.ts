 ...
 * craft, CONTRACT_DATA -> the phase it failed on, TRANSFER -> TRANSFER), and the shared
 * useSponsoredPhaseNavigator routes to the step for the resulting phase.
 */
export function useSponsoredFailureViewModel(): SponsoredFailureViewModel {
  ...
  // re-crafts a fresh order (the reducer's DELIVERY_FAILED -> RENT_SIGNING arm). RENT_PAYMENT
  // never moved funds, and CONTRACT_DATA/TRANSFER resume on the rental already bought.
