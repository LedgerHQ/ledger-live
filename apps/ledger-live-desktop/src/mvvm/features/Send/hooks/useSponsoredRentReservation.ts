 ...
 * syncs in by hash.
 *
 * Fires once per payment, keyed by `paymentTxId`. The orchestration sets that id whenever the rent
 * payment has (possibly) been broadcast — POLLING on the happy path, but also a submit rejection it
 * reconciles straight to DELIVERY_FAILED (funds may have moved) or, once delivered, TRANSFER, none of
 * which enter POLLING. The definitively-unpaid submit failure (RENT_PAYMENT) leaves the id null, so
 * funds that never moved are not locked. A retry re-crafts and pays under a fresh id, getting its own
 * reservation; a re-render at the same id does not. Mounted once at the send layout beside
 * useSponsoredPhaseNavigator; inert off the sponsored path (the id stays null).
 */
export function useSponsoredRentReservation(): void {
  ...
  const { order, payerAddress, paymentTxId } = state;
  ...
  }, [account, parentAccount, order, payerAddress, paymentTxId, reduxDispatch]);
}
