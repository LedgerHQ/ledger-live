...
import type { IconStackViewModelParams, LayoutProps, LayoutStyles } from "../types";

const DEFAULT_BORDER_COLOR = cssVar("var(--color-background-canvas)");
...
function resolveLayoutStyles({
  ...
  LayoutProps,
  "size" | "overlap" | "borderWidth" | "borderColor" | "borderRadius"
>): LayoutStyles {
...
export function useIconStackViewModel<T>({
  ...
  return {
    layoutProps,
    layoutStyles: resolveLayoutStyles(layoutProps),
    visibleItems,
