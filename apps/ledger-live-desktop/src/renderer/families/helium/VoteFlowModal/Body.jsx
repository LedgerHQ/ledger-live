...
const steps: Array<St> = [
  {
    id: "vote",
    label: <Trans i18nKey="helium.votes.flow.steps.vote.title" />,
    component: StepVote,
    noScroll: true,
...
const Body = ({
  ...
  const votes = useVotes().filter(vote => vote.blocksRemaining > 0);
  ...
  const {
    ...
    const transaction = bridge.updateTransaction(bridge.createTransaction(account), {
      ...
      model: {
        mode: "burn",
        payee: votes.length > 0 ? votes[0].outcomes[0].address : "",
        memo: "0",
        hipID: votes.length > 0 ? votes[0].id : "",
      },
    });
  ...
  const stepperProps = {
    title: t("helium.votes.flow.title"),
    device,
    account,
    parentAccount,
    transaction,
