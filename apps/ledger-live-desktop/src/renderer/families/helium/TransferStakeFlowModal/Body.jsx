...
const steps: Array<St> = [
  {
    id: "validator",
    label: <Trans i18nKey="helium.delegation.transfer.flow.steps.validator.title" />,
  ...
  {
    id: "amount",
    label: <Trans i18nKey="helium.delegation.transfer.flow.steps.amount.title" />,
...
const Body = ({
  ...
  const {
    ...
    const { account, validator } = params;

    invariant(account && account.heliumResources, "helium: account and helium resources required");

    invariant(
      validator.stakeStatus === "staked",
      "helium: can transfer stake only if there is something to transfer",
    );
  ...
  const stepperProps = {
    title: t("helium.delegation.transfer.flow.title"),
    device,
    account,
    parentAccount,
    transaction,
