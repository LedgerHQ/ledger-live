...
describe("StepRecordPicker", () => {
  ...
  it("should render the translated fee-record error from status", () => {
    ...
    expect(screen.getByTestId("aleo-record-picker-error")).toBeInTheDocument();
    expect(
      screen.getByText("Another private record is required to cover the network fee."),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        /Top up your balance if needed\. One private record is used for the transfer amount, and you must have another record that can cover the network fee/i,
      ),
    ).toBeInTheDocument();
  });
