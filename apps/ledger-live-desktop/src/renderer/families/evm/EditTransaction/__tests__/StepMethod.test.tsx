...
const createProps = (overrides: Partial<StepProps> = {}): StepProps =>
  ({
    ...
    setEditType: jest.fn(),
    t: (key: string) => key,
    transactionToUpdate: { amount: new BigNumber(1), recipient: "0xabc" },
    ...
    bridgePending: false,
    device: null,
    transaction: { amount: new BigNumber(1), recipient: "0xabc" },
    error: null,
...
describe("EVM EditTransaction StepMethod", () => {
  ...
  it("opens EVM learn more link", async () => {
    ...
    expect(openURL).toHaveBeenCalledWith(urls.editEvmTx.learnMore);
  });

  it("calls EVM patch flow and transitions on continue with cancel", async () => {
  ...
  it("calls EVM patch flow and transitions on continue with speedup", async () => {
    const bridgeUpdateTransaction = jest.fn();
    const tx = createProps().transactionToUpdate;
    const updateTransaction: StepProps["updateTransaction"] = updater => {
      updater(tx);
