...
const ProviderItem = ({ provider, stakeOnClick, redirectIfOnlyProvider }: Props) => {
  ...
  const hasTag = !!provider?.min && i18n.exists(`ethereum.stake_deprecated.${provider.id}.tag`);
      ...
      <Flex flexDirection={"column"} ml={5} flex={"auto"} alignItems="flex-start">
        <Flex alignItems="center">
          <Text variant="bodyLineHeight" fontSize={14} fontWeight="semiBold" mr={2}>
            {t(`ethereum.stake_deprecated.${provider.id}.title`)}
          </Text>
          {hasTag && (
            <Tag
              size="small"
            ...
            >
              {t(`ethereum.stake_deprecated.${provider.id}.tag`)}
            </Tag>
          )}
        </Flex>

        <Text variant="paragraph" fontSize={13} color="neutral.c70">
          {t(`ethereum.stake_deprecated.${provider.id}.description`)}
        </Text>
      </Flex>
