...
class OnboardModal extends PureComponent<Props, State> {
  ...
  handleStartOnboarding = () => {
    ...
    if (!device) {
      this.setState({
        error: new Error("No device connected. Please connect your Ledger device and try again."),
        isProcessing: false,
      });
      return;
    }
    ...
    if (!currency) {
      this.setState({
        error: new Error("Currency not specified. Please select a valid currency and try again."),
        isProcessing: false,
      });
      return;
    }

    // Validate Canton bridge support
    const cantonBridge = getCurrencyBridge(currency) as CantonCurrencyBridge;
    if (!cantonBridge?.onboardAccount) {
      this.setState({
        error: new Error(
          "Canton onboarding is not supported for this currency. Please check your currency selection.",
        ),
        isProcessing: false,
      });
      return;
