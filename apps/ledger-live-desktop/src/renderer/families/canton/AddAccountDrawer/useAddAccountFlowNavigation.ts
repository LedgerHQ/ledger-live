...
import { CANTON_MODULAR_DRAWER_ADD_ACCOUNT_STEP } from "./domain";
...
export const useAddAccountFlowNavigation = ({
  ...
  const navigateToWarningScreen = useCallback(
    (reason?: WarningReason, account?: Account) => {
      ...
      goToStep(CANTON_MODULAR_DRAWER_ADD_ACCOUNT_STEP.ACCOUNTS_WARNING);
    },
  ...
  const navigateToFundAccount = useCallback(
    (account: Account) => {
      ...
      if (onAccountSelected) {
        ...
        goToStep(CANTON_MODULAR_DRAWER_ADD_ACCOUNT_STEP.FUND_ACCOUNT);
      }
    },
  ...
  const navigateToEditAccountName = useCallback(
    (account: Account) => {
      ...
      if (onAccountSelected) {
        ...
        goToStep(CANTON_MODULAR_DRAWER_ADD_ACCOUNT_STEP.EDIT_ACCOUNT_NAME);
      }
    },
    [onAccountSelected, originalCurrency, goToStep],
  );
  ...
  const navigateToSelectAccount = useCallback(() => {
    goToStep(CANTON_MODULAR_DRAWER_ADD_ACCOUNT_STEP.SELECT_ACCOUNT);
  }, [goToStep]);

  const navigateToScanAccounts = useCallback(() => {
    goToStep(CANTON_MODULAR_DRAWER_ADD_ACCOUNT_STEP.SCAN_ACCOUNTS);
  }, [goToStep]);

  const navigateToAccountsAdded = useCallback(() => {
    goToStep(CANTON_MODULAR_DRAWER_ADD_ACCOUNT_STEP.ACCOUNTS_ADDED);
  }, [goToStep]);

  const navigateToConnectDevice = useCallback(() => {
    goToStep(CANTON_MODULAR_DRAWER_ADD_ACCOUNT_STEP.CONNECT_YOUR_DEVICE);
  }, [goToStep]);

  const navigateToCantonOnboard = useCallback(() => {
    goToStep(CANTON_MODULAR_DRAWER_ADD_ACCOUNT_STEP.CANTON_ONBOARD);
  }, [goToStep]);
