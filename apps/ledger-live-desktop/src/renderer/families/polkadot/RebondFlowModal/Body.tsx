...
const steps: Array<St> = [
  {
    id: "amount",
    label: <Trans i18nKey="polkadot.rebond.steps.amount.title" />,
  ...
  {
    id: "connectDevice",
    label: <Trans i18nKey="polkadot.rebond.steps.connectDevice.title" />,
    component: GenericStepConnectDevice,
    onBack: ({ transitionTo }: StepProps) => transitionTo("amount"),
  },
  {
    id: "confirmation",
    label: <Trans i18nKey="polkadot.rebond.steps.confirmation.title" />,
...
const Body = ({ t, stepId, device, onClose, openModal, onChangeStepId, params }: Props) => {
  ...
  const { transaction, setTransaction, account, status, bridgeError, bridgePending } =
    useBridgeTransaction(() => {
      ...
      return {
        ...
        transactionPatch: { mode: "rebond" as const },
      };
    });
  ...
  return (
    <Stepper {...stepperProps}>
      <Track onUnmount event="CloseModalRebond" />
    </Stepper>
  );
};
