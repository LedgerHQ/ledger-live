...
jest.mock("@ledgerhq/live-common/families/stacks/react", () => ({
  __esModule: true,
  getStacksStakingPosition: (account: StacksAccount) => account.stakingPositions?.[0],
}));
...
const account = {
  ...genAccount("stacks-unstake-body", { currency }),
...
describe("UnstakeFlowModal/Body", () => {
  it("renders the two-step Stepper (skips the amount step entirely)", () => {
    renderBody();
    expect(screen.getByTestId("stepper-step-ids")).toHaveTextContent("connectDevice,confirmation");
    expect(screen.getByTestId("stepper-step-id")).toHaveTextContent("connectDevice");
  });

  it("initializes the transaction with mode: undelegate and valAddress from stakingPositions[0].delegate", () => {
    renderBody();
  ...
  it("handleRetry resets the flow back to the connectDevice step", async () => {
