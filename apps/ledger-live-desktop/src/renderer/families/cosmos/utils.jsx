...
const getAccountBannerProps = (state, account, {t, dispatch}) => {
    ...
    const title = redelegate ? t("account.banner.redelegation.title") : t("account.banner.delegation.title");
    ...
    const cta = redelegate ? t("account.banner.redelegation.cta") : t("account.banner.delegation.cta");
    const onClick= () => {
        if(redelegate) {
            dispatch(
