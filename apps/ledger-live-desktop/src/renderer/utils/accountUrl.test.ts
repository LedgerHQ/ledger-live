...
describe("getAccountUrl", () => {
  it("should construct URL for main account without parent", () => {
    expect(getAccountUrl(ETH_ACCOUNT_ID)).toBe(
      `/account/${computeAccountAlias(parseAnyAccountId(ETH_ACCOUNT_ID))}`,
    );
  });
  ...
  it("should construct URL for token account with parent", () => {
    ...
    expect(getAccountUrl(tokenAccountId, ETH_ACCOUNT_ID)).toBe(
      `/account/${computeAccountAlias(parseAnyAccountId(ETH_ACCOUNT_ID))}/${computeAccountAlias(parseAnyAccountId(tokenAccountId))}`,
    );
  });
