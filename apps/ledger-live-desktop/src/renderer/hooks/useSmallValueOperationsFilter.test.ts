...
const defaultInitialStateWithThreshold = {
  settings: {
    ...
    overriddenFeatureFlags: {
      lldHideSmallValueTokenOperations: { enabled: true },
    },
  },
};
...
describe("useSmallValueOperationsFilter", () => {
  beforeEach(() => {
    ...
    // USD magnitude 2: return cents. 50 cents => 0.5 USD (below threshold 1)
  ...
  it("should still hide zero-value operation when threshold FF is disabled", () => {
    const tokenAccount = createTokenAccount();
  ...
  it("should not apply threshold filtering when lldHideSmallValueTokenOperations FF is disabled", () => {
    const tokenAccount = createTokenAccount();
    // non-zero but below threshold — should still be shown since FF is off
    const operation = createOperation({ type: "IN", value: new BigNumber(1_000_000) });
    mockCalculate.mockReturnValue(50); // 0.5 USD, below threshold of 1
  ...
  it("should hide operation when fiat value is below threshold", () => {
    const tokenAccount = createTokenAccount();
    const operation = createOperation({ type: "IN" });
    // 50 cents = 0.5 USD < threshold 1 USD
    mockCalculate.mockReturnValue(50);
  ...
  it("should show operation when fiat value is above or equal to threshold", () => {
    const tokenAccount = createTokenAccount();
    const operation = createOperation({ type: "IN" });
    // 100 cents = 1 USD >= threshold 1 USD
