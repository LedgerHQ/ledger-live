...
/**
 * The read half of the pay card round trip. `init.tsx` reads one `payCard` blob off disk and hands
 * the same object to four restore actions, one per flow that owns a field in it. Nothing else
