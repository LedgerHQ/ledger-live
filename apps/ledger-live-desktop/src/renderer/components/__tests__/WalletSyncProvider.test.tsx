...
describe("Desktop WalletSyncProvider", () => {
  ...
  describe("useDesktopWalletSyncBridge", () => {
    it("should create desktop wallet sync bridge with correct state", () => {
      ...
      expect(result.current.useWalletSyncState()).toEqual(mockWSState);
      expect(result.current.useTrustchain()).toEqual(mockTrustchain);
      expect(result.current.useMemberCredentials()).toEqual(mockMemberCredentials);
  ...
  describe("Error Handling", () => {
    it("should handle null trustchain gracefully", () => {
      ...
      const storeWithoutTrustchain = configureStore({
        reducer: {
          accounts: (state = stateWithoutTrustchain.accounts) => state,
          wallet: (state = stateWithoutTrustchain.wallet) => state,
          trustchain: (state = stateWithoutTrustchain.trustchain) => state,
          application: (state = stateWithoutTrustchain.application) => state,
          assetsDataApi: (state = stateWithoutTrustchain.assetsDataApi) => state,
          countervalues: (state = stateWithoutTrustchain.countervalues) => state,
          devices: (state = stateWithoutTrustchain.devices) => state,
          dynamicContent: (state = stateWithoutTrustchain.dynamicContent) => state,
          market: (state = stateWithoutTrustchain.market) => state,
          modals: (state = stateWithoutTrustchain.modals) => state,
          postOnboarding: (state = stateWithoutTrustchain.postOnboarding) => state,
          settings: (state = stateWithoutTrustchain.settings) => state,
          swap: (state = stateWithoutTrustchain.swap) => state,
          UI: (state = stateWithoutTrustchain.UI) => state,
          walletSync: (state = stateWithoutTrustchain.walletSync) => state,
        },
      ...
      expect(result.current.useTrustchain()).toBeNull();
      expect(result.current.useMemberCredentials()).toBeNull();
      expect(result.current.useWalletSyncState()).toEqual(mockWSState);
    });
