...
describe("utils", () => {
  ...
  describe("getFlowName", () => {
    ...
    it("map for receiveFlow and swapFlow", () => {
      expect(getFlowName(HOOKS_TRACKING_LOCATIONS.receiveModal, {})).toBe(FlowName.receive);
      expect(getFlowName(HOOKS_TRACKING_LOCATIONS.exchange, {})).toBe(FlowName.swap);
    });

    it("sendFlow + transaction.mode === 'send' -> 'send'", () => {
      const req = { transaction: { mode: "send" } };
      expect(getFlowName(HOOKS_TRACKING_LOCATIONS.sendModal, req)).toBe(FlowName.send);
    });
    ...
    it("sendFlow + transaction.mode !== 'send' -> 'staking'", () => {
      ...
      const req3 = {};
      expect(getFlowName(HOOKS_TRACKING_LOCATIONS.sendModal, req1)).toBe(FlowName.staking);
      expect(getFlowName(HOOKS_TRACKING_LOCATIONS.genericDAppTransactionSend, req2)).toBe(
        FlowName.staking,
      );
      expect(getFlowName(HOOKS_TRACKING_LOCATIONS.sendModal, req3)).toBe(FlowName.staking);
    });
