...
import {
  useFreshPortfolioBalance,
  useHasFreshBalances,
} from "~/renderer/hooks/useFreshPortfolio";
...
/**
 * PortfolioBalanceOptimistic - Shows optimistic balance next to normal portfolio balance
 *
 * Usage: Add next to your existing portfolio balance display
 * Example: <FormattedVal .../> <PortfolioBalanceOptimistic ... />
 */
export default function PortfolioBalanceFresh({
  ...
  const hasDifference =
    hasFresh &&
    !freshTotal.eq(
      typeof normalBalance === "number" ? normalBalance : normalBalance.toNumber(),
    );
