...
import ValidatorIcon from "~/renderer/components/Staking/shared/ValidatorIcon";
...
export function Row({
  ...
  const redelegateDisabledTooltip = useMemo(() => {
    ...
    if (formattedRedelegationDate) {
      return (
        <Trans
          i18nKey="staking.delegation.redelegateDisabledTooltip"
    ...
    return (
      <Trans i18nKey="staking.delegation.redelegateMaxDisabledTooltip">
        <b></b>
      </Trans>
    );
  ...
  const dropDownItems = useMemo(
    () => [
      {
        key: "MODAL_STAKE_REDELEGATE",
        label: <Trans i18nKey="staking.delegation.redelegate" />,
        disabled: !_canRedelegate,
        tooltip: redelegateDisabledTooltip,
      },
      {
        key: "MODAL_STAKE_UNDELEGATE",
        label: <Trans i18nKey="staking.delegation.undelegate" />,
        disabled: !_canUndelegate,
        tooltip: _canUndelegate ? null : (
          <Trans i18nKey="staking.delegation.undelegateDisabledTooltip">
            <b></b>
          </Trans>
        ),
      },
      ...(pendingRewards.gt(0)
        ? [
            {
              key: "MODAL_STAKE_CLAIM_REWARDS",
              label: <Trans i18nKey="staking.delegation.reward" />,
            },
          ]
        : []),
    ],
      ...
      <Column>
        {status === "bonded" ? (
          <Box color="positiveGreen" pl={2}>
            <ToolTip content={<Trans i18nKey="staking.delegation.activeTooltip" />}>
              <CheckCircle size={14} />
            </ToolTip>
          </Box>
        ) : (
          <Box color="alertRed" pl={2}>
            <ToolTip content={<Trans i18nKey="staking.delegation.inactiveTooltip" />}>
      ...
      <Column>
        <DropDown
          ...
          renderItem={({ item, isActive }) => {
            if (item.key === "MODAL_STAKE_CLAIM_REWARDS")
              return (
                <>
                  <Divider />
...
export function UnbondingRow({
  ...
  return (
    <Wrapper>
      ...
      <Column>
        <Box color="alertRed" pl={2}>
          <ToolTip content={<Trans i18nKey="staking.undelegation.inactiveTooltip" />}>
            <ExclamationCircleThin size={14} />
          </ToolTip>
        </Box>
      </Column>
