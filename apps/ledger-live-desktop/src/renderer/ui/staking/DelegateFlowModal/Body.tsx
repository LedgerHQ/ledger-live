...
export type Data = {
  ...
  mode?: "delegate" | "undelegate" | undefined;
  eventType?: string;
  stepId?: StepId;
  source?: string;
};
...
const Body = async ({ stepId, params, onChangeStepId, onClose }: Props) => {
  const { t } = useTranslation();
  const dispatch = useDispatch();
  const device = useSelector(getCurrentDevice);
  const openedFromAccount = !!params.account;
  const alpacaApi = getAlpacaApi((params.account as Account).currency.id, "local");
  const validators = await alpacaApi.getValidators().then(v => v.items.map(i => i.address));
  const [defaultBaker] = validators;
  ...
  useEffect(() => {
    ...
    if (
      ...
      patch.recipient = defaultBaker;
    }

    // when changes, we set again
    if (patch.mode !== params.mode || patch.recipient) {
      setTransaction(
