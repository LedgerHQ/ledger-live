...
const StepFinish = ({ currency }: StepProps) => {
  ...
  return (
    <Box alignItems="center" py={6}>
      ...
      <Title id="add-account-success-title">{t("addAccounts.success", { count: 1 })}</Title>
      <Text>{t("addAccounts.successDescription", { count: 1 })}</Text>
    </Box>
  );
};
...
export default memo(StepFinish);
