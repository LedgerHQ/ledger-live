import {
  ...
  type ContentAbTestCopyOverrides,
...
export function installContentAbTestCopyOverrides(
  ...
  let copyOverrides: ContentAbTestCopyOverrides = getContentAbTestCopyOverrides();
  ...
  const applyCopyOverrides = (language: string) => {
    ...
    for (const [key, value] of Object.entries(copyOverrides)) {
      ...
      i18nInstance.addResource(ENGLISH_LANGUAGE, namespace, key, value, { silent: true });
      appliedKeys.add(key);
    }
  };
  ...
  const unsubscribe = subscribeToContentAbTestCopyOverrides(nextCopyOverrides => {
