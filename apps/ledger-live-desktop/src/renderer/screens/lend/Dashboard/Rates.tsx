...
import { CurrentRate } from "@ledgerhq/live-common/families/ethereum/modules/compound";
...
const Row = ({ data, accounts }: { data: CurrentRate; accounts: AccountLikeArray }) => {
  ...
  const openManageModal = useCallback(() => {
    ...
    if (!account && ethAccount) {
      dispatch(
        openModal("MODAL_LEND_EMPTY_ACCOUNT_DEPOSIT", {
          currency: token,
          account: ethAccount,
        }),
      );
      ...
      dispatch(
        openModal("MODAL_LEND_NO_ETHEREUM_ACCOUNT", {
          currency: token,
        }),
      );
