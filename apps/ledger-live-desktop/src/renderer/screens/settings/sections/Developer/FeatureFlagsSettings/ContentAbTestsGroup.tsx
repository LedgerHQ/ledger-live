...
const ContentAbTestsGroup = ({
  ...
  const allEnabled = testNames.every(testName => contentAbTests[testName]?.enabled);
  ...
  return (
    <>
      <Flex flexDirection="row" alignItems="center" justifyContent="space-between">
        <Row
          flex={1}
          onClick={() => setFocusedGroupName(focused ? undefined : CONTENT_AB_TESTS_PROJECT)}
        >
          <Flex flex={1} mr={3} alignItems="center">
            <Box
              ...
              borderRadius={999}
            />
            <Text mr={1}>{CONTENT_AB_TESTS_PROJECT}</Text>
            {someOverridden ? (
              <>
                <Tag active mx={1} type="opacity" size="small">
                  {t("settings.developer.overridden.overriddenLocally")}
                </Tag>
                <Link size="small" type="color" onClick={clearContentAbTestOverrides}>
