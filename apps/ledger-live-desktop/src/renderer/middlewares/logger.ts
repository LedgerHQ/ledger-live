...
const loggerMiddleware: Middleware<object, State> = () => next => action => {
  if (isActionWithType(action)) {
    ...
    // support ticket, and the only gate is NO_DEBUG_ACTION, so it runs in production. The Card
    // api's arguments and payloads are OAuth2 credentials. Strip them before the logger reads them.
