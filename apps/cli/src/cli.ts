...
async function main() {
  ...
  const setups = await Promise.all(families.map(f => loadSetupForFamily(f)));
  ...
  const commands = {
    ...setups
      ...
      .filter(Boolean)
      .flatMap(m =>
        typeof m === "object" && m !== null && "commands" in m
          ? Object.entries((m as { commands: Record<string, unknown> }).commands)
          : [],
      )
