...
const exec = async (opts: staxFetchAndRestoreJobOpts) => {
  ...
  await new Promise<void>(async (resolve) => {
    ...
    const currentHash = await withDevice(deviceId)((t) =>
      from(staxFetchImageHash(t))
    ).toPromise();
    if (currentHash === hash) {
      console.log(
        "Hashes match, skip backup step because we can use the one we have"
      );
    } else {
    ...
    const result = await withDevice(deviceId)((t) =>
      from(t.exchange(Buffer.from("e063000000", "hex")))
    ).toPromise();
...
export default {
  description:
    "Conditionally backup, delete, and restore a custom image on Stax",
  args: [
    deviceOpt,
    {
      name: "fileInput",
