...
// @ts-expect-error — deep subpath may not resolve in all tsconfigs
...
export class AlpacaAdapter {
  async getBalances(descriptor: AccountDescriptor): Promise<Balance[]> {
    ...
    const tokenBalances = await Promise.all(
      tokenAssets.map(async ({ asset, value }) => {
        const token = await api.getTokenFromAsset?.(asset);
  ...
  async getOperations(
    ...
    return { operations: coreOps.map(op => this.mapOperation(descriptor.id, op)), nextCursor };
  }

  private mapOperation(accountId: string, op: CoreOperation): Operation {
