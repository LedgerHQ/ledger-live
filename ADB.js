...
const INSTALL_TIMEOUT = 120000; // TODO Double check 45s makes sense
...
class ADB {
  ...
  async devices() {
    ...
    const devices = _.chain(stdout)
      ...
      .map((s) => _.trim(s))
      .map((s) =>
        s.startsWith("emulator-") ? new EmulatorHandle(s) : new DeviceHandle(s)
      )
      .value();
  ...
  async isPackageInstalled(deviceId, packageId) {
    ...
    const packageRegexp = new RegExp(
      `^package:${escape.inQuotedRegexp(packageId)}$`,
      "m"
    );
  ...
  async install(deviceId, _apkPath) {
    ...
    const command =
      apiLvl >= 23 ? `install -r -g -t ${apkPath}` : `install -rg ${apkPath}`;
  ...
  async remoteInstall(deviceId, path) {
    const apiLvl = await this.apiLevel(deviceId);
    const command =
      apiLvl >= 23 ? `pm install -r -g -t ${path}` : `pm install -rg ${path}`;
  ...
  async getFileSize(deviceId, filename) {
    const { stdout, stderr } = await this.adbCmd(
      deviceId,
      "shell du " + filename
    ).catch((e) => e);
  ...
  async apiLevel(deviceId) {
    ...
    const lvl = Number(
      await this.shell(deviceId, `getprop ro.build.version.sdk`, { retries: 5 })
    );
  ...
  screenrecord(deviceId, { path, size, bitRate, timeLimit, verbose }) {
    ...
    const _bitRate = bitRate > 0 ? ["--bit-rate", String(bitRate)] : [];

    const _timeLimit = timeLimit > 0 ? [`--time-limit`, timeLimit] : [];
  ...
  async getInstrumentationRunner(deviceId, bundleId) {
    ...
    const instrumentationRunner = this._instrumentationRunnerForBundleId(
      instrumentationRunners,
      bundleId
    );
    if (instrumentationRunner === "undefined") {
      throw new DetoxRuntimeError(
        `No instrumentation runner found on device ${deviceId} for package ${bundleId}`
      );
    }
  ...
  _instrumentationRunnerForBundleId(instrumentationRunners, bundleId) {
    const runnerForBundleRegEx = new RegExp(
      `^instrumentation:(.*) \\(target=${bundleId.replace(
        new RegExp("\\.", "g"),
        "\\."
      )}\\)$`,
      "gm"
    ...
    return _.get(
      runnerForBundleRegEx.exec(instrumentationRunners),
      [1],
      "undefined"
    );
  }
  ...
  async shell(deviceId, command, options) {
    const result = await this.adbCmd(
      deviceId,
      `shell "${escape.inQuotedString(command)}"`,
      options
    );
