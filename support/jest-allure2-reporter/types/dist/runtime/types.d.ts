import type { AttachmentsOptions, BuiltinContentAttachmentHandler, BuiltinFileAttachmentHandler, LabelName, LinkType, MaybeNullish, MaybePromise, Parameter, Primitive, Severity, Status, StatusDetails } from '@support/jest-allure2-reporter';
...
export interface AllureRuntime {
    ...
    attachment<T extends AttachmentContent>(name: string, content: MaybePromise<T>, options?: Omit<ContentAttachmentOptions, 'name'>): Promise<string | undefined>;
    attachment<T extends AttachmentContent>(name: string, content: MaybePromise<T>, mimeType?: string): Promise<string | undefined>;
    createAttachment<T extends AttachmentContent, F extends Function_<MaybePromise<T>>>(function_: F, name: string): F;
    createAttachment<T extends AttachmentContent, F extends Function_<MaybePromise<T>>>(function_: F, options: ContentAttachmentOptions): typeof function_;
    fileAttachment(filePath: string | Promise<string>, name?: string): Promise<string | undefined>;
    fileAttachment(filePath: string | Promise<string>, options?: FileAttachmentOptions): Promise<string | undefined>;
    createFileAttachment<F extends Function_<MaybePromise<string>>>(function_: F, name?: string): F;
    createFileAttachment<F extends Function_<MaybePromise<string>>>(function_: F, options?: FileAttachmentOptions): F;
...
export interface AllureRuntimePluginContext {
    ...
    readonly contentAttachmentHandlers: Record<BuiltinContentAttachmentHandler | 'default' | string, ContentAttachmentHandler>;
    readonly fileAttachmentHandlers: Record<BuiltinFileAttachmentHandler | 'default' | string, FileAttachmentHandler>;
...
export type AttachmentHandler<Context extends AttachmentContext> = (context: Readonly<Context>) => MaybePromise<string | undefined>;
