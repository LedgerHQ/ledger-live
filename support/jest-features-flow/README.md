...
The Native project resolves the `react-native` conditional export for workspace `@features/*` and
`@shared/*` packages only. This keeps flow package imports platform-correct without changing each
consumer's Jest config, while third-party Node dependencies retain their default resolution.
