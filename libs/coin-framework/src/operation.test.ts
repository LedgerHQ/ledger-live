...
describe("Operation.ts", () => {
  ...
  describe("isOldestBitcoinPendingOperation", () => {
    ...
    it("should return false if the given date is the same as the only pending operation", () => {
      const testAccount = { ...account, pendingOperations: [pendingCoinOperation0] };

      expect(isOldestBitcoinPendingOperation(testAccount, new Date(1986, 0, 2))).toBe(false);
    });

    it("should return true if the given date is less than the only pending operation", () => {
      const testAccount = {
        ...account,
        pendingOperations: [pendingCoinOperation1, pendingCoinOperation2],
      };
    ...
    it("should return false if the given date is greater than the only pending operation", () => {
      const testAccount = {
        ...account,
        pendingOperations: [pendingCoinOperation0, pendingCoinOperation1],
      };
