...
export function serializeMap<T>(
  ...
  return Buffer.concat([
    encodeSize(keys.length),
    ...keys.flatMap(key => [encodeKey(key), encodeValue(map[key])]),
  ]);
}
...
export function serializeYearMonth(yearMonth: string): Buffer {
  ...
  const year = Number.parseInt(yearMonth.substring(0, 4), 10);
  const month = Number.parseInt(yearMonth.substring(4, 6), 10);
