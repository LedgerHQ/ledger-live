...
describe("utils", () => {
  ...
  describe("serializePath", () => {
    it("should serialize a path array to buffer", () => {
      const path = [0x8000002c, 0x80000397, 0x80000000, 0, 0];
  ...
  describe("decodeWord16", () => {
    it("should decode a 16-bit value", () => {
      expect(decodeWord16(Buffer.from([0x01, 0x00]))).toBe(256);
    });

    it("should decode at a given offset", () => {
      expect(decodeWord16(Buffer.from([0x00, 0x01, 0x00]), 1)).toBe(256);
    });

    it("should throw for buffer too short", () => {
      expect(() => decodeWord16(Buffer.from([0x01]))).toThrow("Buffer too short");
    });
  });

  describe("decodeWord32", () => {
    it("should decode a 32-bit value", () => {
      expect(decodeWord32(Buffer.from([0x00, 0x00, 0x01, 0x00]))).toBe(256);
    });

    it("should throw for buffer too short", () => {
      expect(() => decodeWord32(Buffer.from([0x01, 0x02, 0x03]))).toThrow("Buffer too short");
    });
  });

  describe("decodeWord64", () => {
    it("should decode a 64-bit value", () => {
      expect(decodeWord64(Buffer.from([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00]))).toBe(
        256n,
      );
    });

    it("should throw for buffer too short", () => {
      expect(() => decodeWord64(Buffer.from([0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07]))).toThrow(
        "Buffer too short",
      );
    });
  });
