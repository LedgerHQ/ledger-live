...
- **Linting:** uses [oxlint](https://oxc.rs/docs/guide/usage/linter) with `.oxlintrc.json`. Auto-fix: `pnpm common lint:fix` (oxlint `--fix` only; does not reformat files). **Formatting** is optional: [oxfmt](https://oxc.rs/docs/guide/usage/formatter) using the monorepo root [`.oxfmtrc.json`](../../.oxfmtrc.json) (aligned with `.prettierrc`, e.g. `arrowParens: "avoid"`). Run `pnpm common format` or `pnpm common format:check` when you want to apply or verify oxfmt; live-common-specific paths are ignored via `ignorePatterns` in that root config. From the repo root: `pnpm common lint` (or `pnpm common lint:ci` for CI-style, errors only).
