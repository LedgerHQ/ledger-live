...
test("should serialize a WalletAPI signed transaction", () => {
  const serializedSignedTransaction =
    serializeWalletAPISignedTransaction(SIGNED_TRANSACTION);

  expect(serializedSignedTransaction).toEqual(RAW_SIGNED_TRANSACTION);
});

test("should deserialize a raw WalletAPI signed transaction", () => {
  const signedTransaction = deserializeWalletAPISignedTransaction(
...
describe("Serialize -> Deserialize flow", () => {
  test("should not alter signedTransaction", () => {
    ...
    const expectedSignedTransaction = deserializeWalletAPISignedTransaction(
