...
export function createDeviceOpenHandler(getDeps: () => HandlerDeps): WalletHandlers["device.open"] {
  return params => {
...
export function createDeviceExchangeHandler(
  getDeps: () => HandlerDeps,
): WalletHandlers["device.exchange"] {
  return params => {
    const { device, tracking, manifest } = getDeps();
...
export function createDeviceCloseHandler(
  getDeps: () => HandlerDeps,
): WalletHandlers["device.close"] {
  return ({ transportId }) => {
    const { device, tracking, manifest } = getDeps();
