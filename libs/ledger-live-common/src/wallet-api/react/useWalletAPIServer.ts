...
import type { HandlerDeps, DeviceTransport } from "../handlers/types";
...
import { createAccountGetPublicKeyHandler } from "../handlers/accountGetPublicKey";
...
export function useWalletAPIServer({
  ...
  const mergedCustomHandlers = useMemo(() => {
    const featureFlagsHandlersInstance = featureFlagsHandlers({
      manifest,
      getFeature,
    });
