...
export async function completeExchangeLogic(
  ...
  const toAccount = resolveToAccount(toAccountId, accounts);
  ...
  // `toAccount` is `AccountLike | undefined`; this call mirrors the prior (implicitly-typed)
  // behaviour where makeEmptyTokenAccount was only reached on the swap-to-token path.
  const newTokenAccount = currency
    ? makeEmptyTokenAccount(toAccount as Account, currency)
    : undefined;
  const toParentAccount = toAccount ? getParentAccount(toAccount, accounts) : undefined;
  const exchange = {
    fromAccount,
    fromParentAccount: fromAccount === fromParentAccount ? undefined : fromParentAccount,
    fromCurrency: getCurrencyForAccount(fromAccount),
    toAccount: newTokenAccount ?? toAccount,
...
function resolveToAccount(
  ...
  return accounts.find(a => a.id === realToAccountId);
}
