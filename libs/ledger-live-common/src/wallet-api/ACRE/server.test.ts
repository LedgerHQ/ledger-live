...
describe("ACRE Server Handlers", () => {
  let mockActionDispatcher: jest.Mock;
  let mockUiHooks: any;
  let serverHandlers: any;

  beforeEach(() => {
    jest.clearAllMocks();
    mockActionDispatcher = jest.fn();
    mockUiHooks = {
  ...
  describe("custom.acre.registerYieldBearingEthereumAddress", () => {
    ...
    it("should register new yield-bearing Ethereum address successfully on mobile", async () => {
      ...
      // Verify both Redux actions were called
      expect(mockActionDispatcher).toHaveBeenCalledTimes(2);

      // Verify ADD_ACCOUNT action was called
      expect(mockActionDispatcher).toHaveBeenCalledWith({
        type: "ADD_ACCOUNT",
      ...
      // Verify SET_ACCOUNT_NAME action was called
      expect(mockActionDispatcher).toHaveBeenCalledWith({
    ...
    it("should register new yield-bearing Ethereum address successfully on desktop", async () => {
      ...
      // Verify desktop-specific Redux action
      expect(mockActionDispatcher).toHaveBeenCalledWith({
    ...
    it("should handle existing account gracefully", async () => {
      ...
      expect(mockActionDispatcher).not.toHaveBeenCalled();
    });

    it("should handle missing action dispatcher", async () => {
      const serverHandlersWithoutDispatcher = handlers({
        accounts: [mockAccount],
        tracking: mockTracking,
        manifest: mockManifest,
      ...
      await expect(
        (serverHandlersWithoutDispatcher["custom.acre.registerYieldBearingEthereumAddress"] as any)(
          mockParams,
        ),
      ).rejects.toThrow("No action dispatcher available");
    });
  ...
  describe("Platform-specific behavior", () => {
    it("should dispatch ADD_ACCOUNT for mobile platform", async () => {
      const mobileHandlers = handlers({
        accounts: [mockAccount],
        tracking: mockTracking,
        manifest: mockManifest,
        actionDispatcher: mockActionDispatcher,
      ...
      await (mobileHandlers["custom.acre.registerYieldBearingEthereumAddress"] as any)({
        ethereumAddress: "0x9876543210987654321098765432109876543210",
        tokenContractAddress: "0x1234567890123456789012345678901234567890",
      });
    ...
    it("should dispatch REPLACE_ACCOUNTS for desktop platform", async () => {
      const desktopHandlers = handlers({
        accounts: [mockAccount],
        tracking: mockTracking,
        manifest: mockManifest,
        actionDispatcher: mockActionDispatcher,
      ...
      await (desktopHandlers["custom.acre.registerYieldBearingEthereumAddress"] as any)({
        ethereumAddress: "0x9876543210987654321098765432109876543210",
        tokenContractAddress: "0x1234567890123456789012345678901234567890",
      });
      ...
      expect(mockActionDispatcher).toHaveBeenCalledWith({
        type: "REPLACE_ACCOUNTS",
        payload: expect.arrayContaining([expect.any(Object), mockAccount]),
      });
    });
  });
});
