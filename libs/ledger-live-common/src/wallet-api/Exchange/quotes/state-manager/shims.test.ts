...
/**
 * These modules are re-export shims kept so the apps can go on importing the
 * live-common paths after the endpoint moved to `@domain/api-swap-quotes`.
 * A broken re-export would only show up at app startup, so pin the identity of
 * what they forward.
 */
