...
describe("getAccountBalanceRows", () => {
  ...
  it("drops token assets entirely when the family resolves none, keeping the main row", async () => {
    getBalance.mockResolvedValue([erc20(42n)]);
    getTokenFromAsset.mockResolvedValue(undefined);
    const rows = await read();
    expect(rows).toHaveLength(1);
    expect(rows[0].accountId).toBe(ACCOUNT_ID);
  });

  it("still emits a zero main row when the chain omits the native entry", async () => {
    getBalance.mockResolvedValue([erc20(42n)]);
    const rows = await read();
    expect(rows[0]).toMatchObject({ accountId: ACCOUNT_ID, assetId: "ethereum", balance: "0" });
    expect(rows).toHaveLength(2);
  });

  it("returns a single zero row for an address holding nothing at all", async () => {
    getBalance.mockResolvedValue([]);
    const rows = await read();
    expect(rows).toHaveLength(1);
    expect(rows[0]).toMatchObject({ balance: "0", spendableBalance: "0" });
  });
...
describe("accountRefOf", () => {
  ...
  it("rejects a token account outright", () => {
    expect(() =>
      accountRefOf({ ...account, id: `${ACCOUNT_ID}+ethereum%2Ferc20%2Fusd__coin` }),
    ).toThrow();
  });
});
