...
export type SponsoredSendActions = Readonly<{
  ...
  startRentPayment: (combinedSignature: string) => Promise<void>;
...
export function useSponsoredSendOrchestration(params: UseSponsoredSendOrchestrationParams): {
  ...
  const startRentPayment = useCallback(
    async (combinedSignature: string) => {
      const order = state.order;
      if (!order) return;
      // Only the rent-signing step may broadcast TX-A. `state.order` is retained into TRANSFER/DONE,
      // so a stale or duplicate signature callback firing after the flow advanced must not re-submit
      // and double-charge the rental. A retry re-enters RENT_SIGNING, so it stays allowed.
      if (state.phase !== SPONSORED_PHASE.RENT_SIGNING) return;
      if (inFlightRef.current) return;
      inFlightRef.current = true;
      try {
        const generation = generationRef.current;
        // TX-A's id and the native amount to reserve were captured from the crafted order at CRAFT_SUCCESS.
        const paymentTxId = state.paymentTxId ?? undefined;
        ...
        const reserveRentPayment = () => {
          if (!state.payerAddress || state.reservedNativeAmount == null) return;
          params.onRentPaymentBroadcast?.({
            paymentTxId,
            payerAddress: state.payerAddress,
            reservedNativeAmount: state.reservedNativeAmount,
          });
        };
    ...
    // `state.phase` gates the TX-A broadcast (see the guard above); it must be a dependency so the
    // memoized callback tracks the live phase — POLLING_START/DELIVERY_SUCCESS/TRANSFER_SUCCESS advance
    // the phase without changing another listed dep, which would otherwise keep the closure stale.
    // oxlint's memo-dependencies miscounts it as extra; the authoritative exhaustive-deps agrees it belongs.
    [
