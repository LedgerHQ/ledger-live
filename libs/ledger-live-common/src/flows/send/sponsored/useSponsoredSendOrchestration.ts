  ...
  const startRentPayment = useCallback(
    async (signedTransaction: unknown, paymentTxId?: string) => {
      ...
      try {
        ...
        dispatch(
          await reconcileSubmitFailure(error as Error, {
            submitAttempted,
            seam,
            orderId: order.orderId,
            payerAddress: requestRef.current?.payerAddress,
            paymentTxId,
          }),
        );
        return;
      ...
      try {
        ...
        dispatch(
          await reconcileDeliveryOutcome(error as Error & { paymentTxId?: string }, {
            seam,
            orderId: order.orderId,
            payerAddress,
          }),
