  ...
  const startRentPayment = useCallback(
    async (combinedSignature: string) => {
      ...
      try {
        ...
        if (!seam) throw new Error("Sponsored send is unavailable for this account");
        // The family seam rebuilds its wire-shaped signed payload from the device's combined signature,
        // so this generic flow submits an opaque tx it never constructed.
      ...
      try {
        ...
        // The controller has done its job once the poll settles; drop the ref (only while it is still
        // ours) so a later reset/unmount doesn't abort a dead controller.
