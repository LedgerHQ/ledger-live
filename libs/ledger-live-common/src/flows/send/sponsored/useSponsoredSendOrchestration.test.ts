...
test("closing the dialog (unmount) during polling aborts the delivery poll", async () => {
  ...
  await act(async () => {
    void result.current.actions.startRentPayment({}, "tx-A-hash");
    await started;
  });
...
test("submit failure, provider still reports paid but the chain confirms -> proceeds to TRANSFER", async () => {
  ...
  await act(async () => {
    await result.current.actions.startRentPayment({}, "tx-A-lagging");
  });
  ...
  expect(result.current.state.phase).toBe(SPONSORED_PHASE.TRANSFER);
  expect(result.current.state.paymentTxId).toBe("tx-A-lagging");
});
...
const broadcastInfo = { paymentTxId: "txA", payerAddress: "TPayer", payCoinAmt: "5.0" };
...
test("onRentPaymentBroadcast fires on the happy submit path with the payer and amount", async () => {
  ...
  await act(async () => {
    await result.current.actions.startRentPayment({}, "txA");
  });
