...
export function useCoinControlScreenViewModelCore<TNetworkFees = unknown>({
  ...
  const onSelectStrategy = useCallback(
    (value: string) => {
      ...
      if (coinControlConfig == null) return;
  ...
  const toggleUtxoExclusion = useCallback(
    (rowKey: string) => {
      if (coinControlConfig == null) return;
      if (!("utxoStrategy" in transaction) || transaction.utxoStrategy == null) return;
      if (transaction.utxoStrategy.strategy !== coinControlConfig.customStrategyValue) return;
  ...
  const onToggleUtxoExclusion =
    coinControlConfig != null &&
    "utxoStrategy" in transaction &&
    transaction.utxoStrategy != null &&
    transaction.utxoStrategy.strategy === coinControlConfig.customStrategyValue
      ? toggleUtxoExclusion
      : undefined;
