...
function transformAssetsResponse(
  ...
  const enrichedCryptoOrTokenCurrencies = convertApiAssets(responseWithMocks.cryptoOrTokenCurrencies);
...
async function fetchAssetsPage(
  ...
  const rawWithMocks = injectAleoMockData(raw);
  const enrichedCryptoOrTokenCurrencies = convertApiAssets(rawWithMocks.cryptoOrTokenCurrencies);
