...
export function injectAleoMockData(response: RawApiResponse): RawApiResponse {
  ...
  return {
    ...
    currenciesOrder: {
      ...
      metaCurrencyIds: response.currenciesOrder.metaCurrencyIds.includes("aleo")
        ? response.currenciesOrder.metaCurrencyIds
        : ["aleo", ...response.currenciesOrder.metaCurrencyIds],
    },
    markets: {
      ...response.markets,
      ...aleoData.markets,
