...
export const sendBTCBasedCoin = withDeviceController(({ getDevice }) => async (tx: Transaction) => {
...
export const sendBTC = withDeviceController(({ getDevice }) => async (tx: Transaction) => {
  const buttons = getDevice().buttonFactory();
