...
export function waitForSpeculosReady(
  ...
  return new Promise((resolve, reject) => {
    ...
    function retry() {
      ...
      if (elapsed >= timeout) {
        reject(new Error(errorMessage ?? `❌ Speculos not ready after ${timeout}s at ${url}`));
      } else {
        console.warn(`🔁 Waiting for Speculos at ${url} (${elapsed}ms elapsed)`);
