...
describe("market countervalues API", () => {
  ...
  it("does not send a one-character filter by default", async () => {
    await fetchList({ counterCurrency: "usd", search: "b" });

    expect(getLastRequestUrl().searchParams.get("filter")).toBeNull();
  });

  it("sends a one-character filter when requested", async () => {
    await fetchList({ counterCurrency: "usd", search: " B ", minSearchLength: 1 });

    expect(getLastRequestUrl().searchParams.get("filter")).toBe("b");
  });
});
