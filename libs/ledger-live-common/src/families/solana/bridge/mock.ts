...
import type { ChainAPI, Config } from "@ledgerhq/coin-solana/network/index";
...
import { buildIntentData, computeIntentType, getAssetFromToken } from "./api";
      ...
      get(_, propKey) {
        ...
        return function (...rawArgs: unknown[]) {
          ...
          const mock =
            ...
            // The fee never reaches the signed bytes, so any recorded answer will do.
...
const getTransactionStatus: SolanaMockBridge["getTransactionStatus"] = async (account, t) => {
  ...
  // Opening a token account and revoking a delegation carry no recipient.
  const needsRecipient = t.mode !== "opt-in" && t.mode !== "revoke";
...
async function signWithMockedChain(
  ...
  const intent = transactionToIntent(
    ...
    computeIntentType as never,
    undefined,
    buildIntentData as never,
  );
...
const accountBridge: SolanaMockBridge = {
  ...
  // `toAccountRaw` reads these off the bridge, so a mocked account survives a reload.
  ...accountRawAssign,
};
