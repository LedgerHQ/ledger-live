...
/**
 * `templateId` is all the Solana signer kit needs to clear-sign a partner transaction. The legacy
 * signer also wants `deviceModelId`, which comes from a signing context this hook is not given, so
 * a legacy device signs a token transfer without its descriptors — see LIVE-35692.
 */
