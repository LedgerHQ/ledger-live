...
/** Crafting adds the rent to a typed amount, and carves it out of a send-max one. */
export function getStakeCreationDeposit({
  amount,
  stakeAccountRent,
  useAllAmount,
}: {
  ...
  return useAllAmount ? amount : amount.plus(stakeAccountRent ?? 0);
}
...
/** Delegating reuses the memo to carry the stake account, which is not a user memo. */
