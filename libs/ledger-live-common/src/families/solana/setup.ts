...
let _legacySignerInstance: LegacySignerSolana | null = null;
...
export function getSolanaSignerInstance(
  ...
  if (canDMKSignerBeUsed(transport)) {
    ...
    if (!_legacySignerInstance) {
      _legacySignerInstance = new LegacySignerSolana(transport);
    }
    return _legacySignerInstance;
  }
}
