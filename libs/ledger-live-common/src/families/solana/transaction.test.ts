...
describe("solana transaction serialization", () => {
  it.each([
    ...
    ["a partner-built transaction", { raw: "AQID", templateId: "tpl-1" }],
    [
      "a stake creation carrying its seed",
      { mode: "stake", familySpecificData: { stakeAccountSeed: "seed-1" } },
    ],
...
describe("solana formatTransaction", () => {
  ...
  it("carries the asset identity through the raw shape, so a restored SPL transaction stays one", () => {
    const raw = toTransactionRaw({
      family: "solana",
      mode: "send",
      amount: new BigNumber(1),
      recipient: "rec",
      assetReference: "mint1",
      assetOwner: "owner1",
    } as Transaction);

    expect(raw).toMatchObject({ assetReference: "mint1", assetOwner: "owner1" });
    expect(fromTransactionRaw(raw)).toMatchObject({
      assetReference: "mint1",
      assetOwner: "owner1",
    });
  });
