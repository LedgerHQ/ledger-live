...
/** `transferFee`, `stakeAccountRent` and `raw` are absent from the raw shape by design. */
export const fromTransactionRaw = (tr: TransactionRaw): Transaction => ({
  ...
  // Without these a restored SPL transaction would be rebuilt as a native one.
  ...(tr.assetReference ? { assetReference: tr.assetReference } : {}),
  ...(tr.assetOwner ? { assetOwner: tr.assetOwner } : {}),
});
...
export const toTransactionRaw = (t: Transaction): TransactionRaw => ({
  ...
  ...(t.assetReference ? { assetReference: t.assetReference } : {}),
  ...(t.assetOwner ? { assetOwner: t.assetOwner } : {}),
});
...
/** One line for the summary log: must stay synchronous and never throw. */
