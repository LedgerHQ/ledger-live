...
describe("getWalletAPITransactionSignFlowInfos", () => {
  describe("should properly get infos for Solana TX", () => {
    ...
    it("carries the token sub-account the live app named, on the main account the flow passes", () => {
      // `wallet-api/logic.ts` calls this adapter with the *main* account, so the sub-account can
      // only come from the model.
      const { liveTx } = signFlowInfos({
        family: "solana",
        amount: new BigNumber(100000),
        recipient: "0xABCDEFG",
        model: { kind: "token.transfer", uiState: { subAccountId: "js:2:solana:xpub:+usdc" } },
      } as WalletAPITransaction);

      expect(liveTx).toMatchObject({ mode: "send", subAccountId: "js:2:solana:xpub:+usdc" });
    });

    it("leaves no sub-account when the live app sends the empty placeholder", () => {
      const { liveTx } = signFlowInfos({
        family: "solana",
        amount: new BigNumber(100000),
        recipient: "0xABCDEFG",
        model: { kind: "token.transfer", uiState: { subAccountId: "" } },
      } as WalletAPITransaction);

      expect(liveTx.subAccountId).toBeUndefined();
    });
