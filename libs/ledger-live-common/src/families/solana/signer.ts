...
export const createSigner: CreateSigner<SolanaSigner> = (transport: Transport) => {
  ...
  return {
    ...
    signTransaction: async (path: string, txBase64: string, resolution?: Resolution) => {
      ...
      // `LegacySignerSolana` throws on a resolution with no `deviceModelId`, which no signing
      // context reaches this far; the DMK signer resolves the certificate itself — see LIVE-35692.
