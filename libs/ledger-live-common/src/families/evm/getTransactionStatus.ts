...
const validateAmount = (
  ...
  // if no amount or 0
  if (!tx.amount || tx.amount.isZero()) {
    errors.amount = new AmountRequired(); // "Amount required"
  } else if (tx.amount.plus(estimatedFees).isGreaterThan(account.balance)) {
...
export const getTransactionStatus: AccountBridge<EvmTransaction>["getTransactionStatus"] =
  (account, tx) => {
    ...
    const [amountErr, amountWarn] = validateAmount(account, tx, estimatedFees);
    ...
    return Promise.resolve({
      ...
      estimatedFees,
      amount: tx.amount,
      totalSpent: tx.amount?.plus(estimatedFees) || new BigNumber(0),
    });
  };
