  ...
  const res = {
    ...
    estimate: {
      ...
      expirationTimestampSecs:
        transaction.options.expirationTimestampSecs || "",
    },
  ...
  try {
    ...
    if (!completedTx.success) {
      switch (true) {
        ...
        default: {
          throw Error(
            `Simulation failed with following error: ${completedTx.vm_status}`
          );
        }
      }
    } else {
      res.estimate.maxGasAmount = BigNumber(
        Math.ceil(
          completedTx.gas_used
            ? +completedTx.gas_used * ESTIMATE_GAS_MUL
            : DEFAULT_GAS
        )
      ).toString();
      ...
      if (transaction.skipEmulation) {
        res.fees = res.fees
          .plus(
            BigNumber(
              transaction.options.gasUnitPrice ||
                res.estimate.gasUnitPrice ||
                DEFAULT_GAS_PRICE
            )
          )
          .multipliedBy(
            BigNumber(
              transaction.options.maxGasAmount ||
                res.estimate.maxGasAmount ||
                DEFAULT_GAS
            )
          );
      } else {
      ...
      res.estimate.expirationTimestampSecs =
        completedTx.expiration_timestamp_secs;
    }
  } catch (e: any) {
    log(e.message);
    throw e;
