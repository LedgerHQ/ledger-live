...
function assertAleo(
  result: ReturnType<typeof useAleoQuickAmountSelector>,
): asserts result is ReturnType<typeof useAleoQuickAmountSelector> & { isAleo: true } {
  expect(result.isAleo).toBe(true);
}
...
describe("useAleoQuickAmountSelector", () => {
  ...
  it("reports isAleo false for an account of another family", () => {
    const account = {
    ...
    const { result } = renderHook(() =>
      useAleoQuickAmountSelector(account, transaction, jest.fn()),
    );

    expect(result.current).toEqual({ isAleo: false });
  });
