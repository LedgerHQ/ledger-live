...
const cosmosMinAmount = new BigNumber(10000);
const cosmosMaxAccounts = 32;

const osmosisMinAmount = new BigNumber(10);
const osmosisMaxAccounts = 16;
...
const cosmos: AppSpec<Transaction> = {
  ...
  minViableAmount: cosmosMinAmount,
  transactionCheck: ({ maxSpendable }) => {
    invariant(maxSpendable.gt(cosmosMinAmount), "balance is too low");
  },
  ...
  mutations: [
    {
      ...
      transaction: ({ account, siblings, bridge, maxSpendable }) => {
        ...
        return {
          ...
          updates: [
            {
              recipient: pickSiblings(siblings, cosmosMaxAccounts).freshAddress,
            },
            {
              amount,
            },
...
const osmosis: AppSpec<Transaction> = {
  ...
  minViableAmount: osmosisMinAmount,
  transactionCheck: ({ maxSpendable }) => {
    invariant(maxSpendable.gt(osmosisMinAmount), "balance is too low");
  },
  test: ({ account, operation, optimisticOperation }) => {
    const allOperationsMatchingId = account.operations.filter(
      (op) => op.id === operation.id
  ...
  mutations: [
    {
      ...
      transaction: ({ account, siblings, bridge, maxSpendable }) => {
        invariant(maxSpendable.gt(osmosisMinAmount), "balance is too low");
        const sibling = pickSiblings(siblings, osmosisMaxAccounts);
    ...
    },
    {
      ...
      transaction: ({ account, bridge }) => {
        ...
        let remaining = getMaxDelegationAvailable(
          ...
          .minus(osmosisMinAmount.times(2))
