...
describe("cosmos/logic", () => {
  ...
  describe("useCosmosFormattedDelegations", () => {
    ...
    it("should not display the banner", async () => {
      jest.spyOn(preloadedData, 'getCurrentCosmosPreloadData').mockReturnValue({validators});
    ...
    it("should return display redelegate mode", async () => {
      ...
      account.cosmosResources.redelegations.push({validatorDstAddress: expensiveValidator.validatorAddress})
      const result = await getAccountBannerState(account);
      expect(result).toStrictEqual({
        display: true,
        redelegate: true,
