...
const defaultEndpoint = getEnv(
	"API_COSMOS_BLOCKCHAIN_EXPLORER_API_ENDPOINT" //< TODO: Fix the name
).replace(/\/$/, "");

export class ZilliqaAPI {
	protected _defaultEndpoint: string = defaultEndpoint;
	protected _namespace = "zilliqa";
	...
	getAccountInfo = async (
		...
		try {
			...
			throw new Error(
				`"Error during zilliqa synchronization: "${e.message}`
			);
		}
	};
	...
	getAccount = async (
		...
		try {
			const { data } = await network({
				method: "GET",
				url: `${this._defaultEndpoint}/zilliqa/auth/v1beta1/accounts/${address}`,
			});
	...
	getHeight = async (): Promise<number> => {
		const { data } = await network({
			method: "GET",
			url: `${this._defaultEndpoint}/zilliqa/base/tendermint/v1beta1/blocks/latest`,
		});
	...
	getAllBalances = async (
		...
		const { data } = await network({
			method: "GET",
			url: `${this._defaultEndpoint}/zilliqa/bank/v1beta1/balances/${address}`,
		});
	...
	getDelegations = async (
		...
		const { data: data1 } = await network({
			method: "GET",
			url: `${this._defaultEndpoint}/zilliqa/staking/v1beta1/delegations/${address}`,
		});
		...
		for (const d of data1.delegation_responses) {
			const { data: data2 } = await network({
				method: "GET",
				url: `${this._defaultEndpoint}/zilliqa/staking/v1beta1/validators/${d.delegation.validator_address}`,
			});
		...
		const { data: data3 } = await network({
			method: "GET",
			url: `${this._defaultEndpoint}/zilliqa/distribution/v1beta1/delegators/${address}/rewards`,
		});
	...
	getRedelegations = async (address: string): Promise<any> => {
		...
		const { data } = await network({
			method: "GET",
			url: `${this._defaultEndpoint}/zilliqa/staking/v1beta1/delegators/${address}/redelegations`,
		});
	...
	getUnbondings = async (address: string): Promise<any> => {
		...
		const { data } = await network({
			method: "GET",
			url: `${this._defaultEndpoint}/zilliqa/staking/v1beta1/delegators/${address}/unbonding_delegations`,
		});
	...
	getWithdrawAddress = async (address: string): Promise<string> => {
		const { data } = await network({
			method: "GET",
			url: `${this._defaultEndpoint}/zilliqa/distribution/v1beta1/delegators/${address}/withdraw_address`,
		});
	...
	getTransactions = async (address: string): Promise<any> => {
		const receive = await network({
			method: "GET",
			url:
				`${this._defaultEndpoint}/zilliqa/tx/v1beta1/txs?events=` +
	...
	simulate = async (tx_bytes: Array<any>): Promise<BigNumber> => {
		try {
			const { data } = await network({
				method: "POST",
				url: `${this._defaultEndpoint}/zilliqa/tx/v1beta1/simulate`,
				data: {
					tx_bytes: tx_bytes,
				},
			});
	...
	broadcast = async ({
		...
		const { data } = await network({
			method: "POST",
			url: `${this._defaultEndpoint}/zilliqa/tx/v1beta1/txs`,
			data: {
				tx_bytes: Array.from(
		...
		if (data.tx_response.code != 0) {
			// error codes: https://github.com/zilliqa/zilliqa-sdk/blob/master/types/errors/errors.go
			throw new Error(
...
export const defaultZilliqaAPI = new ZilliqaAPI();
