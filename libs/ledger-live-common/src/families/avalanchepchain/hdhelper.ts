...
class HDHelper {
    ...
    static async getInstance(publicKey: string, chainCode: string): Promise<HDHelper> {
        if (!this.instance) {
            this.instance = new this(publicKey, chainCode);
        }

        this.instance.hdIndex = await this.instance.findAvailableIndexNode();
    ...
    async getAllUXTOsForAddresses(addresses: string[], endIndex: any = undefined): Promise<UTXOSet> {
        const pChain = avalancheClient().PChain();
        let response;
        ...
        if (len >= 1024) {
            const subUtxos = await this.getAllUXTOsForAddresses(addresses, nextEndIndex);
