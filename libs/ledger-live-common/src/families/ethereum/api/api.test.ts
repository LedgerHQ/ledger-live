...
describe("apiForCurrency", () => {
  ...
  describe("getTransactions", () => {
    ...
    it("should have a default batch size of 2000", async () => {
      ...
      expect((formatUrlInput.query as ParsedUrlQueryInput).batch_size).toEqual(2000);
    });

    it("should have filtering query parameter to true", async () => {
      await apiForCurrency(currencyMock).getTransactions(addressMock);
      const formatUrlInput: URL.UrlObject = urlFormatSpy.mock.lastCall[0];
      expect((formatUrlInput.query as ParsedUrlQueryInput).filtering).toEqual(true);
    });

    describe("when batch_size is defined", () => {
      it("should use provided batch_size", async () => {
        const batchSizeMock = 42;
        await apiForCurrency(currencyMock).getTransactions(addressMock, undefined, batchSizeMock);
        const formatUrlInput: URL.UrlObject = urlFormatSpy.mock.lastCall[0];
        expect((formatUrlInput.query as ParsedUrlQueryInput).batch_size).toEqual(batchSizeMock);
      });
    });
    ...
    describe("when blockHeight is defined", () => {
      ...
      it("should use ascending order", async () => {
        ...
        const formatUrlInput: URL.UrlObject = urlFormatSpy.mock.lastCall[0];
        expect((formatUrlInput.query as ParsedUrlQueryInput).order).toEqual("ascending");
      });

      it("should use defined blockHeight for backend request", async () => {
        const blockHeightMock = 9000;
        await apiForCurrency(currencyMock).getTransactions(addressMock, blockHeightMock);
        const formatUrlInput: URL.UrlObject = urlFormatSpy.mock.lastCall[0];
        expect((formatUrlInput.query as ParsedUrlQueryInput).from_height).toEqual(blockHeightMock);
        // ↓ this is necessary in order to get transactions after the block height and not before. Order is still descending in the end
        expect((formatUrlInput.query as ParsedUrlQueryInput).order).toEqual("ascending");
      });
    });
    ...
    describe("when token is defined", () => {
      it("should use defined token for backend request", async () => {
        ...
        const formatUrlInput: URL.UrlObject = urlFormatSpy.mock.lastCall[0];
        expect((formatUrlInput.query as ParsedUrlQueryInput).token).toEqual(tokenMock);
      });
    });

    it("should return backend txs in txs attribute", async () => {
      const result = await apiForCurrency(currencyMock).getTransactions(addressMock);
