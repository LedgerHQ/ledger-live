...
export const prepareTransaction: AccountBridge<Transaction>["prepareTransaction"] = async (
  ...
  const networkInfo = tx.networkInfo || (await getNetworkInfo(account.currency));

  if (EIP1559ShouldBeUsed(account.currency)) {
    const maxPriorityFeePerGas = inferMaxPriorityFeePerGas(tx, networkInfo as NetworkInfo);
    const maxFeePerGas = inferMaxFeePerGas(tx, maxPriorityFeePerGas, networkInfo as NetworkInfo);
  ...
  if (isEthereumAddress(tx.recipient)) {
    ...
    const protoTransaction = (() => {
      try {
        ...
        const recipient =
          subAccount?.type === "TokenAccount" ? subAccount.token.contractAddress : tx.recipient;
        return {
