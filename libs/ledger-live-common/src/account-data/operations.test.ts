...
describe("getAccountOperationPage", () => {
  ...
  it("treats an empty-string cursor as the end of the stream", async () => {
    // Several modules send "" rather than omitting `next`.
  ...
  it("keeps a zero cursor — a falsy cursor is not the end of the stream", async () => {
    // Some modules page with a numeric offset, and `next: 0` is a real first page. Reporting the
    // history as complete there would hide everything past it.
    listOperations.mockResolvedValue({ items: [core("op-1")], next: 0 });
    const page = await read();
