...
export const toSignedOperationRaw = (
  ...
  const out: SignedOperationRaw = {
    operation: toOperationRaw(operation, preserveSubOperation, bridge),
    signature,
  };
  if (rawData) out.rawData = rawData;
  if (expirationDate) out.expirationDate = expirationDate.toISOString();
  return out;
};
...
export const fromSignedOperationRaw = async (
  ...
  if (rawData) out.rawData = rawData;
  if (expirationDate) out.expirationDate = new Date(expirationDate);
  return out;
};
