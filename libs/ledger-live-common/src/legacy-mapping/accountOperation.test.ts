...
describe("toAccountOperations", () => {
  ...
  it("returns one newest-first list, not two ordered halves", () => {
    // The main account's operations and its token accounts' are walked separately, so the
    // concatenation is unordered even though each half is not. A consumer reading through
    ...
    expect(rows.map(r => r.id)).toEqual(["tok-new", "own-old"]);
  });
