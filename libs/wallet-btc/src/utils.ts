 ...
 */
export function scriptToAddress(script: Buffer, crypto: ICrypto): string {
  try {
    return btc.address.fromOutputScript(script, crypto.network as btc.Network);
  } catch (err) {
    ...
    if (script.length === P2TR_SCRIPT_LENGTH && script[0] === OP_1 && script[1] === OP_PUSH_32) {
      ...
      return bech32m.encode(crypto.network.bech32, words);
    }
    throw err;
  }
}
