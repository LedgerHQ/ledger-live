...
describe("testing zcash transactions", () => {
  const explorer = new BitcoinLikeExplorer({
    cryptoCurrency: walletBtcCurrency("zcash"),
  });
  ...
  it("carries the source block height on each selected input", async () => {
    ...
    // Zcash-specific: the scan must carry each selected UTXO's source block height,
    // so downstream signing can build a valid Overwinter transaction.
    const blockHeights = inputs.map(input => input.block_height);
    expect(blockHeights).toEqual(expect.arrayContaining([1806010, 2738883]));
  });
});
