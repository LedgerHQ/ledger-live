    ...
    return setup({
      ...
      guards: {
        ...
        isDeprecation: ({ context }) => {
          const deviceDeprecated = context.input.deprecationConfig; // ou cfg.deviceDeprecated selon ton source
          const today = new Date();
          const modelId = context._internalState.deviceModel;

          if (!modelId || !deviceDeprecated || deviceDeprecated.length === 0) return false;
          if (context._internalState.deprecationShown) return false;

          const isPast = (dateStr?: string) => !!dateStr && new Date(dateStr) < today;

          return deviceDeprecated.some(
            dd =>
              dd.deviceModelId === modelId &&
              (isPast(dd.errorScreen?.date) ||
                isPast(dd.infoScreen?.date) ||
                isPast(dd.warningClearSigningScreen?.date)),
          );
        },
      },
      actions: {
