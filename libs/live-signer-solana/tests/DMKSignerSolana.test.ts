...
describe("DMKSignerSolana", () => {
  ...
  describe("getAddress", () => {
    it("returns decoded address without display", async () => {
      ...
      expect((signer as any).dmkSigner.getAddress).toHaveBeenCalledWith("m/0'/0'", {
        checkOnDevice: false,
      });
    ...
    it("returns decoded address with display", async () => {
      ...
      expect((signer as any).dmkSigner.getAddress).toHaveBeenCalledWith("m/1'/0'", {
        checkOnDevice: true,
      });
      expect(result).toEqual({ address: bytes });
    });
  ...
  describe("signTransaction", () => {
    it("resolves with signature buffer on Completed status", async () => {
      ...
      expect((signer as any).dmkSigner.signTransaction).toHaveBeenCalledWith("path", tx);
  ...
  describe("signMessage", () => {
    it("resolves with signature buffer on Completed status", async () => {
      const sigArray = Uint8Array.from([100, 200]);
      const observable = of({ status: DeviceActionStatus.Completed, output: sigArray });
      (signer as any).dmkSigner = {
        signMessage: jest.fn().mockReturnValue({ observable }),
      };

      const result = await signer.signMessage("path", "deadbeef");
      expect((signer as any).dmkSigner.signMessage).toHaveBeenCalledWith("path", "deadbeef");
      expect(result).toEqual({ signature: Buffer.from(sigArray) });
    });
    ...
    it("rejects on observable error", async () => {
      ...
      await expect(signer.signMessage("path", "cafebabe")).rejects.toThrow("msg error");
    });
  });
});
