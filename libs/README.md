...
> [!WARNING]
> **Legacy.** New code goes in the [monorepo layers](../docs/monorepo-architecture.md). `libs/` is maintained, not grown.
...
Some published packages will also move to [ts-libs](https://github.com/LedgerHQ/ts-libs) rather than to the monorepo layers — see [LIVE-37105](https://ledgerhq.atlassian.net/browse/LIVE-37105).

## Still adding something here?

Only for a new [coin module](./coin-modules) or its coin-tester, a package published to npm, or a
proportionate fix in an existing package. Code extracted from `ledger-live-common` goes to the
monorepo layers, not to a new `libs/*` package. Anything else needs an argument in the PR
description.
