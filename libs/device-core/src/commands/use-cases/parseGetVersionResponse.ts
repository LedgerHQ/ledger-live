...
export function parseGetVersionResponse(response: Buffer): FirmwareInfoEntity {
  ...
  let recoveryKeyState: Buffer | undefined;
  ...
  if (isBootloader) {
    ...
    if (!isOSU) {
      ...
      if (isRecoveryKeySupported(seVersion, deviceModel?.id)) {
        const recoveryKeyStateLength = data[i++];
        recoveryKeyState = Buffer.from(data.slice(i, i + recoveryKeyStateLength));
        i += recoveryKeyStateLength;
      }
    }
  }
