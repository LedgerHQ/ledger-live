...
const getDefinition = (
  ...
  try {
    ...
    const contractAddress = ENTRIES_CHECKS.contractAddress(definitionJSON.contract_address);
...
const importERC20Tokens = async (chain, outputFolder) => {
  ...
  for (const tokenId of tokenIds) {
    const [definitionJSON, signature] = await Promise.allSettled([
      fs
        .readFile(path.join(chain.definitionPath, tokenId, "common.json"), "utf-8")
        .then(JSON.parse),
      fs
        .readFile(path.join(chain.signaturePath, tokenId, "ledger_signature.json"), "utf-8")
        .then(JSON.parse),
    ...
    const signatureBuffer = getSignatureBuffer(chain, definitionJSON, signature);
  ...
  await fs.writeFile(path.join(outputDir, "erc20.json"), JSON.stringify(tokens));
  // add signature file
  await fs.writeFile(
