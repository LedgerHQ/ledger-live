...
test("getAddress without display", async () => {
  ...
  expect(address).toEqual("13MLUBBKhTG6pbUdwwFZXnj7P4R7gB6q2janJf3fFipGjLqDV1k");
});
...
test("signPaymentV2 with no memo", async () => {
  const transport = await openTransportReplayer(
    RecordStore.fromString(`
        => e00800004a00e1f50500000000b88800000000000019000000000000000001351a71c22fefec2231936ad2826b217ece39d9f77fc6c49639926299c386929500000000000000000000000000000000
  ...
  const txnToSign = new PaymentV2({
    fee: 35000,
    payments: [{ amount: 100000000, payee, tokenType: "0" }],
    nonce: 25,
    payer,
  });
  ...
  expect(Buffer.from(txn.signature as Uint8Array).toString("hex")).toBe(expectedSig);
});

test("signPaymentV2 with a memo", async () => {
  const transport = await openTransportReplayer(
    RecordStore.fromString(`
        => e00800004a00e1f50500000000b88800000000000019000000000000000001351a71c22fefec2231936ad2826b217ece39d9f77fc6c49639926299c3869295796f6c6f000000000000000000000000
...
test("signStakeValidatorV1", async () => {
  ...
  expect(Buffer.from(txn.ownerSignature as Uint8Array).toString("hex")).toBe(expectedSig);
});
...
test("signTransferValidatorStakeV1 as old owner", async () => {
  ...
  const { signature, txn } = await helium.signTransferValidatorStakeV1(txnToSign, "old");
  const expectedSig =
  ...
  expect(Buffer.from(txn.oldOwnerSignature as Uint8Array).toString("hex")).toBe(expectedSig);
});
...
test("signTransferValidatorStakeV1 as new owner", async () => {
  ...
  const { signature, txn } = await helium.signTransferValidatorStakeV1(txnToSign, "new");
  const expectedSig =
  ...
  expect(txn.oldAddress?.b58).toBe(oldAddress.b58);
  expect(txn.newAddress?.b58).toBe(newAddress.b58);
  expect(Buffer.from(txn.newOwnerSignature as Uint8Array).toString("hex")).toBe(expectedSig);
});
