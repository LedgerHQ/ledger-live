...
const getAdditionalDataForContract = async (
  ...
  if (shouldResolve.nft) {
    const nftInfo = await getNFTInfo(contractAddress, chainIdUint4, loadConfig);
  ...
  if (shouldResolve.token) {
    const erc20SignaturesBlob = await findERC20SignaturesInfo(loadConfig, chainIdUint4);
...
const resolveTransaction: LedgerEthTransactionService["resolveTransaction"] = async (
  ...
  const chainIdUint4 = getChainIdAsUint4(parsedTransaction.chainId);
