...
import Exchange, { ExchangeTypes, type PartnerKeyInfo } from "./Exchange";
import { signatureExport, signatureImport } from "./DerUtils";
...
describe("Check Exchange until payload signature", () => {
  ...
  describe("Check SWAP", () => {
    it("Legacy SWAP", async () => {
      ...
      const payloadSignature = await signMessage(
        encodedPayload,
        Buffer.from(partnerPrivKey),
        "der",
      );
    ...
    it("NG SWAP", async () => {
      ...
      const payloadSignature = await signMessage(encodedPayload, Buffer.from(partnerPrivKey), "rs");
      await exchange.checkTransactionSignature(payloadSignature);
    });
...
describe("Test internal sign and verification functionality", () => {
  describe("With Ledger keys", () => {
    ...
    test("simple signature", async () => {
      ...
      expect(ecc.verify(sig, hash, pubKey)).toBeTruthy();
    });
    test("APDU signature generated by this test suite", async () => {
      ...
      const hash = new Uint8Array(hashBuffer);
      expect(ecc.verify(convertSignatureFromDER(sig), hash, pubKey)).toBeTruthy();
    });
...
async function appExchangeDatasetTest(signFormat: PartnerSignFormat) {
  ...
  const pubKey = ecc.pointFromScalar(privKey, isCompressed);
