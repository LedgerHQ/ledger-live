...
test("provideTrustedDynamicDescriptor", async () => {
  ...
  expect(
    await solana.provideTrustedDynamicDescriptor({
...
test("should throw on invalid derivation path", async () => {
  ...
  const solana = new Solana(transport);
  return expect(solana.getAddress("some invalid derivation path", false)).rejects.toThrow("input");
});
...
test("report blind signature required", async () => {
  ...
  return expect(solana.getAppConfiguration()).rejects.toThrow("blind signature");
});
