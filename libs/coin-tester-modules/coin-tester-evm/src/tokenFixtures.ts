...
// Hardcoded tokens for EVM tests
...
// Setup mock store with all hardcoded tokens
const mockStore = setupMockCryptoAssetsStore({
  findTokenById: async (id: string) => {
    const tokens: Record<string, TokenCurrency> = {
      [USDC_ON_ETHEREUM.id]: USDC_ON_ETHEREUM,
      [USDC_ON_POLYGON.id]: USDC_ON_POLYGON,
      [USDC_ON_SCROLL.id]: USDC_ON_SCROLL,
      [MIM_ON_BLAST.id]: MIM_ON_BLAST,
      [BRIDGED_USDC_ON_SONIC.id]: BRIDGED_USDC_ON_SONIC,
      [STCORE_ON_CORE.id]: STCORE_ON_CORE,
    };
    return tokens[id];
  },
});

// Also set it for coin-framework
setCryptoAssetsStoreForCoinFramework(mockStore);
