...
export const mineMany = async (address: string) => {
...
export const getCurrentBlock = async () => {
  const information = await client.command("getblockcount");
  return information;
};

export const mineTo = async (recipientAddress: string) => {
...
export const sendTo = async (recipientAddress: string, amount: number) => {
  ...
  try {
    ...
    console.log("Transaction Details (before mining):", JSON.stringify(tx, null, 2));
    ...
    console.log("Transaction Details (after mining):", JSON.stringify(tx, null, 2));
  } catch (error: any) {
...
export const sendAutomatedRaw = async (destinationAddress: string, amount: number) => {
  ...
  console.log(`Creating raw transaction: Sending ${amount} BTC to ${destinationAddress}...`);
...
export const sendRaw = async (recipientAddress: string, amount: number, sequence = 4294967294) => {
  ...
  try {
    ...
    const tx = await client.getTransaction({ txid: txId, verbose: true });
