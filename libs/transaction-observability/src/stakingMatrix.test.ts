 ...
 * is all the broadcast stage gets. `operationType: null` means the action is unrecoverable at
 * broadcast — the family produces a generic type there.
 *
...
/** Currency ids that route to a family, so a new cosmos-SDK chain is covered for free. */
const CURRENCY_FAMILIES: Array<[string, string]> = [
  ["cardano", "cardano"],
  ["celo", "celo"],
  ["cosmos", "cosmos"],
  ["osmo", "cosmos"],
...
describe("staking action derivation", () => {
  ...
  describe("currency ids resolve to a family with a staking vocabulary", () => {
    it.each(CURRENCY_FAMILIES)("%s -> %s", (_currencyId, family) => {
      expect(MATRIX[family]).toBeDefined();
      expect(MATRIX[family].length).toBeGreaterThan(0);
    });
  });
});
