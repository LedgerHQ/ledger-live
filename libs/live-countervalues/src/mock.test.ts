...
import { getFiatCurrencyByTicker, getCryptoCurrencyById, tokensById } from "@ledgerhq/cryptoassets";
...
test("DAI EUR latest price", async () => {
  ...
  const state = await loadCountervalues(initialState, {
    trackingPairs: [
      {
        from: tokensById["ethereum/erc20/dai_stablecoin_v2_0"]!,
        to: getFiatCurrencyByTicker("EUR"),
        startDate: new Date(),
      },
    ],
  ...
  expect(
    calculate(state, {
      value: 100000000,
      from: (await tokensById["ethereum/erc20/dai_stablecoin_v2_0"])!,
      to: getFiatCurrencyByTicker("EUR"),
    }),
  ).toBeUndefined();
});
