 ...
 */
export function makeGetAccountShape(evmAPI: EvmAPI): GetAccountShape {
  return async (infos) => {
    const { initialAccount, address, derivationMode, currency } = infos;
    const { blockHeight, balance } = await getBalanceAndBlock(
      currency,
      address
    );
    ...
    const lastCoinOperations = await (async () => {
      try {
        const { getLastCoinOperations } = evmAPI.getExplorerApi(currency);
    ...
    const newSubAccounts = await getSubAccounts(evmAPI)(
      infos,
      accountId,
    ...
    const confirmedOperations = await Promise.all(
      confirmPendingOperations
    ).then((ops) => ops.filter((op): op is Operation => !!op));
 ...
 */
export function getSubAccounts(evmAPI: EvmAPI) {
  return async (
    ...
    const lastERC20OperationsAndCurrencies = await (async () => {
      try {
        const { getLastTokenOperations } = evmAPI.getExplorerApi(currency);
