 ...
 */
export const prepareCoinTransaction =
  (evmAPI: EvmAPI) =>
  async (
    account: Account,
    ...
    if (typedTransaction.useAllAmount) {
      ...
      const additionalFees = await getAdditionalLayer2Fees(evmAPI)(
        account.currency,
        draftTransaction
      );
 ...
 */
export const prepareTokenTransaction =
  (evmAPI: EvmAPI) =>
  async (
    account: Account,
...
export const prepareTransaction =
  ...
  async (
    ...
    return isTokenTransaction
      ? await prepareTokenTransaction(evmAPI)(
          account,
          subAccount,
          typedTransaction
        )
      : await prepareCoinTransaction(evmAPI)(account, typedTransaction);
  };
