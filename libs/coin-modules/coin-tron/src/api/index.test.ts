...
describe("createApi", () => {
  ...
  it("should resolve the coin config from the context and thread it down", async () => {
    ...
    expect(getBalance).toHaveBeenCalledWith("address", mockTronConfig);
  });
  ...
  it("should pass parameters correctly", async () => {
    ...
    expect(broadcast).toHaveBeenCalledWith("transaction", mockTronConfig);
    expect(combine).toHaveBeenCalledWith("tx", "signature");
    expect(estimateFees).toHaveBeenCalledWith(intent, mockTronConfig);
    expect(craftTransaction).toHaveBeenCalledWith(intent, undefined, mockTronConfig);
    expect(getBalance).toHaveBeenCalledWith("address", mockTronConfig);
    expect(getAccountInfo).toHaveBeenCalledWith("address", mockTronConfig);
