...
describe("estimateFees [integ]", () => {
  ...
  describe("Super Representative sender (bandwidth + energy fully covered)", () => {
    describe("native", () => {
      ...
      it("to a fresh recipient costs exactly createAccountFee + createNewAccountFeeInSystemContract", async () => {
        ...
        expect(fee).toBe(BigInt(params.createAccountFee + params.createNewAccountFeeInSystemContract));
      });
    });
  ...
  describe("fresh sender (no bandwidth, no energy)", () => {
    describe("native", () => {
      it("to an active recipient charges only the bandwidth fee (~0.27 TRX)", async () => {
        const fee = await estimateFees(
          sendIntent({ sender: freshAddress(), recipient: ACTIVE_RECIPIENT }),
        );

        // ~270 bytes × 1000 sun/byte; allow ±30% for tronweb tx-size variations.
        expect(fee).toBeGreaterThanOrEqual(BigInt(200_000));
        expect(fee).toBeLessThanOrEqual(BigInt(400_000));
      });
    });

    describe("TRC20", () => {
      it("to an active recipient charges the full energy fee", async () => {
        const fee = await estimateFees(
          sendIntent({
            sender: freshAddress(),
            recipient: ACTIVE_RECIPIENT,
        ...
        // No frozen energy → bandwidth + full energy ≈ several TRX.
        expect(fee).toBeGreaterThan(BigInt(ONE_TRX.toFixed()));
      });
    });
  });
});
