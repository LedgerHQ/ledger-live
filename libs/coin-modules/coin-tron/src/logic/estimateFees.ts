...
export const estimateEnergy = async (intent: TronIntent): Promise<number> => {
  ...
  if (intent.useAllAmount) {
    simulatedAmount = await tokenBalance(intent.sender, intent.asset.assetReference);
  }
...
async function tokenBalance(sender: string, assetReference: string): Promise<BigNumber> {
...
export async function estimateFees(transactionIntent: TronIntent): Promise<FeeEstimation> {
  ...
  try {
    ...
    try {
      energyRequired = new BigNumber(await estimateEnergy(transactionIntent)).integerValue(
