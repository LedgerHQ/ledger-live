 ...
 * successful read). Exhausting that budget is surfaced as EnergyDelegationTimeoutError (with the last
 * error as its `cause`) — the delivery is uncertain, so the caller must reconcile on-chain rather
 * than treat it as a definitive failure. An explicit "failed" status is a verdict, not a blip, and
...
export async function awaitEnergyDeliveryWith(
  ...
  for (;;) {
    // Stop polling the moment the caller abandons the flow (reset/unmount): its result is discarded
    // anyway, so continuing would just burn network calls until the timeout. Abort is distinct from
    // the timeout so the caller skips the on-chain reconciliation a timeout triggers.
    ...
    try {
      ...
      // Read exhaustion is uncertain delivery, not a definitive failure: the paid rental may already
      // have delivered energy the failing reads couldn't see. Throw the timeout error (raw cause
      // preserved) so the caller's final on-chain reconciliation runs — treating it as a definitive
      // failure would re-craft on retry and charge a second rental (LIVE-32780 AC2).
      if (++consecutiveErrors >= maxConsecutiveErrors) throw timedOut(error);
