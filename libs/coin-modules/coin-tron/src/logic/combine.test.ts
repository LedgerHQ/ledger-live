import { combine, recoverDeviceSignature, TX_LEN_PREFIX_HEX_WIDTH } from "./combine";
import { decodeTransaction } from "./utils";

describe("combine", () => {
  it("combines in a local way the rawTx and its signature (so it can be used easily in the broadcast)", () => {
    // GIVEN
    const rawTx =
      "";
    const signature = "0B7E480C202D77F02E84C4E86A4CEF2D44623E670F455558C6FA8F09F5715E66";

    // WHEN
    const result = combine(rawTx, [signature]);

    // THEN
    const txLength = parseInt(result.slice(0, TX_LEN_PREFIX_HEX_WIDTH), 16);
    expect(txLength).toEqual(rawTx.length);
    expect(result.slice(TX_LEN_PREFIX_HEX_WIDTH, txLength + TX_LEN_PREFIX_HEX_WIDTH)).toEqual(
      rawTx,
    );
    expect(result.slice(TX_LEN_PREFIX_HEX_WIDTH + txLength).length).toEqual(64);
    expect(result.slice(TX_LEN_PREFIX_HEX_WIDTH + txLength)).toEqual(signature);
  });

  it("throws when the tx is too long to fit the length prefix", () => {
    const signature = "0B7E480C202D77F02E84C4E86A4CEF2D44623E670F455558C6FA8F09F5715E66";
    // 0x10000 hex chars — one past what the 4-digit prefix can encode; would otherwise mis-slice on recovery.
    const oversizedTx = "0".repeat(0x10000);

    expect(() => combine(oversizedTx, [signature])).toThrow(/too long to length-prefix/);
  });
});

describe("recoverDeviceSignature", () => {
  it("recovers the raw device signature from a combined string (inverse of combine)", () => {
    const rawTx =
      "";
    const signature = "0B7E480C202D77F02E84C4E86A4CEF2D44623E670F455558C6FA8F09F5715E66";

    expect(recoverDeviceSignature(rawTx, combine(rawTx, [signature]))).toEqual(signature);
  });
});

describe("decodeTransaction", () => {
  it("creates a transaction in TronWeb format", async () => {
    const result = await decodeTransaction(
      "",
    );

    expect(result).toEqual({
      // visible: true,
      txID: "477eaebbf508f7e2030f52917d34d264cfef5764d7f73eda582708e5762ceb25",
      raw_data: {
        contract: [
          {
            parameter: {
              value: {
                amount: 1000,
                owner_address: "41fd49eda0f23ff7ec1d03b52c3a45991c24cd440e",
                to_address: "4198927ffb9f554dc4a453c64b2e553a02d6df514b",
              },
              type_url: "type.googleapis.com/protocol.TransferContract",
            },
            type: "TransferContract",
          },
        ],
        ref_block_bytes: "0ee5",
        ref_block_hash: "2e5fc67747a428af",
        expiration: 1740477510000,
        timestamp: 1740477451901,
      },
      raw_data_hex:
        "",
    });
  });
});
