...
const mockFetchTronContract = jest.fn();
...
jest.mock("../network", () => ({
  ...
  fetchTronContract: (...args: unknown[]) => mockFetchTronContract(...args),
...
describe("validateIntent", () => {
  beforeEach(() => {
    ...
    // An activated, non-contract recipient: the default for a plain send.
  ...
  it("still estimates when customFees carry no resource figures", async () => {
  ...
  it("does not crash on a TRC-20 send whose customFees carry no breakdown", async () => {
    ...
    const result = await validateIntent(intent, [nativeBalance(10_000_000n)], {
      value: 500_000n,
      parameters: { feesStrategy: "medium" },
    });
  ...
  describe("token amount validation", () => {
    ...
    it("rejects a TRC20 send to an unactivated, non-contract recipient", async () => {
    ...
    it("allows a TRC20 send to a smart contract that has no account entry", async () => {
      mockFetchTronAccount.mockResolvedValue([]);
      mockFetchTronContract.mockResolvedValue({ contract_address: RECIPIENT });
  ...
  describe("staking modes", () => {
    ...
    it("reports amount 0 and no fee spend for a non-spending mode", async () => {
      ...
      const result = await validateIntent(makeStakingIntent("claimReward"), [
        nativeBalance(10_000_000n),
      ]);
    ...
    describe("withdrawExpireUnfreeze", () => {
      it("rejects when nothing is unfreezing", async () => {
        const result = await validateIntent(makeStakingIntent("withdrawExpireUnfreeze"), [
          nativeBalance(10_000_000n),
        ]);
    ...
    describe("vote", () => {
      ...
      it("rejects an empty vote list", async () => {
        ...
        const result = await validateIntent(makeStakingIntent("vote", { votes: [] }), [
          nativeBalance(10_000_000n),
        ]);
