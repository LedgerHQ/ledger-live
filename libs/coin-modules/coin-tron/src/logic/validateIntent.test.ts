...
const mockNetworkValidateAddress = jest.fn();
jest.mock("../network", () => ({
  validateAddress: (...args: unknown[]) => mockNetworkValidateAddress(...args),
}));
...
describe("validateIntent", () => {
  beforeEach(() => {
    ...
    mockNetworkValidateAddress.mockResolvedValue(true);
  });
  ...
  describe("recipient validation", () => {
    ...
    it("rejects a malformed recipient address", async () => {
      mockNetworkValidateAddress.mockResolvedValueOnce(false);
  ...
  describe("native amount validation", () => {
    it("rejects a zero amount when not useAllAmount", async () => {
      const result = await validateIntent(makeIntent({ amount: 0n }), [
        nativeBalance(10_000_000n),
      ]);
    ...
    it("subtracts locked balance from available", async () => {
      const balances: Balance[] = [{ value: 10_000_000n, locked: 9_000_000n, asset: { type: "native" } }];
    ...
    it("clamps useAllAmount to 0 when fees exceed balance", async () => {
      const result = await validateIntent(
        makeIntent({ amount: 0n, useAllAmount: true }),
  ...
  describe("token amount validation", () => {
    ...
    it("rejects when native balance cannot cover the token transfer fees", async () => {
      const result = await validateIntent(tokenIntent, [
        nativeBalance(100n),
        trc20Balance(5_000_000n),
      ]);
