...
// raw_data_hex of 540 hex chars = 270 bytes (typical native send size).
const rawTx270Bytes = { transaction: "00".repeat(270) };
const rawTx285Bytes = { transaction: "00".repeat(285) };
const rawTx350Bytes = { transaction: "00".repeat(350) };
...
describe("estimateFees", () => {
  beforeEach(() => {
    ...
    mockCraftTransaction.mockResolvedValue(rawTx270Bytes);
  });
  ...
  describe("trc10 send", () => {
    beforeEach(() => {
      mockCraftTransaction.mockResolvedValue(rawTx285Bytes);
    });
  ...
  describe("trc20 send", () => {
    beforeEach(() => {
      mockCraftTransaction.mockResolvedValue(rawTx350Bytes);
    });
  ...
  describe("fallback", () => {
    it("returns the activation-fee worst case when network fails for native send", async () => {
      mockGetTronAccountNetwork.mockRejectedValue(new Error("network down"));

      const result = await estimateFees(sendNative);

      expect(result).toBe(BigInt(ACTIVATION_FEES.toString()));
    });
