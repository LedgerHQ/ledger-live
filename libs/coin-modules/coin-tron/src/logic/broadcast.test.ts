...
describe("broadcast function", () => {
  ...
  it("should broadcast a transaction string as a byte-preserving full transaction hex", async () => {
    ...
    const result = await broadcast(mockLogger, mockConfig, "0008abcd1234aabbccdd");

    // Assembled Transaction protobuf: field 1 (raw_data) = 0x0a 0x04 + abcd1234,
    // field 2 (signature) = 0x12 0x04 + aabbccdd.
    expect(mockBroadcastHexTron).toHaveBeenCalledWith(
      mockLogger,
      mockConfig,
      "0a04abcd12341204aabbccdd",
    );
  ...
  it("encodes multi-byte varint lengths for raw_data >= 128 bytes", async () => {
    ...
    await broadcast(mockLogger, mockConfig, `${lengthPrefix}${rawTx}${signature}`);

    // field 1 (raw_data): 0x0a + varint(130)=0x8201 ; field 2 (signature): 0x12 + varint(4)=0x04
    expect(mockBroadcastHexTron).toHaveBeenCalledWith(
      mockLogger,
      mockConfig,
      `0a8201${rawTx}1204${signature}`,
    );
  });
  ...
  it("should broadcast a TxObject successfully", async () => {
    ...
    const result = await broadcast(mockLogger, mockConfig, txObject);

    expect(mockBroadcastHexTron).not.toHaveBeenCalled();
    expect(mockBroadcastTron).toHaveBeenCalledWith(mockLogger, mockConfig, txObject);
    expect(result).toBe("mockedTxID");
  });
  ...
  it("should throw an error if broadcastHexTron fails", async () => {
    ...
    await expect(broadcast(mockLogger, mockConfig, "0008abcd1234aabbccdd")).rejects.toThrow(
      "Broadcasting failed",
    );
  });
  ...
  it("should throw on a malformed signed transaction string", async () => {
    ...
    await expect(broadcast(mockLogger, mockConfig, "zzzz")).rejects.toThrow(/malformed/);
    await expect(broadcast(mockLogger, mockConfig, "0008abcd")).rejects.toThrow(/malformed/);
    expect(mockBroadcastHexTron).not.toHaveBeenCalled();
  });
});
