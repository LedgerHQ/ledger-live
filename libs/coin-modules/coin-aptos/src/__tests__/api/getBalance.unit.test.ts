...
describe("getBalance", () => {
  ...
  beforeEach(() => {
    ...
    mockGetBalances = jest.fn();
    mockGetBalances = mockGetBalances = jest
      .fn()
      .mockImplementation(async (_address: string) =>
        Promise.resolve([{ asset_type: APTOS_ASSET_ID, amount: BigNumber(10) }]),
      );
