...
function checkWriteSets(tx: AptosTransaction, event: AptosEvent, event_name: string): boolean {
  return tx.changes.some(change => {
    return isChangeOfAptos(change, event, event_name);
  });
}

export function isChangeOfAptos(writeSetChange: WriteSetChange, event: AptosEvent, event_name: string): boolean {
  ...
  const mr = writeSetChange.data as MoveResource<{
