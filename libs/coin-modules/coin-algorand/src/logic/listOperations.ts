...
export async function listOperations(
  ...
  const { minHeight, order } = options;

  const transactions = await getAllAccountTransactions(address, minHeight);
  ...
  // Return operations with empty next token (all fetched at once)
  return [operations, ""];
}
