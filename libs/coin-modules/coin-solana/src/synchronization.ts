...
> {
  ...
  const signatures = await fetch(api.config.endpoint, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
    body: JSON.stringify(
      accounts.map(({ associatedTokenAccount, knownTokenAccount }) => ({
        jsonrpc: "2.0",
  ...
  const signaturesJSON: Array<{
    result: ConfirmedSignatureInfo[];
    id: `${string}_${string}_${number}`;
...
export const getAccountShapeWithAPI = async (
  ...
  const mintInfos = (await api.getMultipleAccounts(mints))
    .map(account =>
