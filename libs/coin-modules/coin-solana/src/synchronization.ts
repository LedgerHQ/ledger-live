...
import { epochOnce, toSolanaTokenAccExtensions } from "./logic/tokenAccountShapes";
...
async function newSubAcc({
  ...
  const extensions =
    mintExtensions || assocTokenAcc.info.extensions
      ? await toSolanaTokenAccExtensions(api, assocTokenAcc, epochOnce(api), mintExtensions)
      : undefined;
