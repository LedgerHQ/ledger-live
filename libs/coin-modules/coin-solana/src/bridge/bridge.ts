...
function makeBroadcast(
  getChainAPI: (config: Config) => Promise<ChainAPI>,
): AccountBridge<Transaction, TransactionStatus, SolanaAccount>["broadcast"] {
  return async info => {
    const config: Config = {
...
): AccountBridge<Transaction, TransactionStatus, SolanaAccount>["signOperation"] {
  return info => {
    const config: Config = {
      endpoint: endpointByCurrencyId(info.account.currency.id),
    };
...
export function makeBridges({
  ...
  accountBridge: AccountBridge<Transaction, TransactionStatus, SolanaAccount>;
} {
  const getAddress = resolver(signerContext);
  const { sync, scan } = makeSyncAndScan(getQueuedAPI, getAddress);

  const accountBridge: AccountBridge<Transaction, TransactionStatus, SolanaAccount> = {
    createTransaction,
