...
export const getNftMetadata = async (
  ...
  const { data }: { data: NFTMetadataResponse[] } = await network({
    method: "POST",
    url: `https://nft.api.live.ledger.com/v2/solana/${params.chainId}/contracts/tokens/infos`,
    data: input,
  });
...
export const getNftCollectionMetadata = async (
  ...
  const { data }: { data: NFTCollectionMetadataResponse[] } = await network({
    method: "POST",
    url: `https://nft.api.live.ledger.com/v2/solana/${params.chainId}/contracts/infos`,
    data: input,
  });
