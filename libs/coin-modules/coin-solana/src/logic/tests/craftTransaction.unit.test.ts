...
describe("craftTransaction – token edge cases", () => {
  ...
  it("surfaces a recipient account whose data cannot be parsed", async () => {
    ...
    // Only the recipient's own account is unparseable; its derived ATA simply does not exist, so
    // without the guard the crafting would silently target that derived address instead.
    mockedGetMaybeTokenAccount.mockImplementation(async (address: string) =>
      address === recipient ? new Error("unparseable token account") : undefined,
    );
    mockedBuildTokenTransferInstructions.mockResolvedValue([]);
    ...
    ).rejects.toThrow("unparseable token account");
  });
