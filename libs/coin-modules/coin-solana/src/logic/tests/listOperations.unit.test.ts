...
describe("listOperations", () => {
  ...
  it("keeps a native operation whose signature the owner and an ATA both report, whatever the source order", async () => {
    const ata = ataAddressOf(USDC_MINT_FIXTURE);
    // The cursor decides the order the sources are asked in; put the ATA first.
    const cursor = Buffer.from(
      JSON.stringify({ [ata]: "ataSig", [TEST_ADDRESS]: "ownerSig" }),
      "utf8",
    ).toString("base64");
    mockGetSignaturesForAddress.mockResolvedValue([
      { signature: "shared", slot: 100, blockTime: 1700000000, err: null },
    ]);
    mockGetParsedTransactions.mockResolvedValue([
      {
        ...
        meta: { fee: 5000, preBalances: [1_000_000], postBalances: [994_000], err: null },
      },
    ] as never);
    ...
    expect(result.items.map(op => op.type)).toContain("OUT");
  });

  it("does not fetch a transaction already below minHeight", async () => {
    mockGetSignaturesForAddress.mockResolvedValue([
      { signature: "old", slot: 50, blockTime: 1700000000, err: null },
    ]);
    mockGetParsedTransactions.mockResolvedValue([]);
  ...
  it("keeps the cursor of a source whose page came back empty while another progressed", async () => {
    const ata = ataAddressOf(USDC_MINT_FIXTURE);
    const cursor = Buffer.from(
      JSON.stringify({ [TEST_ADDRESS]: "ownerSig", [ata]: "ataSig" }),
      "utf8",
    ).toString("base64");
    mockGetSignaturesForAddress.mockImplementation(async address =>
    ...
    expect(JSON.parse(Buffer.from(result.next as string, "base64").toString("utf8"))).toEqual({
      [TEST_ADDRESS]: "sig1",
      [ata]: "ataSig",
    });
  });
  ...
  describe("account operations beyond transfers", () => {
    ...
    it.each([
      ["spl-associated-token-account", "create", "OPT_OUT"],
      ["spl-associated-token-account", "createIdempotent", "OPT_OUT"],
  ...
  describe("multi-source paging", () => {
    ...
    it("keeps a token-account signature the owner's own stream never returned", async () => {
      const ata = ataAddressOf(USDC_MINT_FIXTURE);
      mockGetParsedTokenAccountsByOwner.mockResolvedValue({
        value: [ataFixture(USDC_MINT_FIXTURE)],
      });
      const onlyOnAta = { signature: "ataOnly", slot: 100, blockTime: 1700000000, err: null };
      mockGetSignaturesForAddress.mockImplementation(async (source: string) =>
        source === ata ? [onlyOnAta] : [],
      );
      mockGetParsedTransactions.mockResolvedValue([
        {
          transaction: {
