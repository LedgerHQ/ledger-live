...
describe("validateIntent", () => {
  describe("partner-built transaction", () => {
    const prebuilt = {
      ...makeIntent({ recipient: "", amount: 0n }),
      data: { type: "buffer", value: Buffer.from("AQID", "base64") },
    } as IntentArg;
  ...
  describe("staking intents", () => {
    describe("stake.createAccount", () => {
      ...
      it("should validate a correct stake.createAccount intent", async () => {
        ...
        expect(result.errors).toEqual({});
        expect(result.amount).toBe(1_000_000_000n);
        expect(result.totalSpent).toBe(1_000_000_000n + 5000n + BigInt(STAKE_ACC_RENT_EXEMPT));
      });

      it("leaves the rent inside the amount when sending all", async () => {
        const result = await validateIntent(
          makeStakeIntent({ useAllAmount: true } as Partial<StakingTransactionIntent>),
          makeBalances(),
          { value: 5000n },
        );

        expect(result.totalSpent).toBe(result.amount + 5000n);
      });
      ...
      it("still validates an intent carrying the stake account seed", async () => {
        const result = await validateIntent(
          makeStakeIntent({
            ...
            data: { type: "stakeAccountSeed", value: "seed" },
    ...
    describe("checks the legacy bridge used to run", () => {
      ...
      it("delegates a stake that never had a validator", async () => {
        const balances = makeStakeBalances({ state: "deactivating" });
        delete (balances[balances.length - 1].stake as Partial<Stake>).delegate;

        const result = await validateIntent(delegateIntent(), balances, { value: 5000n });
  ...
  describe("token transfers", () => {
    ...
    describe("recipient checks the legacy bridge used to run", () => {
      ...
      it("refuses to send from a frozen token account", async () => {
        mockedGetMaybeTokenMint.mockResolvedValue({
          onChainAcc: { data: { program: "spl-token" } },
          info: { decimals: 6 },
        } as unknown as Awaited<ReturnType<typeof getMaybeTokenMint>>);
        mockedGetMaybeTokenAccount.mockImplementation(async (address: string) =>
          address === "sender-ata" ? ({ state: "frozen" } as never) : undefined,
        );
        ...
        expect(result.errors.amount).toBeInstanceOf(SolanaTokenAccountFrozen);
      });
  ...
  describe("restored legacy checks", () => {
    it("requires an amount to split a stake account", async () => {
      const result = await validateIntent(
        {
          intentType: "staking",
          type: "stake.split",
    ...
    it("refuses to split a stake account the sender cannot stake", async () => {
      const result = await validateIntent(
        {
          intentType: "staking",
          type: "stake.split",
    ...
    it("refuses to split when only the unstake reserve could pay the fee", async () => {
      const balances = makeStakeBalances();
      balances[0] = { value: 1_000_010_000n, asset: { type: "native" }, locked: 1_000_008_000n };
    ...
    const splMint = (decimals = 6) =>
      ({ info: { decimals }, onChainAcc: { data: { program: "spl-token" } } }) as never;

    const authorityIntent = (type: string, recipient = RECIPIENT) =>
      makeIntent({
        type,
        recipient,
        asset: { type: "spl-token", name: "USDC", assetReference: SENDER },
      } as never) as IntentArg;

    it("refuses to open a token account that already exists", async () => {
      mockedGetMaybeTokenMint.mockResolvedValue(splMint());
      mockedGetMaybeTokenAccount.mockResolvedValue({ state: "initialized" } as never);

      const result = await validateIntent(authorityIntent("token.createATA"), makeBalances(), {
        value: 5000n,
      });

      expect(result.errors.amount).toBeInstanceOf(SolanaTokenAccountNotAllowed);
    });

    it("refuses to revoke against a token account that does not exist", async () => {
      mockedGetMaybeTokenMint.mockResolvedValue(splMint());
      mockedGetMaybeTokenAccount.mockResolvedValue(undefined as never);

      const result = await validateIntent(authorityIntent("token.revoke"), makeBalances(), {
        value: 5000n,
      });

      expect(result.errors.amount).toBeInstanceOf(SolanaTokenAccounNotInitialized);
    });

    it("reports an empty approval recipient instead of throwing on it", async () => {
      mockedGetMaybeTokenMint.mockResolvedValue(splMint());
      mockedGetMaybeTokenAccount.mockImplementation(async address => {
        if (!address) throw new Error("Invalid public key input");
        return { state: "initialized" } as never;
      });

      const result = await validateIntent(authorityIntent("token.approve", ""), makeBalances(), {
        value: 5000n,
      });
    ...
    it("refuses an approval whose delegate is the sender's own token account", async () => {
      mockedGetMaybeTokenMint.mockResolvedValue(splMint());
      mockedGetMaybeTokenAccount.mockResolvedValue({ state: "initialized" } as never);
      const ownerAta = "4Nd1mBQtrMJVYVfKf2PJy9NZUZdTAsp7D4xWLs4gDB4T";
      const api = {
        ...makeTransferApi(),
        findAssocTokenAccAddress: jest.fn().mockResolvedValue(ownerAta),
      } as unknown as ChainAPI;

      const result = await validateIntent(
        authorityIntent("token.approve", ownerAta),
        makeBalances(),
        { value: 5000n },
        api,
      );
    ...
    it("refuses a max stake creation that would leave nothing to stake", async () => {
      const result = await validateIntent(
        {
          intentType: "staking",
          type: "stake.createAccount",
        ...
        [{ value: 3_000_000_000n, asset: { type: "native" }, locked: 3_000_000_000n }],
        { value: 5000n },
      );
    ...
    it("refuses a delegation paid out of the unstake reserve", async () => {
      const result = await validateIntent(
        {
          intentType: "staking",
          type: "stake.delegate",
        ...
        makeStakeBalances({}, 1_000_000_000n).map(b =>
          b.stake ? b : { ...b, locked: 1_000_000_000n },
        ),
        { value: 5000n },
      );
    ...
    it("refuses a token authority command when every lamport is locked", async () => {
      const result = await validateIntent(
        makeIntent({
          type: "token.revoke",
          asset: { type: "spl-token", name: "USDC", assetReference: RECIPIENT } as never,
        }) as IntentArg,
        [{ value: 3_000_000_000n, asset: { type: "native" }, locked: 3_000_000_000n }],
        { value: 5000n },
      );
    ...
    it("refuses to revoke against a frozen token account", async () => {
      mockedGetMaybeTokenMint.mockResolvedValue({
        info: { decimals: 6 },
        onChainAcc: { data: { program: "spl-token" } },
      } as never);
      mockedGetMaybeTokenAccount.mockResolvedValue({ state: "frozen" } as never);
    ...
    it("refuses a recipient whose derived token account address is funded but not a token account", async () => {
      mockedGetMaybeTokenMint.mockResolvedValue({
        info: { decimals: 6 },
        onChainAcc: { data: { program: "spl-token" } },
      } as never);
      mockedGetMaybeTokenAccount.mockImplementation(async (address: string) =>
        address === "sender-ata" ? ({ state: "initialized" } as never) : undefined,
      );
      const api = {
        ...makeTransferApi(),
        getBalance: jest.fn().mockResolvedValue(2_039_280),
        findAssocTokenAccAddress: jest.fn(async (owner: string) =>
          owner === SENDER ? "sender-ata" : "occupied-ata",
        ),
      } as unknown as ChainAPI;

      const result = await validateIntent(
        makeIntent({
          asset: { type: "spl-token", name: "USDC", assetReference: SENDER } as never,
        }) as IntentArg,
        [...makeBalances(), { value: 100n, asset: { type: "spl-token" } as never }],
        { value: 5000n },
        api,
      );

      expect(result.errors.recipient).toBeInstanceOf(SolanaTokenAccounNotInitialized);
    });

    it("counts a Token-2022 transfer fee against the token balance", async () => {
      const asset = { type: "spl-token-2022", name: "TSLAx", assetReference: RECIPIENT } as never;
      const balances: Balance[] = [...makeBalances(), { value: 100n, asset }];

      const result = await validateIntent(
        makeIntent({ amount: 99n, asset }) as IntentArg,
        balances,
        {
          value: 5000n,
          parameters: {
            transferFee: { feeBps: 100, transferAmountIncludingFee: 110 },
          },
        } as FeesArg,
      );
