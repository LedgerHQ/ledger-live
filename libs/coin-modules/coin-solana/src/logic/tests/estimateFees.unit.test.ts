...
jest.mock("../craftTransaction", () => ({
  ...
  buildTransactionWithAPI: jest.fn(),
}));
...
const { buildTransactionWithAPI } = jest.requireMock("../craftTransaction");
...
function setupBuildMock(blockhash = "oldBlockhash") {
  ...
  buildTransactionWithAPI.mockResolvedValue([mockOnChainTx, {}, jest.fn()]);
...
describe("estimateFees", () => {
  ...
  it("should map token intent to token.transfer kind", async () => {
    ...
    const tx = buildTransactionWithAPI.mock.calls[0][1];
    expect(tx.model.kind).toBe("token.transfer");
  });

  it("should propagate errors from estimateTxFee", async () => {
    buildTransactionWithAPI.mockRejectedValueOnce(new Error("RPC error"));
