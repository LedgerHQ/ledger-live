...
export async function listOperations(
  ...
  for (;;) {
    ...
    for (const { source, signatures: sourceSignatures } of perSource) {
      ...
      // A source that came with a cursor keeps it, so a tolerated per-entry RPC error cannot
      // silently drop it from the walk and lose its older history.
      if (
        !before[source] &&
        covered === sourceSignatures.length &&
...
function detectAccountOperation(
  ...
  switch (ix.program) {
    case PARSED_PROGRAMS.SPL_ASSOCIATED_TOKEN_ACCOUNT:
      ...
      return ix.type === "create" || ix.type === "createIdempotent"
        ? isFeePayer
          ? "OPT_OUT"
          : "OPT_IN"
        : undefined;
