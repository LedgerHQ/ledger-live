...
export async function listOperations(
  ...
  for (;;) {
    ...
    const ownerSignatures = new Set(
      (perSource.find(entry => entry.source === address)?.signatures ?? []).map(
    ...
    for (const { source, signatures: sourceSignatures } of perSource) {
      for (const signature of sourceSignatures) {
        ...
        if (!ownerSignatures.has(signature.signature)) {
          fromTokenStreamOnly.add(signature.signature);
        }
    ...
    // Before fetching: a signature already below `minHeight` would be downloaded only to be dropped.
    const signatures = page
      .map(entry => entry.signature)
      .filter(signature => !(minHeight > 0 && signature.slot < minHeight));
    ...
    for (const { source, signatures: sourceSignatures } of perSource) {
      ...
      // Presence, not truthiness: a source carried forward with no cursor yet holds `""`, and must
      // stay in the walk rather than be read as never having been asked.
      if (
        !(source in before) &&
        covered === sourceSignatures.length &&
        sourceSignatures.length < rpcLimit
      ) {
        continue;
      }
...
/** Canonical associated accounts only: `getBalance` reports no others. */
 ...
 */
/** The RPC prefixes a memo with its byte length (`[5] hello`), counted in bytes, not UTF-16 units. */
...
function detectAccountOperation(
  ...
  switch (ix.program) {
    case PARSED_PROGRAMS.SPL_ASSOCIATED_TOKEN_ACCOUNT: {
      // The program emits `create`, and newer versions `createIdempotent`.
      const creates = ix.type === "create" || ix.type === "createIdempotent";
...
/** With a `mint`, a multi-mint transaction keeps each operation's parties to its own asset. */
...
function computeTokenBalanceDeltas(
  ...
  // Summed per mint: moving tokens between two of the wallet's own accounts nets to zero.
