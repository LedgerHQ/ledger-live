...
/** At most one `getEpochInfo` per call, and none unless a transfer-fee extension asks for it. */
...
export async function getTokenAccountShapes(
  ...
  const extensionsByMint = await readMintExtensions(
    api,
    token2022Accounts.map(account => toTokenAccountWithInfo(account).info.mint.toBase58()),
  );
