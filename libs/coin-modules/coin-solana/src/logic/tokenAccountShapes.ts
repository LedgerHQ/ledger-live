...
import { tryParseAsMintAccount } from "../network/chain/account/parser";
import { getTokenAccruedInterestDelta, toTokenAccountWithInfo } from "../network/chain/web3";
...
function epochOnce(api: ChainAPI): () => Promise<number> {
  let pending: Promise<number> | undefined;
  return () => (pending ??= api.getEpochInfo().then(({ epoch }) => epoch));
}
...
export async function getTokenAccountShapes(
  ...
  ].flatMap(({ account, tokenProgram }) => {
    const assocTokenAcc = toTokenAccountWithInfo(account);
    const associatedAddress = getAssociatedTokenAddressSync(
      assocTokenAcc.info.mint,
      owner,
      undefined,
      getTokenAccountProgramId(tokenProgram),
    );
    return associatedAddress.equals(account.pubkey) ? [{ assocTokenAcc, tokenProgram }] : [];
  });

  const extensionsByMint = await readMintExtensions(
    api,
    entries.flatMap(({ assocTokenAcc, tokenProgram }) =>
      tokenProgram === PARSED_PROGRAMS.SPL_TOKEN_2022 ? [assocTokenAcc.info.mint.toBase58()] : [],
    ),
  );

  await Promise.all(
    entries.map(async ({ assocTokenAcc, tokenProgram }) => {
      const { info } = assocTokenAcc;
      const mint = info.mint.toBase58();

      const mintExtensions =
        tokenProgram === PARSED_PROGRAMS.SPL_TOKEN_2022 ? extensionsByMint.get(mint) : undefined;

      shapes[mint] = {
        ...(info.state ? { state: info.state } : {}),
        ...(mintExtensions || info.extensions
          ? {
              extensions: await toSolanaTokenAccExtensions(
                api,
                assocTokenAcc,
                mintExtensions,
                getEpoch,
...
/** Solana caps `getMultipleAccounts` at 100 addresses. */
const MINT_BATCH_SIZE = 100;

async function readMintExtensions(
  api: ChainAPI,
  mints: string[],
): Promise<Map<string, MintExtensions | undefined>> {
  const byMint = new Map<string, MintExtensions | undefined>();
  const unique = [...new Set(mints)];
  if (unique.length === 0) return byMint;

  for (const batch of chunk(unique, MINT_BATCH_SIZE)) {
    const accounts = await api.getMultipleAccounts(batch);
    batch.forEach((mint, index) => {
      const account = accounts[index];
      if (!account || !("parsed" in account.data)) return;
      const parsed = tryParseAsMintAccount(account.data);
      if (!parsed || parsed instanceof Error) return;
      if (account.data.program !== PARSED_PROGRAMS.SPL_TOKEN_2022) return;
      byMint.set(mint, parsed.extensions);
    });
  }
...
async function toSolanaTokenAccExtensions(
  ...
  getEpoch: () => Promise<number> = epochOnce(api),
) {
