...
export async function validateIntent(
  api: ChainAPI,
  transactionIntent: TransactionIntent<StringMemo | MemoNotSupported, SolanaTransactionIntentData>,
  balances: Balance[],
  ...
  if ("data" in transactionIntent && transactionIntent.data.type === "buffer") {
...
async function validateTokenTransfer(
  ...
  const [senderAccount, recipientOrError] = await Promise.all([
    getMaybeTokenAccount(senderAta, api).then(asAccountOrUndefined),
    getTokenRecipient(intent.recipient, mintAddress, tokenProgram, api),
  ]);

  // Absent is not an error: the legacy path skips the initialized check on a synced sub-account.
  const senderStateError = senderAccount && validateAssociatedTokenAccountState(senderAccount);
  if (senderStateError) {
    errors.amount = senderStateError;
    return;
  }
...
async function validateOwnerTokenAccountState(
  ...
  // The create instruction is not idempotent.
  ...
  // Only once the recipient passed the common checks: the lookup below throws on a bad address.
  if (intent.type === "token.approve" && !errors.recipient) {
...
async function validateStakingIntent(
  ...
  switch (intent.type) {
    case "stake.createAccount": {
      ...
      const created = await computeCreateAccountAmount(
        api,
        intent,
        available,
        estimatedFees,
      ...
      totalSpent = amount + estimatedFees + (intent.useAllAmount ? 0n : created.rent);
      break;
    }
    case "stake.delegate":
    ...
    case "stake.withdraw":
      ...
      break;
    case "stake.split": {
      const stake = resolveStakeAccount(intentMemo(intent) || intent.recipient, balances, errors);
      // Crafting signs the split with the sender as the authorized staker.
      if (stake && !stakeAuthority(stake).canStake) {
      ...
      amount = clampPositive(intent.amount);
      totalSpent = estimatedFees;
      // A split moves funds between stake accounts, so it releases no reserve to pay its own fee.
      validateFeeCoverage(estimatedFees, available, errors);
      break;
    }
    default:
...
async function computeCreateAccountAmount(
  ...
  estimatedFees: bigint,
  errors: Record<string, Error>,
): Promise<{ amount: bigint; rent: bigint }> {
  if (!intent.recipient) {
  ...
  const [reserve, rent] = await Promise.all([
    unstakeReserve(api, intent.sender),
    fetchStakeAccountRentExempt().then(value => value ?? 0n),
  ]);
  ...
  if (intent.useAllAmount) {
    ...
    if (!errors.recipient && !errors.amount && allAmount > 0n) {
      ...
      if (
        ...
        clampPositive(allAmount - rent) < stakeMinimumDelegation
      ) {
        errors.amount = amountTooLowError(stakeMinimumDelegation);
      }
    }
    return { amount: allAmount, rent };
  }
  if (intent.amount <= 0n) {
    errors.amount = new AmountRequired();
  } else if (intent.amount + estimatedFees + reserve + rent > available) {
    errors.amount = new NotEnoughBalance();
  ...
  return { amount: intent.amount, rent };
}
...
function validateAmount(
  ...
  if (isTokenTransfer) {
    const tokenBalance = findBalance(intent.asset, balances);
    // A Token-2022 transfer fee is debited on top, so the balance has to cover the gross amount.
...
> {
  ...
  if (recipientTokenAccount === undefined) {
    ...
    if (!shouldCreateAsAssociatedTokenAccount) {
      ...
      // Funded but not a token account: "no account" would let the flow reach a doomed transaction.
