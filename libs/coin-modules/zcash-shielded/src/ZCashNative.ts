...
export class ZCashNative {
  ...
  syncShielded(args: SyncShieldedArgs): Observable<ShieldedSyncResult> {
    return new Observable(subscriber => {
      // Tracked here so the teardown can abort an in-flight gRPC stream
      // immediately, without waiting for the next `stream.next()` to resolve.
      let aborted = false;
      let activeStream: NativeStream | null = null;
      const isCancelled = () => aborted || subscriber.closed;
      ...
      return () => {
        aborted = true;
        if (activeStream) {
          log(ZCASH_LOG_TYPE, "teardown — cancelling active stream");
          try {
            activeStream.cancel();
          } catch (err) {
  ...
  private async syncChunk(
    ...
    onStream: (stream: NativeStream) => void = () => {},
    ...
    // Expose the stream to the caller so a teardown can cancel it
    // immediately rather than waiting for the next `stream.next()` to resolve.
    onStream(stream);
