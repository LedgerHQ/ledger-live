...
describe("syncShielded", () => {
  ...
  test("returns the shielded balance", async () => {
    ...
    expect(steps[steps.length - 1]).toMatchObject({
      lastBlockProcessed: testAccount1.transactions[4].tx.height,
      processedBlocks: nBlocks,
      remainingBlocks: 0,
      syncState: "running",
      progress: 0,
  ...
  test("returns the shielded balance in batches of max size 3", async () => {
    ...
    expect(steps[steps.length - 1]).toMatchObject({
      lastBlockProcessed: lastBlockHeight,
      processedBlocks: nBlocks,
      remainingBlocks: 0,
      syncState: "running",
      progress: 0,
  ...
  test("retrieves and processes also the latest block if a new blocks became available while processing", async () => {
    ...
    expect(steps[steps.length - 1]).toMatchObject({
      processedBlocks: nBlocks + additionalLastBlocks,
      remainingBlocks: 0,
      lastBlockProcessed: lastBlockHeight + additionalLastBlocks,
      syncState: "running",
      progress: 0,
      estimatedTimeRemaining: { hours: 0, minutes: 0 },
      transactions: testAccount1.transactions.map(({ tx, decryptedData }) => ({
