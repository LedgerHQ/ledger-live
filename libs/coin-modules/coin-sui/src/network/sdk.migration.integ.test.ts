...
/** Active delegator with steady history; same one used in `getListOperations` integ tests. */
const ACTIVE_DELEGATOR = "0x33444cf803c690db96527cec67e3c9ab512596f4ba2d4eace43f0b4f716e0164";
...
type ShapeSpec =
  ...
  | { object: Record<string, ShapeSpec>; allowExtra?: boolean }
...
function assertSpec(value: unknown, spec: ShapeSpec, path: string): void {
  ...
  if ("object" in spec) {
    ...
    if (!spec.allowExtra) {
      const extra = Object.keys(obj).filter(k => !(k in spec.object));
      // Extras are tolerated by default — different transports surface different metadata.
      void extra;
    }
    return;
  }
...
describe("JSON-RPC vs GraphQL shape parity (live mainnet)", () => {
  ...
  describe("getAllBalancesCached", () => {
    const balanceItem: ShapeSpec = {
      object: {
        ...
        lockedBalance: { object: {}, allowExtra: true },
  ...
  describe("getOperations", () => {
    ...
    const opItem: ShapeSpec = {
      object: {
        ...
        extra: { object: {}, allowExtra: true },
      },
      allowExtra: true,
    };

    it("returns the same op shape on both transports; digest sets overlap", async () => {
      const accountId = `js:2:sui:${ACTIVE_DELEGATOR}:sui`;
  ...
  describe("getOperationExtra", () => {
    ...
    beforeAll(async () => {
      // Pick the newest finalised digest for the active delegator. Whether it's a
      ...
      if (ops.length === 0) throw new Error("Active delegator has no recent ops — refresh fixture");
    ...
    it("returns the same Record<string,string> shape on both transports", async () => {
      ...
      assertShapeBoth(rpc, gql, { object: {}, allowExtra: true }, "getOperationExtra");
  ...
  describe("getListOperations (alpaca cursor model)", () => {
    const alpacaOpItem: ShapeSpec = {
      object: {
        ...
        recipients: { array: "string" },
        value: "bigint",
        asset: { object: {}, allowExtra: true },
        tx: {
          object: {
            hash: "non-empty-string",
            block: { object: {}, allowExtra: true },
            fees: "bigint",
            date: "Date",
            failed: "boolean",
          },
    ...
    it("second page (cursor-driven): both transports return strictly older items than the cursor", async () => {
      ...
      if (!first.next) {
        throw new Error(
          "[sui-migration] fixture stale: ACTIVE_DELEGATOR has fewer than one page of history. " +
            "Refresh the fixture address (TODO: pick a busier delegator) rather than masking the gap.",
        );
      }
  ...
  describe("paymentInfo", () => {
    ...
    it("returns the same shape on both transports for the same dry-run input", async () => {
      ...
      const rpc = await paymentInfo(ACTIVE_DELEGATOR, fakeTx, JSON_RPC_ID);
      const gql = await paymentInfo(ACTIVE_DELEGATOR, fakeTx, GRAPHQL_ID);
