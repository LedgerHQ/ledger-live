...
export const getCheckpoint = async (
  ...
  // Digest guard for GraphQL: sequence numbers fit in 16 digits (UInt53);
  // base58 digests (~44 chars) can't be passed to `Query.checkpoint(sequenceNumber:)`.
  const isSequenceNumber = id.length > 0 && id.length <= 16 && /^\d+$/.test(id);
