...
export const getLastBlock = () =>
  withApi(async api => {
    ...
    const { digest, sequenceNumber } = await api.getCheckpoint({ id: checkpoint });

    return { digest, sequenceNumber };
  });
...
export const getListOperations = async (addr: string, cursor = ""): Promise<Op<SuiAsset>[]> =>
  withApi(async api => {
    const opsOut = await loadOperation({ api, addr, type: "OUT", cursor });
    const opsIn = await loadOperation({ api, addr, type: "IN", cursor });

    const rawTransactions = [...opsIn, ...opsOut].sort(
      (a, b) => Number(b.timestampMs) - Number(a.timestampMs),
    );

    return rawTransactions.map(t => transactionToOp(addr, t));
  });
