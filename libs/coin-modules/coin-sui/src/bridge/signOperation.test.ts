...
describe("buildSignOperation", () => {
  ...
  describe("Successful signing flow", () => {
    ...
    it("should sign transaction with correct parameters", done => {
      signOperation({ account: mockAccount, deviceId, transaction: mockTransaction })
        ...
        .subscribe({
          next: () => {},
          complete: () => {
            expect(mockSuiSigner.signTransaction).toHaveBeenCalledWith(
    ...
    it("should create message with intent correctly", done => {
      signOperation({ account: mockAccount, deviceId, transaction: mockTransaction })
        .pipe(take(3))
        .subscribe({
          next: () => {},
          complete: () => {
            expect(mockMessageWithIntent).toHaveBeenCalledWith("TransactionData", fakeUnsignedTx);
            done();
          },
          error: done,
        });
    ...
    it("should serialize signature correctly", done => {
      signOperation({ account: mockAccount, deviceId, transaction: mockTransaction })
        .pipe(take(3))
        .subscribe({
          next: () => {},
          complete: () => {
            expect(mockToSerializedSignature).toHaveBeenCalledWith({
              signature: fakeSignature,
        ...
        .subscribe({
          next: event => {
            if (event.type === "signed") {
              ...
              expect(signedOperation.signature).toBe(Buffer.from(fakeSignature).toString("base64"));
              expect(signedOperation.rawData.serializedSignature).toBe(fakeSerializedSignature);
              expect(signedOperation.rawData.unsigned).toBe(
                Buffer.from(fakeUnsignedTx).toString("base64"),
              );
              done();
            }
          },
  ...
  describe("Error handling", () => {
    ...
    it("should throw error when signature verification fails", done => {
      mockVerifyTransactionSignature.mockResolvedValue(false);
    ...
    it("should propagate errors from signTransaction", done => {
      ...
      (mockSuiSigner.signTransaction as jest.Mock).mockRejectedValue(signError);
