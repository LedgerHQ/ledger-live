...
describe("MultiversX API Integration Tests", () => {
  ...
  describe("getSequence", () => {
    it("returns nonce as bigint for address with transaction history", async () => {
      ...
      const sequence = await api.getNextSequence(TEST_ADDRESSES.withEgld);
    ...
    it("returns 0n or positive bigint for system contract (new-ish account)", async () => {
      ...
      const sequence = await api.getNextSequence(TEST_ADDRESSES.zeroBalance);
    ...
    it("throws descriptive error for invalid address", async () => {
      await expect(api.getNextSequence("invalid-address")).rejects.toThrow(
    ...
    it("wraps network errors with consistent error message format", async () => {
      ...
      await expect(badApi.getNextSequence(TEST_ADDRESSES.withEgld)).rejects.toThrow(
  ...
  describe("listOperations", () => {
    ...
    let operations: Awaited<ReturnType<typeof api.listOperations>>["items"];
    let cursor: Awaited<ReturnType<typeof api.listOperations>>["next"];
    let zeroBalanceOperations: Awaited<ReturnType<typeof api.listOperations>>["items"];

    beforeAll(async () => {
      // Single API call for most tests - fetch 10 operations
      operationsResult = await api.listOperations(TEST_ADDRESSES.withEgld, {
        minHeight: 0,
        limit: 10,
      });
      operations = operationsResult.items;
      cursor = operationsResult.next;

      // Separate call for zero balance address
      const { items: zeroOps } = await api.listOperations(TEST_ADDRESSES.zeroBalance, {
        minHeight: 0,
      });
    ...
    it("throws error for invalid address format (AC: #4)", async () => {
      await expect(api.listOperations("invalid-address", { minHeight: 0 })).rejects.toThrow(
        /Invalid MultiversX address/,
      );
    });

    it("returns a Page with items array and optional next cursor", () => {
      expect(operationsResult).toHaveProperty("items");
      expect(operationsResult).toHaveProperty("next");

      expect(Array.isArray(operations)).toBe(true);
      // next is a pagination cursor (string) or undefined when there is no further page
      if (cursor !== undefined) {
  ...
  describe("listOperations - ESDT operations (Story 2.2)", () => {
    ...
    let esdtOperations: Awaited<ReturnType<typeof api.listOperations>>["items"];
    ...
    beforeAll(async () => {
      ...
      esdtOperations = esdtOperationsResult.items;
    }, TIMEOUT_EXTENDED); // Extended timeout for API calls with pagination
    ...
    it(
      "returns operations for address with only EGLD transactions (AC: #4, Subtask 7.4)",
      async () => {
        // Use address that likely has only EGLD (system contract)
        const { items: ops } = await api.listOperations(TEST_ADDRESSES.zeroBalance, {
          minHeight: 0,
          limit: 50,
        });
    ...
    it(
      "pagination works with mixed operations (Subtask 7.6)",
      async () => {
        // Get first page from the address with moderate activity
        const { items: firstPage, next: cursor } = await api.listOperations(
          TEST_ADDRESSES.withModerateEsdtActivity,
          { minHeight: 0, limit: 10 },
        );
        ...
        if (cursor) {
          const { items: secondPage } = await api.listOperations(
            TEST_ADDRESSES.withModerateEsdtActivity,
            { minHeight: 0, limit: 10, cursor: cursor },
          );
  ...
  describe("listOperations - sorting (Story 2.3)", () => {
    ...
    let sortedOperationsDesc: Awaited<ReturnType<typeof api.listOperations>>["items"];
    let sortedOperationsAsc: Awaited<ReturnType<typeof api.listOperations>>["items"];

    beforeAll(async () => {
      // Fetch operations in descending order (default)
      const { items: descOps } = await api.listOperations(TEST_ADDRESSES.withEgld, {
        minHeight: 0,
        limit: 20,
        order: "desc",
      });
      ...
      const { items: ascOps } = await api.listOperations(TEST_ADDRESSES.withEgld, {
        minHeight: 0,
        limit: 20,
        order: "asc",
      });
    ...
    it(
      "returns consistent ordering across repeated calls - AC: #2",
      async () => {
        const { items: result1 } = await api.listOperations(TEST_ADDRESSES.withEgld, {
          minHeight: 0,
          limit: 15,
        });
        const { items: result2 } = await api.listOperations(TEST_ADDRESSES.withEgld, {
          minHeight: 0,
          limit: 15,
        });
  ...
  describe("getStakes (Story 3.1)", () => {
    ...
    it(
      "validates stake structure when delegations exist (AC: #1, Subtask 8.2)",
      async () => {
    ...
    it(
      "returns empty items for address with no delegations (AC: #3, Subtask 8.3)",
      async () => {
    ...
    it(
      "wraps network errors with descriptive message",
      async () => {
        // Use invalid endpoint to trigger network error
        const badApi = createApi({
          apiEndpoint: "https://invalid-endpoint-that-does-not-exist-12345.com",
  ...
  describe("craftTransaction (Story 4.1)", () => {
    ...
    // Note: ESDT intents with an empty assetReference are now rejected by
    // validateIntent (see validateIntent.test.ts), not by craftTransaction.

    it(
      "uses full token balance when useAllAmount is true for ESDT (Story 4.2)",
      async () => {
        ...
        const esdtBalance = balances.find(
          b =>
            ...
            b.asset.assetReference === "WEGLD-bd4d79",
        );
  ...
  describe("craftTransaction - staking (Story 4.3)", () => {
    ...
    it("crafts withdraw transaction (AC: #3)", async () => {
      ...
      // Gas limit should be GAS.DELEGATE (AC: #6) - matches the legacy bridge
      expect(parsed.gasLimit).toBe(GAS.DELEGATE);
    });
    ...
    // Note: staking-intent validation (non-native asset, non-validator recipient) now
    // lives in validateIntent and is covered by validateIntent.test.ts.
  ...
  describe("combine (Story 4.5)", () => {
    ...
    it("combines transaction with valid signature (AC: #1, #2)", async () => {
      const result = await api.combine(VALID_UNSIGNED_TX, VALID_SIGNATURE);
    ...
    it("resulting transaction has signature field (AC: #2)", async () => {
    ...
    it("combined transaction can be parsed as valid JSON (AC: #1)", async () => {
      const unsignedTx = JSON.stringify({
        nonce: 2,
    ...
    it("signature format is preserved as-is (AC: #2)", async () => {
      ...
      const result = await api.combine(VALID_UNSIGNED_TX, hexSignature);
      const parsed = JSON.parse(result);

      expect(parsed.signature).toBe(hexSignature);
    });

    it("handles invalid signature without error per AC #3 (validation on broadcast)", async () => {
      const invalidSignature = "not-a-valid-hex-signature";

      const result = await api.combine(VALID_UNSIGNED_TX, invalidSignature);
      const parsed = JSON.parse(result);
    ...
    it("combine output is compatible with broadcast input format (integration chain test)", async () => {
      ...
      const signedTx = await api.combine(unsignedTx, signature);
  ...
  describe("listOperations - pagination consistency (Story 2.3, AC: #3)", () => {
    it(
      ...
      async () => {
        // Fetch page 1
        const { items: page1, next: cursor1 } = await api.listOperations(TEST_ADDRESSES.withEgld, {
          minHeight: 0,
          limit: 10,
        });
        ...
        if (cursor1) {
          ...
          const { items: page2, next: cursor2 } = await api.listOperations(
            TEST_ADDRESSES.withEgld,
            { minHeight: 0, limit: 10, cursor: cursor1 },
          );
          ...
          if (cursor2) {
            const { items: page3 } = await api.listOperations(TEST_ADDRESSES.withEgld, {
              minHeight: 0,
              limit: 10,
              cursor: cursor2,
            });
    ...
    it(
      ...
      async () => {
        ...
        if (cursor1 && page1.length > 0) {
          const { items: page2 } = await api.listOperations(TEST_ADDRESSES.withEgld, {
            minHeight: 0,
            limit: 10,
            cursor: cursor1,
            order: "desc",
    ...
    it(
      ...
      async () => {
        const { next: cursor } = await api.listOperations(TEST_ADDRESSES.withEgld, {
          minHeight: 0,
          limit: 5,
        });
  ...
  describe("estimateFees", () => {
    it("should estimate fees for native transfer (AC1)", async () => {
      ...
      expect(result.parameters?.gasPrice).toBeGreaterThan(0n);
    });
    ...
    it("should estimate fees for ESDT transfer (AC2)", async () => {
      ...
      expect(result.value).toBeGreaterThan(0n);
      // ESDT transfers use the dedicated ESDT gas limit (higher than native).
    ...
    it("should estimate fees for delegation operation (AC3)", async () => {
      ...
      expect(result.value).toBeGreaterThan(0n);
      // Delegation uses the DELEGATE gas limit.
    ...
    it("should estimate fees for claim rewards operation (AC3)", async () => {
      ...
      expect(result.value).toBeGreaterThan(0n);
      // Claim rewards uses the cheaper CLAIM gas limit.
  ...
  describe("validateIntent", () => {
    it(
      "validates native transfer intent with real balances",
      async () => {
        const balances = await api.getBalance(TEST_ADDRESSES.withEgld);
        ...
        // No custom fees were provided, so estimatedFees is 0 and totalSpent === amount.
        expect(result.estimatedFees).toBe(0n);
        expect(result.totalSpent).toBe(result.amount + result.estimatedFees);
      },
      TIMEOUT_STANDARD,
    );

    it(
      "validates ESDT transfer intent with real balances",
      async () => {
        const balances = await api.getBalance(TEST_ADDRESSES.withTokens);
    ...
    it(
      "detects insufficient balance errors correctly",
      async () => {
        const balances = await api.getBalance(TEST_ADDRESSES.withEgld);
    ...
    it(
      "validates with custom fees parameter",
      async () => {
        const balances = await api.getBalance(TEST_ADDRESSES.withEgld);
  ...
  describe("Edge Cases - Empty and New Accounts (Story 5.6)", () => {
    ...
    describe("getBalance edge cases (AC: #1)", () => {
      ...
      it("returns 0n value (not null, undefined, or empty)", async () => {
        ...
        expect(nativeBalance.value).not.toBe(0);
      });
    });
    ...
    describe("listOperations edge cases (AC: #2, #4)", () => {
      it(
        "returns valid array structure for system contract address (AC: #2)",
        async () => {
          // Note: System contract may have incoming transactions on mainnet
          // This test verifies the return type is always a valid array
          const { items: operations, next: cursor } = await api.listOperations(
        ...
        TIMEOUT_EXTENDED,
      ); // Extended timeout for potentially slow API response

      it("returns Page structure even for empty results", async () => {
        const result = await api.listOperations(EDGE_CASE_ADDRESSES.systemContract, {
          minHeight: 0,
          limit: 10,
        });

        // Must return a Page { items, next }
      ...
      it(
        "pagination works correctly for very active account (AC: #4)",
        async () => {
          // Fetch first page
          const { items: page1, next: cursor1 } = await api.listOperations(
          ...
          if (cursor1) {
            ...
            const { items: page2, next: cursor2 } = await api.listOperations(
              EDGE_CASE_ADDRESSES.highActivity,
              { minHeight: 0, limit: 5, cursor: cursor1 },
            );
            ...
            if (cursor2) {
              const { items: page3 } = await api.listOperations(EDGE_CASE_ADDRESSES.highActivity, {
                minHeight: 0,
                limit: 5,
                cursor: cursor2,
              });
      ...
      it("pagination boundary: last page returns remaining items", async () => {
        ...
        const { items: allOps } = await api.listOperations(EDGE_CASE_ADDRESSES.highActivity, {
          minHeight: 0,
          limit: 100,
        });
        ...
        if (allOps.length > 5) {
          ...
          while (currentCursor && iterations < maxIterations) {
            const { items: nextPage, next: nextCursor } = await api.listOperations(
              EDGE_CASE_ADDRESSES.highActivity,
              {
                minHeight: 0,
                limit: 5,
      ...
      it(
        "pagination with empty page at end doesn't throw",
        async () => {
          // Use an address with limited history
          const { items: ops, next: cursor } = await api.listOperations(
            EDGE_CASE_ADDRESSES.systemContract,
            { minHeight: 0, limit: 10 },
          );
          ...
          if (cursor) {
            const { items: nextOps } = await api.listOperations(
              EDGE_CASE_ADDRESSES.systemContract,
              { minHeight: 0, limit: 10, cursor: cursor },
            );
    ...
    describe("getSequence edge cases", () => {
      it("new account returns 0n nonce", async () => {
        const sequence = await api.getNextSequence(EDGE_CASE_ADDRESSES.systemContract);
      ...
      it("nonce is never negative", async () => {
        const sequence1 = await api.getNextSequence(EDGE_CASE_ADDRESSES.systemContract);
        const sequence2 = await api.getNextSequence(EDGE_CASE_ADDRESSES.highActivity);
      ...
      it("active account returns positive nonce", async () => {
        const sequence = await api.getNextSequence(EDGE_CASE_ADDRESSES.highActivity);
    ...
    describe("invalid address handling edge cases", () => {
      ...
      it("empty string address throws error", async () => {
        ...
        await expect(api.getNextSequence("")).rejects.toThrow(/Invalid MultiversX address/);
        await expect(api.listOperations("", { minHeight: 0 })).rejects.toThrow(
          /Invalid MultiversX address/,
        );
    ...
    describe("network error edge cases", () => {
      ...
      it("API with invalid endpoint throws error for listOperations", async () => {
        ...
        await expect(
          badApi.listOperations(TEST_ADDRESSES.withEgld, { minHeight: 0 }),
        ).rejects.toThrow();
      });
      ...
      it("all methods throw errors for invalid endpoint", async () => {
        ...
        try {
          await badApi.getNextSequence(TEST_ADDRESSES.withEgld);
        } catch (e) {
          errors.push(e as Error);
        }
  ...
  describe("combine - integration tests (Story 5.7)", () => {
    ...
    describe("craftTransaction → combine chain (AC: #1, #4)", () => {
      it("combines unsigned transaction from craftTransaction with mock signature (AC: #1)", async () => {
        ...
        const signedTx = await api.combine(unsignedTx, MOCK_SIGNATURE);
