...
describe("mapToOperation", () => {
  ...
  const createTransaction = (
    // Allow explicit `undefined` overrides (exactOptionalPropertyTypes): several tests
    // deliberately override a default field with `undefined` to exercise missing-field paths.
    overrides: {
    ...
    ({
      ...
      gasLimit: 50000,
      ...overrides,
      // Cast because `overrides` may set a field to `undefined` (missing-field test paths),
      // which exactOptionalPropertyTypes rejects against the non-optional return shape.
  ...
  it("treats a missing status as not failed", () => {
    const tx = createTransaction({ status: undefined });
  ...
  it("handles fee as string value", () => {
    const tx = createTransaction({
      fee: "50000000000000" as unknown as typeof BigNumber.prototype,
    });
  ...
  it("handles value as string value", () => {
    const tx = createTransaction({
      value: "2000000000000000000" as unknown as typeof BigNumber.prototype,
    });
  ...
  it("handles null fee by defaulting to 0", () => {
    const tx = createTransaction({
      fee: null as unknown as typeof BigNumber.prototype,
      fees: undefined,
    });
...
describe("mapToStake", () => {
  ...
  it("correctly calculates total amount including rewards and undelegating (Subtask 6.5)", () => {
    const delegation = createDelegation({
      ...
      userUndelegatedList: [
        ...
        { amount: "300000000000000000", seconds: 172800 },
      ],
    });
  ...
  it("includes all components in total: stake + rewards + undelegating + withdrawable", () => {
    const delegation = createDelegation({
      ...
      userUndelegatedList: [{ amount: "300000000000000000", seconds: 86400 }],
    });
  ...
  it("does not double-count withdrawable stake when both userUnBondable and completed undelegations are present", () => {
    const delegation = createDelegation({
      ...
      userUnBondable: "500000000000000000",
      userUndelegatedList: [
        { amount: "500000000000000000", seconds: 0 },
        { amount: "200000000000000000", seconds: 3600 },
      ],
    });
  ...
  it("handles missing userUnBondable gracefully (defaults to 0)", () => {
    const delegation = {
      ...
      userActiveStake: "1000000000000000000",
      claimableRewards: "0",
      userUnBondable: undefined as unknown as string,
      userUndelegatedList: [],
    };
  ...
  describe("state mapping with improved logic (AC: #2, #3)", () => {
    ...
    it("returns 'deactivating' when undelegation has seconds > 0 (AC: #2 - unstaking)", () => {
      const delegation = createDelegation({
        ...
        claimableRewards: "0",
        userUnBondable: "0",
        userUndelegatedList: [{ amount: "500000000000000000", seconds: 3600 }],
      });
    ...
    it("returns 'deactivating' when mixed undelegations (some pending, some complete)", () => {
      const delegation = createDelegation({
        ...
        userUndelegatedList: [
          ...
          { amount: "300000000000000000", seconds: 7200 },
        ],
      });
