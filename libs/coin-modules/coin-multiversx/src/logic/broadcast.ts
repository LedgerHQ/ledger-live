...
import type { MultiversXProtocolTransaction, MultiversXSignedTransaction } from "../types";
 ...
 */
export async function broadcast(signedTx: string, apiClient: MultiversXApiClient): Promise<string> {
  ...
  if (typeof tx.nonce !== "number" || tx.nonce < 0) {
    throw new Error(
      `Invalid signed transaction: nonce must be a non-negative number, got ${tx.nonce}`,
    );
  }

  if (typeof tx.gasPrice !== "number" || tx.gasPrice <= 0) {
    throw new Error(
      `Invalid signed transaction: gasPrice must be a positive number, got ${tx.gasPrice}`,
    );
  }

  if (typeof tx.gasLimit !== "number" || tx.gasLimit < MIN_GAS_LIMIT) {
    throw new Error(
      `Invalid signed transaction: gasLimit must be at least ${MIN_GAS_LIMIT}, got ${tx.gasLimit}`,
    );
  }
  ...
  if (typeof tx.value !== "string" || tx.value.trim() === "") {
    throw new Error(
      `Invalid signed transaction: value must be a non-empty string, got ${tx.value}`,
    );
  }
  ...
  try {
    ...
    throw new Error(
      `Invalid signed transaction: value must be a valid numeric string, got ${tx.value}`,
    );
  }
  ...
  if (typeof tx.chainID !== "string" || tx.chainID.trim() === "") {
    throw new Error(
      `Invalid signed transaction: chainID must be a non-empty string, got ${tx.chainID}`,
    );
  }
  ...
  // Create the signed transaction format expected by the network layer
  const signedOperation: MultiversXSignedTransaction = {
    signature,
    rawData,
  };
  ...
  try {
    ...
    if (error instanceof Error) {
      ...
      const enhancedError: Error & { cause?: unknown } = new Error(
        `Transaction broadcast failed: ${error.message}`,
      );
