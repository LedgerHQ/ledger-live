...
describe("craftTransaction", () => {
  ...
  describe("input validation (ADR-003)", () => {
    ...
    it("throws for an erd1 address of correct length but invalid bech32 checksum", () => {
      const badChecksum = "erd1" + "q".repeat(58);
      expect(badChecksum).toHaveLength(62);

      expect(() => craftTransaction({ ...mockInput, sender: badChecksum })).toThrow(
        "craftTransaction failed: invalid sender address format",
      );
      expect(() => craftTransaction({ ...mockInput, recipient: badChecksum })).toThrow(
        "craftTransaction failed: invalid recipient address format",
      );
    });
  ...
  describe("ESDT token transfers", () => {
    ...
    it("treats a missing tokenIdentifier as native EGLD transfer", () => {
      // When tokenIdentifier is absent, implementation treats it as native EGLD transfer
      // Per design: tokenIdentifier determines ESDT vs native, not a validation error
      const { tokenIdentifier: _tokenIdentifier, ...noTokenInput } = esdtInput;
    ...
    it("correctly encodes full ESDT transfer data string", () => {
      ...
      const expectedAmountHex = "05f5e100";
  ...
  describe("delegation modes", () => {
    ...
    describe("withdraw mode", () => {
      ...
      it("uses GAS.DELEGATE gas limit for withdraw mode (AC: #6)", () => {
    ...
    describe("reDelegateRewards mode", () => {
      ...
      it("uses GAS.DELEGATE gas limit for reDelegateRewards mode (AC: #6)", () => {
    ...
    describe("common delegation behavior", () => {
      ...
      it.each(delegationModes)("%s mode maintains correct transaction structure", mode => {
        ...
        expect(parsed.version).toBe(TRANSACTION_VERSION_DEFAULT);
        expect(parsed.options).toBe(TRANSACTION_OPTIONS_TX_HASH_SIGN);
        // Delegation modes always carry a non-empty base64-encoded data field.
        expect(typeof parsed.data).toBe("string");
    ...
    describe("gas limit selection", () => {
      ...
      it("withdraw mode uses GAS.DELEGATE by default", () => {
        const input: CraftTransactionInput = { ...mockDelegationInput, mode: "withdraw" };
        const result = craftTransaction(input);
        const parsed = JSON.parse(result.transaction);
      ...
      it("reDelegateRewards mode uses GAS.DELEGATE by default", () => {
        const input: CraftTransactionInput = { ...mockDelegationInput, mode: "reDelegateRewards" };
        const result = craftTransaction(input);
        const parsed = JSON.parse(result.transaction);
  ...
  describe("staking type mapping", () => {
    it("accepts supported staking types", () => {
      expect(isSupportedStakingType("delegate")).toBe(true);
      expect(isSupportedStakingType("unDelegate")).toBe(true);
      expect(mapStakingTypeToMode("claim_rewards")).toBe("claimRewards");
    });

    it("rejects unsupported staking types", () => {
      expect(isSupportedStakingType("notAStakingType")).toBe(false);
      expect(() => mapStakingTypeToMode("notAStakingType")).toThrow("unsupported staking type");
    });

    it("does not treat Object.prototype members as supported types", () => {
      for (const key of ["toString", "valueOf", "constructor", "hasOwnProperty", "__proto__"]) {
        expect(isSupportedStakingType(key)).toBe(false);
        expect(() => mapStakingTypeToMode(key)).toThrow("unsupported staking type");
      }
    });
  });

  describe("isStakingIntent", () => {
    const intent = (over: Partial<TransactionIntent>): TransactionIntent =>
      ({ intentType: "transaction", type: "send", ...over }) as TransactionIntent;

    it("classifies framework-tagged staking intents", () => {
      expect(isStakingIntent(intent({ intentType: "staking", type: "delegate" }))).toBe(true);
    });

    it("classifies staking types the framework tags as transaction (unDelegate/claimRewards/reDelegateRewards)", () => {
      for (const type of ["unDelegate", "claimRewards", "reDelegateRewards"]) {
        expect(isStakingIntent(intent({ intentType: "transaction", type }))).toBe(true);
      }
    });

    it("does not classify plain transfers as staking", () => {
      expect(isStakingIntent(intent({ intentType: "transaction", type: "send" }))).toBe(false);
    });
  });
});
