...
describe("buildSubAccounts", () => {
  ...
  it("preserves existing token account order", async () => {
    ...
    // Mock findTokenByAddressInCurrency to return tokens based on the ESDT identifier (3rd arg)
    getCryptoAssetsStore.mockReturnValue({
      findTokenByAddressInCurrency: jest
        .fn()
        .mockImplementation((_address: string, _currencyId: string, identifier: string) => {
          if (identifier === "USDC-123456") return Promise.resolve(mockToken1);
          if (identifier === "USDT-789012") return Promise.resolve(mockToken2);
  ...
  it("returns same token account when balance unchanged", async () => {
    ...
    getESDTOperations.mockResolvedValue([]);
    const { mergeOps } = jest.requireMock("@ledgerhq/ledger-wallet-framework/bridge/jsHelpers");
