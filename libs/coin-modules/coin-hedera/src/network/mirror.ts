...
import {
  ...
  type HederaMirrorAccount,
  type HederaMirrorAccountsResponse,
  type HederaMirrorToken,
  type HederaMirrorTransaction,
} from "./types";
import { HederaAddAccountError } from "../errors";
import { LedgerAPI4xx } from "@ledgerhq/errors";
import { LiveNetworkResponse } from "@ledgerhq/live-network/lib-es/network";

const fetch = <Result>(path: string) => {
  return network<Result>({
    method: "GET",
    url: `${getEnv("API_HEDERA_MIRROR")}${path}`,
  });
};
