...
function getCommonMirrorOperationData(
  ...
  // Same buffer as `lastBlockV2`, else a final tx reads as unconfirmed for one window.
...
export async function listOperationsV2(
  ...
  // Floor at the page's oldest tx, else every page re-fetches back to the first transfer.
  // Desc only: asc needs a ceiling, which `minTimestamp` can't express.
  const pageFloor =
