...
import { AccountLike, Operation as LiveOperation } from "@ledgerhq/types-live";
...
export const mapIntentToSDKOperation = (txIntent: TransactionIntent) => {
  ...
  if (txIntent.type === "send" && txIntent.asset.type !== "native") {
...
export const isTokenAssociateTransaction = (
  tx: Transaction,
): tx is Extract<Transaction, { mode: "token-associate" }> => {
  return tx.mode === "token-associate";
};
