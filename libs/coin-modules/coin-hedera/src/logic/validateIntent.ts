...
async function validateTokenAssociate({
  ...
  invariant(currency.units[0], "hedera: unit not found");
  ...
  const hbarMagnitude = currency.units[0].magnitude;
  const hbarAvailable = new BigNumber(nativeAvailable.toString()).shiftedBy(-hbarMagnitude);
