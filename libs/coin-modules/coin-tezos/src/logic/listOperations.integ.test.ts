...
describe("listOperations — mainnet FA2 / convertTokenOperation", () => {
  ...
  it("attributes fees from parent native transaction when resolvable (FA2)", async () => {
    ...
    for (const op of withParentHint) {
      // Token ops always carry fees = 0n (LIVE-30365): the parent native op with
      // the same hash already holds the fees; copying them here would double-count.
      expect(op.tx.fees).toBe(0n);
      // feesPayer is still populated so the UI can display who paid at tx level.
