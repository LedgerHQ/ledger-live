...
describe("onboard", () => {
  ...
  describe("buildPairWalletConnect", () => {
    it("should emit INIT, PREPARE with URI, and SUCCESS with session topic", async () => {
      ...
      expect(prepareEvent).toMatchObject({
        walletConnectUri: expect.stringContaining("wc:test-uri"),
      });
    ...
    it("should construct correct walletConnectUri with encoded URI", async () => {
      ...
      expect(prepareEvent.walletConnectUri).toContain("wc:abc123@2?relay-protocol=irn&symKey=xyz");
    });
