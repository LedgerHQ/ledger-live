...
import type { SubmitCredentialRequest } from "../types/network";
...
describe("submitCredential integration test", () => {
  ...
  let payload: SubmitCredentialRequest;
  ...
  beforeAll(() => {
    ...
    payload = JSON.parse(payloadContent) as SubmitCredentialRequest;
  });
  ...
  it("should validate payload structure matches SubmitCredentialRequest type", () => {
    // Validate version
    expect(payload.v).toBe(0);
  ...
  it("should handle invalid payload structure", async () => {
    ...
    } as any as SubmitCredentialRequest;
