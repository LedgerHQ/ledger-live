 ...
 *
 * @param currency - The Concordium currency
 * @param credentialData - The credential deployment data in proxy format
 * @returns Submission ID that can be used to check status via getSubmissionStatus
 */
export async function submitCredential(
  currency: CryptoCurrency,
  credentialData: SubmitCredentialRequest,
): Promise<{ submissionId: string }> {
  return withClient(currency, async client => {
    const request: LiveNetworkRequest<SubmitCredentialRequest> = {
      method: "PUT",
      url: "/v0/submitCredential/",
      data: credentialData,
    };
    return client.request<{ submissionId: string }, SubmitCredentialRequest>(request);
  });
}
