 ...
 * @param options - List operations options (minHeight determines scan start)
...
export async function getOperations(
  ...
  try {
    ...
    const next = hasMore ? JSON.stringify(endHeight + 1) : undefined;

    return { items: operations, next };
  } catch (error) {
