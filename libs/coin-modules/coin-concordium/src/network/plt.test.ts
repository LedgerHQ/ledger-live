...
describe("readAccountTokens", () => {
  ...
  // Two entries for one token cannot both be its standing, and the lookup would
  // answer from the first — which may be the permissive copy.
  it("rejects the whole list when one token appears twice", () => {
    const entries = [
      accountToken({ denyList: true }),
      accountToken({ denyList: true }, { denyList: true }),
    ];
    expect(readAccountTokens(entries)).toBeUndefined();
  });
});
