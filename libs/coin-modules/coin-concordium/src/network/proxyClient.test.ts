...
describe("proxyClient", () => {
  ...
  describe("getSubmissionStatus", () => {
    it("should fetch submission status", async () => {
      mockNetwork.mockResolvedValue({ data: { status: "finalized" } });
      const currency = createMockCurrency();

      const result = await getSubmissionStatus(currency, "submission-123");

      expect(result).toEqual({ status: "finalized" });
      expect(mockNetwork).toHaveBeenCalledWith(
        expect.objectContaining({
          method: "GET",
          url: "https://wallet-proxy.concordium.com/v0/submissionStatus/submission-123",
        }),
      );
    });
  });
  ...
  describe("getTransactionCost", () => {
    it("should fetch transaction cost", async () => {
      ...
      const result = await getTransactionCost(currency, "simpleTransfer", 1);
  ...
  describe("getIdentityProviders", () => {
    it("should fetch identity providers", async () => {
      const mockProviders = [{ ipIdentity: 0, ipDescription: { name: "Test Provider" } }];
      mockNetwork.mockResolvedValue({ data: mockProviders });
      const currency = createMockCurrency();

      const result = await getIdentityProviders(currency);

      expect(result).toEqual(mockProviders);
      expect(mockNetwork).toHaveBeenCalledWith(
        expect.objectContaining({
          method: "GET",
          url: "https://wallet-proxy.concordium.com/v2/ip_info",
        }),
      );
    });
  });
  ...
  describe("submitRawTransaction", () => {
    it("should submit raw transaction as binary", async () => {
      mockNetwork.mockResolvedValue({ data: { submissionId: "raw-123" } });
      const currency = createMockCurrency();

      const result = await submitRawTransaction(currency, "deadbeef");

      expect(result).toEqual({ submissionId: "raw-123" });
      expect(mockNetwork).toHaveBeenCalledWith(
        expect.objectContaining({
          method: "PUT",
          url: "https://wallet-proxy.concordium.com/v0/submitRawTransaction/",
      ...
      expect(mockNetwork.mock.calls[0][0].data).toBeInstanceOf(ArrayBuffer);
    });
  });
  ...
  describe("getOperations", () => {
    ...
    it("should filter transactions by minimum sequence number", async () => {
      ...
      const result = await getOperations(currency, "test-address", "account-id", { from: 3 });

      // Only tx with id >= 3 should be included
      expect(result).toHaveLength(1);
      expect(result[0].hash).toBe("tx5");
    });
    ...
    it("should use default size when not specified", async () => {
      ...
      await getOperations(currency, "test-address", "account-id", { from: 0 });
    ...
    it("should use provided size param", async () => {
      ...
      await getOperations(currency, "test-address", "account-id", { from: 0, size: 50 });
