 ...
 *
 * A repeated `tokenId` taints it for the same reason. Two entries cannot both
 * describe the account's standing on one token, and the lookup would answer
 * from whichever copy the proxy happened to list first.
 */
export function readAccountTokens(accountTokens: unknown): PltAccountToken[] | undefined {
  ...
  for (const value of accountTokens) {
    ...
    if (!entry || seenIds.has(entry.token.tokenId)) return undefined;
    seenIds.add(entry.token.tokenId);
    entries.push(entry);
  }
  return entries;
}
