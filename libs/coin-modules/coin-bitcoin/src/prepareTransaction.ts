...
export function sameExcludeUTXOs(
  a: Array<{ hash: string; outputIndex: number }>,
  b: Array<{ hash: string; outputIndex: number }>,
): boolean {
  if (a.length !== b.length) return false;
  const normalize = (u: { hash: string; outputIndex: number }): string =>
    `${u.hash}:${u.outputIndex}`;
  const aKeys = a.map(normalize).sort();
  const bKeys = b.map(normalize).sort();
...
export const prepareTransaction: AccountBridge<Transaction>["prepareTransaction"] = async (
  ...
  if (
    ...
    sameExcludeUTXOs(currentExclusions, mergedExclusions)
  ) {
    // nothing changed
    return transaction;
  }
