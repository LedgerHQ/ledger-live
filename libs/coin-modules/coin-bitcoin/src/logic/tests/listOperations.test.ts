...
// listOperations walks the xpub through wallet-btc. The account's explorer endpoint is now resolved
// from `context.config(currencyId).explorer.uri` (threaded through buildSyncedAccount); point it at
// the fake host and intercept the pristine-sync routes.
