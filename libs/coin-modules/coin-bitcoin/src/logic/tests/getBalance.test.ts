...
// getBalance walks the xpub through wallet-btc. The account's explorer endpoint is now resolved from
// `context.config(currencyId).explorer.uri` (threaded through buildSyncedAccount), so we point it at
// the fake host and intercept the Ledger explorer v4 routes a pristine sync touches.
