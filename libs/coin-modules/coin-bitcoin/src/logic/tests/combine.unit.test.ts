...
// bitcoinjs-lib is mocked so we can assert combine merges the signed PSBT into the crafted one.
...
describe("logic/combine", () => {
  ...
  it("throws when no signed PSBT is provided", () => {
    expect(() => combine("UNSIGNED_PSBT", [])).toThrow(/expected exactly one signed PSBT, got 0/);
  });

  it("throws when more than one signature is provided (Bitcoin is single-signature)", () => {
    expect(() => combine("UNSIGNED_PSBT", ["a", "b"])).toThrow(
      /expected exactly one signed PSBT, got 2/,
    );
  });
});
