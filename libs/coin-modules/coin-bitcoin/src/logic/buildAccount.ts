...
import { getExplorer } from "../network";
...
export function deriveAccountMeta(derivationPath: string | undefined): AccountMeta {
  ...
  // Standard BIP32 paths may carry the `m/` (or `M/`) root, e.g. `m/84'/0'/0'`. Drop it before
  // indexing so `parts[0]` is the purpose, not the literal `m` (which would parse to NaN).
...
export async function buildSyncedAccount(
  ...
  // wallet-btc builds the account's explorer from the currency's default endpoint. When the Alpaca
  // config pins an explorer URI (ADR-019), swap in the config-resolved explorer so the account-scoped
  // methods (getBalance, listOperations, estimateFees, craftTransaction) honor
  // `context.config(currencyId).explorer.uri` — the same endpoint lastBlock/broadcast use via the
  // network layer. Absent an override, the wallet-btc default endpoint is already correct.
  if (config?.explorer?.uri) {
    account.xpub.explorer = getExplorer(currencyId, config);
  }
