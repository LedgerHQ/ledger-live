...
describe("broadcast", () => {
  ...
  describe("UTXO spent detection", () => {
    describe("skipping the check", () => {
      it("should skip check entirely when extra.inputs is absent", async () => {
      ...
      it("should skip check when extra.inputs is an empty array", async () => {
        jest.spyOn(wallet, "broadcastTx").mockResolvedValue("mock-hash");

        const signedOp: SignedOperation = {
          ...mockSignedOperation,
          operation: { ...mockSignedOperation.operation, extra: { inputs: [] } },
        };
      ...
      it("should throw InvalidTransactionError and not call broadcastTx", async () => {
        ...
        const signedOp: SignedOperation = {
          ...
          operation: {
            ...
            extra: { inputs: [{ hash: "tx-hash", outputIndex: 0, address: "addr-a" }] },
          },
        };

        await expect(broadcast({ account: mockAccount, signedOperation: signedOp })).rejects.toMatchObject({
    ...
    describe("API call counts", () => {
      ...
      it("should call broadcastTx after a successful check", async () => {
        ...
        const signedOp: SignedOperation = {
          ...
          operation: {
            ...mockSignedOperation.operation,
            extra: { inputs: [{ hash: "our-hash", outputIndex: 0, address: "addr-a" }] },
          },
        };
