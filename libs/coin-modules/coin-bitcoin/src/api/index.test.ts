...
describe("api/createApi — each method delegates to its logic function (currencyId threaded)", () => {
  ...
  it("validateAddress → validateAddress(address, { ...parameters, captured currencyId })", async () => {
    await impl().validateAddress(ctx, "bc1qrecipient", { currencyId: "bitcoin" } as never);
    expect(jest.mocked(validateAddress)).toHaveBeenCalledWith("bc1qrecipient", {
      currencyId: "bitcoin",
    });
  });

  it("validateAddress fills the captured currencyId when the caller omits it", async () => {
    await impl().validateAddress(ctx, "bc1qrecipient", {} as never);
    expect(jest.mocked(validateAddress)).toHaveBeenCalledWith("bc1qrecipient", {
      currencyId: "bitcoin",
    });
  });

  it("validateAddress overrides a conflicting caller currencyId with the factory's", async () => {
    await impl().validateAddress(ctx, "bc1qrecipient", { currencyId: "litecoin" } as never);
    expect(jest.mocked(validateAddress)).toHaveBeenCalledWith("bc1qrecipient", {
      currencyId: "bitcoin",
    });
  });
