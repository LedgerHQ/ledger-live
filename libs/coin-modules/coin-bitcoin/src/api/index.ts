 ...
 * shipping path. Zcash is out of scope. `getNextSequence`, staking, `craftRawTransaction`,
 * `register`, `call` are omitted — `withDefaults` answers "not supported" for each (Bitcoin has no
 * account nonce, no staking, etc.). `getBlock`/`getBlockInfo` are implemented (block explorer data).
 *
...
export function createApi(currencyId: string) {
  return {
    ...
    getBlockInfo: (context: BitcoinContext, height: number): Promise<BlockInfo> =>
      getBlockInfo(context, currencyId, height),

    getBlock: (context: BitcoinContext, height: number): Promise<Block> =>
      getBlock(context, currencyId, height),

    // The factory is instantiated per currency (ADR-019), so the captured `currencyId` is the
    // network to validate against — merge it in last so an omitted parameter is filled and a
    // caller-supplied one can never validate against a different network (cf. the Cosmos factory).
