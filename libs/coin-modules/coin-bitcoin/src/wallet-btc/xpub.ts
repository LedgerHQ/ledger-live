...
class Xpub {
    ...
    if (originalTxId && originalTxObj) {
      try {
        await this.enforceRbfAbsoluteFeeIncrease({
          ...
          outputsValue,
          dustLimit,
          replacementVsizeEstimate: maxTxSizeCeil(
            inputs.length,
            outputs.map(o => o.script),
            true,
            this.crypto,
  ...
  private async enforceRbfAbsoluteFeeIncrease({
    ...
    outputsValue,
    dustLimit,
    replacementVsizeEstimate,
    fallbackReducibleAddress,
  }: {
    ...
    dustLimit: number;
    replacementVsizeEstimate?: number;
    fallbackReducibleAddress?: string;
  }): Promise<void> {
    ...
    const requiredDeltaVsize = Math.max(originalVsize, replacementVsizeEstimate ?? 0);
    const minAbsoluteFeeIncrease = incrementalFeeRateSatVb
      .times(requiredDeltaVsize)
    ...
    // We need to increase the fee by reducing a wallet-controlled output.
    // Prefer explicit change output, but for cancel/send-max-like replacements
    // the wallet-controlled output can be the main recipient output to self.
    const additionalFeeNeeded = minAbsoluteFeeIncrease - actualFeeIncrease;
    const changeOutput = outputs.find(o => o.isChange);
    const fallbackReducibleOutput =
      !changeOutput && fallbackReducibleAddress
        ? outputs.find(o => !o.isChange && o.address === fallbackReducibleAddress)
        : undefined;
    const reducibleOutput = changeOutput ?? fallbackReducibleOutput;
    if (!reducibleOutput) {
      throw new Error(
    ...
    const newChangeValue = reducibleOutput.value.minus(additionalFeeNeeded);
    if (newChangeValue.lt(dustLimit)) {
      const changeIndex = outputs.indexOf(reducibleOutput);
      outputs.splice(changeIndex, 1);
    } else {
      reducibleOutput.value = newChangeValue;
    }
  }
