  ...
  withApi(currency, async api => {
    ...
    for (const nonce in txpoolContent.pending) {
      result.push({
        ...
        type: txpoolContent.pending[nonce].type,
        gasUsed: "0x0",
        gasLimit: txpoolContent.pending[nonce].gas?.toString(),
        gasPrice: txpoolContent.pending[nonce].gasPrice.toString(),
        maxFeePerGas: txpoolContent.pending[nonce].maxFeePerGas?.toString() ?? null,
        maxPriorityFeePerGas: txpoolContent.pending[nonce].maxPriorityFeePerGas?.toString() ?? null,
        input: txpoolContent.pending[nonce].input?.toString() ?? null,
        status: null,
