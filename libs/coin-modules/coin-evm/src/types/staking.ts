 ...
 *   `evmRpcUrl` in `apiConfig` must be set; redelegations are skipped otherwise.
...
export type StakingContractConfig = {
  ...
  apiConfig?: {
    ...
    /**
     * EVM JSON-RPC URL used to call the chain's address precompile to resolve
     * the canonical Cosmos bech32 address for a given EVM address.  Required
     * for `cosmos-rest` redelegation strategy — redelegations are skipped when
     * absent.  Must be used together with `precompileAddress`.
     *
     * EVM and Cosmos addresses are derived from the same secp256k1 public key
     * but through different hash algorithms (`keccak256` vs
     * `RIPEMD160∘SHA256`), so a direct bech32 re-encoding of the EVM bytes
     * would produce the wrong Cosmos address.
     */
    evmRpcUrl?: string;
    /**
