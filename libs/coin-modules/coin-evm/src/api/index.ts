...
// `call` has a narrower return (Promise<string>) than the framework's CallResult, so it is omitted
// from the base CoinModuleApi and re-declared with the EVM-specific signature.
type EvmCoinModuleApi = Omit<
  CoinModuleApi<EvmConfigInfo, MemoNotSupported, BufferTxData>,
  "call"
> & {
  call: (context: EvmContext, params: CallParams) => Promise<string>;
};

export function createApi(currencyId: string): EvmCoinModuleApi {
