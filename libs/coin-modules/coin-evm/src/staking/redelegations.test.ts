...
describe("redelegations", () => {
  describe("evmAddressToCosmos", () => {
    it("should produce a valid sei bech32 address from an EVM address", () => {
      const result = evmAddressToCosmos("0x1234567890abcdef1234567890abcdef12345678", "sei");
      expect(result).toMatch(/^sei1[qpzry9x8gf2tvdw0s3jn54khce6mua7l]+$/);
    });

    it("should strip the 0x prefix before encoding", () => {
      const addr = "abcdef1234567890abcdef1234567890abcdef12";
      expect(evmAddressToCosmos(`0x${addr}`, "sei")).toBe(evmAddressToCosmos(addr, "sei"));
    });

    it("should respect the hrp", () => {
      const evmAddress = "0x1234567890abcdef1234567890abcdef12345678";
      expect(evmAddressToCosmos(evmAddress, "sei")).toMatch(/^sei1/);
      expect(evmAddressToCosmos(evmAddress, "cosmos")).toMatch(/^cosmos1/);
    });

    it("should produce a deterministic result for the same input", () => {
      const addr = "0x1111111111111111111111111111111111111111";
      expect(evmAddressToCosmos(addr, "sei")).toBe(evmAddressToCosmos(addr, "sei"));
    });
  });
  ...
  describe("resolveRedelegationValidators", () => {
    ...
    it("should decode from cached contractPayload in operation.extra", async () => {
      ...
      expect(result!.srcValidatorAddress).toBe(SRC);
      expect(result!.dstValidatorAddress).toBe(DST);
  ...
  describe("buildRedelegationsFromOps", () => {
    ...
    it("should build a redelegation from a recent REDELEGATE op with cached calldata", async () => {
      ...
      expect(result[0].validatorDstAddress).toBe(DST);
      expect(result[0].amount.isEqualTo(EXPECTED_WEI.toString())).toBe(true);
      expect(result[0].completionDate > new Date()).toBe(true);
    });
