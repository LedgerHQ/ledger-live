...
/**
 * Fetch raw EVM transaction calldata (the `input` field) for a given tx hash
 * via the `NodeApi` interface.  Returns `undefined` when the call fails.
 */
async function fetchTxDataFromRpc(
  ...
  try {
    const tx = await getNodeApi(currency).getTransaction(currency, txHash);
    return tx.input && tx.input !== "0x" ? tx.input : undefined;
  } catch (e) {
