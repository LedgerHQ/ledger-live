...
async function getCosmosAddr(
  ...
  try {
    ...
    log("coin-evm/staking", "getCosmosAddr: precompile call failed", {
      evmRpcUrl,
      evmAddress,
      error: e,
    });
 ...
 * not been associated on-chain yet) or `evmRpcUrl` / `cosmosAddressPrecompile`
 * are not configured.
 */
async function fetchCosmosRestRedelegations(
  ...
  try {
    ...
    log("coin-evm/staking", "fetchCosmosRestRedelegations: REST request failed", { url, error: e });
    return [];
  }
...
export async function resolveRedelegationValidators(
  ...
  try {
    ...
    log("coin-evm/staking", "resolveRedelegationValidators: calldata decoding failed", {
      currencyId: currency.id,
...
export async function resolveStakingValidator(
  ...
  try {
    ...
    log("coin-evm/staking", "resolveStakingValidator: calldata decoding failed", {
      currencyId: currency.id,
      txHash: operation.hash,
      operationType,
      error: e,
  ...
  const redelegations = await Promise.all(
    eligibleOperations.map(async op => {
      ...
      try {
        ...
        log("coin-evm/staking", "buildRedelegationsFromOps: calldata decoding failed", {
          currencyId,
          txHash: op.hash,
          error: e,
        });
