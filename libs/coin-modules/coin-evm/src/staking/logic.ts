...
export const EVM_MAX_REDELEGATIONS = 7;
...
export function canRedelegate(
  ...
  const now = new Date();
  // Cosmos SDK caps concurrent redelegation entries at EVM_MAX_REDELEGATIONS (7) per account.
  const activeRedelegations = redelegations.filter(rd => rd.completionDate > now);
  if (activeRedelegations.length >= EVM_MAX_REDELEGATIONS) return false;
...
export function decodeRedelegatePayload(
  ...
  try {
    ...
    const [src, dst] = d;
    return { srcValidatorAddress: String(src), dstValidatorAddress: String(dst) };
  } catch {
    return null;
  }
}
