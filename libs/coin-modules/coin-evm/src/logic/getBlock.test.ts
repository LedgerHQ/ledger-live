...
describe("getBlock", () => {
  ...
  it("returns block with transactions and ERC20 transfers using bulk receipts", async () => {
    ...
    const result = await getBlock(createContext(), "", 12345);
  ...
  it("does not fallback when getBlockReceipts fails for another reason", async () => {
    ...
    await expect(getBlock(createContext(), "", 12345)).rejects.toThrow("timeout");
  ...
  it("when traceBlockErigon throws UnsupportedRpcMethodError and empty is not configured, the exception is propagated so that the caller can retry to fetch the whole block", async () => {
    ...
    await expect(getBlock(createContext(), "", 12345)).rejects.toThrow(error);
  ...
  it("with default source list, propagates trace errors when explorer and both trace methods fail at runtime", async () => {
    ...
    await expect(getBlock(createContext(), "", 12345)).rejects.toMatchObject({
  ...
  it("with default source list, propagates traceBlockErigon error when traceBlockGeth is unavailable", async () => {
    ...
    await expect(getBlock(createContext(), "", 12345)).rejects.toThrow(erigonError);
    expect(mockTraceBlockErigon).toHaveBeenCalledWith(expect.anything(), 12345);
  });
  ...
  describe("zkSync L1→L2 priority transactions (receipt type 0xff)", () => {
    ...
    it("does not apply zkSync corrections to other currencies (e.g. ethereum) even when receipt type is 0xff", async () => {
      ...
      const result = await getBlock(createContext(), "ethereum", 12345);
      expect(result.transactions[0].operations).toEqual([
        {
          type: "transfer",
          address: USER,
