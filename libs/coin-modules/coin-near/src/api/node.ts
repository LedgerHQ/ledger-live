...
const API_NEAR_INDEXER = 'https://api.nearblocks.io'
//const API_NEARBLOCKS_TOKEN = ''
...
export const getStakingPositions = async (
  ...
  const delegatedValidators = await liveNetwork<{deposit: string, validator_id: string}[]>({
    url: `${API_NEAR_INDEXER}/v1/kitwallet/staking-deposits/${address}`,
  });
...
export const getValidators = makeLRUCache(
  async (): Promise<NearValidator[]> => {
    const currencyConfig = getCoinConfig();

    const delegatedValidators = await liveNetwork<validatorsType>({
      url: `${API_NEAR_INDEXER}/v1/validators?per_page=24`,
      //headers: {Authorization: `Bearer ${API_NEARBLOCKS_TOKEN}`}
    });

    const validators = delegatedValidators.data.validatorFullData.map(({ accountId, currentEpoch, poolInfo }) => ({
