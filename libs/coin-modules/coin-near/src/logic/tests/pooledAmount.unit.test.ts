...
describe("pooledAmount", () => {
  ...
  it("sums active and activating stake entries for unstake, from balances alone", async () => {
    ...
    await expect(pooledAmount("unstake", ADDRESS, VALIDATOR, balances)).resolves.toBe(15n);
  ...
  it("sums only withdrawable stake entries for withdraw, from balances alone", async () => {
    ...
    await expect(pooledAmount("withdraw", ADDRESS, VALIDATOR, balances)).resolves.toBe(3n);
    expect(getStakingPositions).not.toHaveBeenCalled();
  });
  ...
  it("ignores stake entries delegated to a different pool", async () => {
    ...
    await expect(pooledAmount("unstake", ADDRESS, VALIDATOR, balances)).resolves.toBe(0n);
    expect(getStakingPositions).not.toHaveBeenCalled();
  });
  ...
  it("falls back to the network when balances carry no stake entries", async () => {
    ...
    await expect(pooledAmount("unstake", ADDRESS, VALIDATOR, balances)).resolves.toBe(20n);
  ...
  it("falls back to the network when no balances are passed at all", async () => {
    ...
    await expect(pooledAmount("withdraw", ADDRESS, VALIDATOR, undefined)).resolves.toBe(7n);
    expect(getStakingPositions).toHaveBeenCalledWith(ADDRESS);
  });
  ...
  it("returns 0 when the network has no position for the given pool", async () => {
    ...
    await expect(pooledAmount("unstake", ADDRESS, VALIDATOR, undefined)).resolves.toBe(0n);
  });
});
