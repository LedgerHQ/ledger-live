...
describe("listOperations via MSW", () => {
  it("maps an OUT transaction over the real HTTP path", async () => {
    // No X-Next-Page-After header → getAllTransactions stops after one page.
    server.use(
      http.get(TX_URL, () =>
        HttpResponse.json([
          {
    ...
    // listOperations fetches all pages via getAllTransactions and returns no cursor.
  ...
  it("returns no items when the indexer has no data", async () => {
