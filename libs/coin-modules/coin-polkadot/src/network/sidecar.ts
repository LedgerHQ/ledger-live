...
export const getStakingProgress = async (
  ...
  // doesn't work with westend
  let progress: SidecarPalletStakingProgress = {
    at: {
      hash: "",
      height: "",
    },
  ...
  try {
    progress = await fetchStakingProgress(currency);
  } catch (error) {
    console.warn(error);
  }

  const consts = await getConstants(currency);
