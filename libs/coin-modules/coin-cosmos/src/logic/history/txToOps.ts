...
/**
 * Map raw Cosmos txs into the (`@types/live`-derived) `CosmosOperation` shape. Owned by the logic
 * layer so both the Alpaca `listOperations` and the bridge's `getAccountShape` consume it from here.
 * `info` is framework-neutral (`{ address, unitCode }`) — no bridge types.
 */
