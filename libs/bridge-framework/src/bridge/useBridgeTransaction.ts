...
import { delay } from "@ledgerhq/coin-framework/lib/promise";
import type { Account, AccountBridge, AccountLike, TransactionCommon, TransactionStatusCommon } from "@ledgerhq/types-live";
...
export type Result<T extends TransactionCommon> = {
  transaction: T | null | undefined;
...
type Actions<T extends TransactionCommon> =
  | {
      type: "setAccount";
      account: AccountLike;
      parentAccount: Account | null | undefined;
...
type Reducer<T extends TransactionCommon> = (
  state: State<T>,
  action: Actions<T>
) => State<T>;

const initial: State<TransactionCommon> = {
  account: null,
  parentAccount: null,
  transaction: null,
  status: {
...
const reducer = <T extends TransactionCommon>(
  state: State<T>,
  action: Actions<T>
): State<T> => {
  switch (action.type) {
...
const useBridgeTransaction = <T extends TransactionCommon>(
  optionalInit?: (() => Partial<State<T>>) | null | undefined
): Result<T> => {
  const [
    {
        ...
        .then(
          (result) => {
            ...
            dispatch({
              type: "onStatus",
              status,
              transaction: preparedTransaction as T,
            });
          },
          (e) => {
            if (ignore) return;
