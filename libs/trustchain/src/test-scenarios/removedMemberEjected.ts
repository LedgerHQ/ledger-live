...
export async function scenario(transport: Transport) {
  ...
  const jwt = await sdk1.auth(trustchain, member1creds);
  const member2 = { name: name2, id: member2creds.pubkey, permissions: 0xffffffff };
  await sdk1.addMember(jwt, trustchain, member1creds, member2);

  // member2 gets a auth token
  const jwt2 = await sdk2.auth(trustchain, member2creds);

  // member1 removes member2
  await sdk1.removeMember(transport, newDeviceJWT, trustchain, member1creds, member2);
  ...
  await expect(sdk2.restoreTrustchain(jwt2, trustchain.rootId, member2creds)).rejects.toThrow(
    TrustchainEjected,
  );
