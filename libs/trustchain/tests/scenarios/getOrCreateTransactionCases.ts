...
export async function scenario(transport: Transport, { sdkForName }: ScenarioOptions) {
  ...
  const { trustchain: t1, type: type1 } = await sdk1.getOrCreateTrustchain(member1creds, callbacks);
  ...
  const { trustchain: t2, type: type2 } = await sdk1.getOrCreateTrustchain(member1creds, callbacks);
  ...
  const { trustchain: t3, type: type3 } = await sdk2.getOrCreateTrustchain(member2creds, callbacks);
