...
export const getFiltersForMessage = async (
  ...
  if (!calServiceURL) {
    // No service URL configured, cannot fetch filters
    return undefined;
  }
  ...
  try {
    ...
    return targetObject?.eip712_signatures?.[verifyingContract]?.[schemaHash];
  } catch (e) {
    // No static fallback, return undefined if API fails
    return undefined;
  }
};
