...
On web `CardDetails` owns one reveal view model and passes it into the card face and the
actions row. Omit `unlock` and that value is `null`: the face never flips and `CardActions`
lays out Freeze and More alone.
...
└── src/
    ...
    │   │   ├── CardDetails.web.tsx            # Reveal VM passed to the face and CardActions
    ...
    │   ├── CardFlip/
    │   │   ├── CardFlip.web.tsx               # Card face; flips when a reveal VM is passed in
    │   │   ├── CardFlip.native.tsx            # Stub until LWM reveal UI
    │   │   └── CardFlipView.web.tsx           # Flip to the PAN/CVV image
    │   ├── Reveal/
    │   │   ├── Reveal.web.tsx                 # View tile
