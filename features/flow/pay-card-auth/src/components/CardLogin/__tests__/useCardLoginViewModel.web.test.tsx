...
describe("mapSnapshotToViewModel", () => {
  it.each(["idle", "error"] as const)("offers the login action in %s", value => {
    const login = mapSnapshotToViewModel(value, null, onLoginPress, intro);
  ...
  it.each([
    ...
    expect(mapSnapshotToViewModel(value, null, onLoginPress, intro)?.isLoading).toBe(true);
  });
  ...
  it("offers nothing once the card holder is signed in", () => {
    ...
    expect(mapSnapshotToViewModel("ready", null, onLoginPress, intro)).toBeNull();
  });

  it("shows no message while there is no error", () => {
    expect(mapSnapshotToViewModel("idle", null, onLoginPress, intro)?.errorMessage).toBeNull();
  });

  it("hands the intro props straight through", () => {
    expect(mapSnapshotToViewModel("idle", null, onLoginPress, intro)?.intro).toBe(intro);
  });
  ...
  it.each([
    ...
    const login = mapSnapshotToViewModel("error", errorKind, onLoginPress, intro);
...
async function renderIdleLogin(store: ReturnType<typeof buildStore>) {
  const rendered = renderHook(
    () => useCardLoginViewModel({ openHostedLogin: mockPorts.openHostedLogin, oauthConfig }),
