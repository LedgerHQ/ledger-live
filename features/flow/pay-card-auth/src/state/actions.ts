 ...
 *
 * Only an action that ignores the event can live here. `setup()` types a named action with the whole
 ...
 */
export const publishSignedIn = ({ context }: ContextArgs) => context.ports.setSignedIn(true);

export const publishSignedOut = ({ context }: ContextArgs) => context.ports.setSignedIn(false);
