...
describe("useConfirmPasswordViewModel", () => {
  ...
  it("locks the CTA while saving", async () => {
    let release = () => {};
    const onConfirmed = jest.fn(
      () =>
  ...
  it("saves once even when the field is submitted again mid-derivation", async () => {
    let release = () => {};
    const onConfirmed = jest.fn(
      () =>
        new Promise<void>(resolve => {
    ...
    // The disabled CTA is not the only way in: the keyboard's return key stays live.
    await act(async () => {
      await result.current.viewModel.onConfirm();
    });
