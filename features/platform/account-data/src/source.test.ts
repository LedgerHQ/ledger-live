...
describe("registerAccountBalanceSources", () => {
  it("hands back a list a caller cannot mutate", () => {
    registerAccountBalanceSources([source("a", 1)]);
    const registered = getAccountBalanceSources() as AccountBalanceSource[];
    expect(() => registered.push(source("b", 2))).toThrow();
