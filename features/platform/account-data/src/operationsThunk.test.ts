...
describe("fetchAccountOperations", () => {
  ...
  it("does not coalesce a head read of the same account under a different ref", async () => {
    const seen: string[] = [];
    const { source } = scripted([{ operations: head, complete: true }], {
      getOperations: async asked => {
        seen.push(asked.address);
        return { operations: head, complete: true };
      },
    });
    const store = makeStore();
    await Promise.all([
      store.dispatch(fetchAccountOperations(ref, { sources: [source] })),
      store.dispatch(
        fetchAccountOperations({ ...ref, address: "0xdef" }, { sources: [source], maxAge: 0 }),
      ),
    ]);
  ...
  it("keeps the token rows the head read produced, tagged by their own account", async () => {
    const tokenRow = op("tok-1", "2026-01-31T11:00:00.000Z");
    const { source } = scripted([{ operations: [...head, tokenRow], complete: true }]);
    const store = makeStore();
    await store.dispatch(fetchAccountOperations(ref, { sources: [source] }));

    expect(select.selectAccountOperations(store.getState(), accountId).map(o => o.id)).toContain(
      "tok-1",
    );
  });
