...
an *address*, so a token row arrives with its parent's read.

There is no token-account ref to pass: `AccountRef.accountId` is an `AccountId`, and that primitive
refuses the `+` a token account id carries. Read a token balance from `subAccountBalances` on the
parent's hook, or from `accountBalancesSlice.selectors.selectAccountBalance` with the token account's
own id — the row is already there.
