...
schema, token-read or write failure inside renewal sends `/v1/auth/logout` with the most recent access
token available, then clears the session locally and publishes signed-out without waiting for the
remote answer. The logout request carries the captured token instead of reading storage, so immediate
local cleanup cannot discard it before it is sent. This deliberately signs users out during a
token-endpoint outage in exchange for one recovery rule.
