...
echo "Opening Bitcoin Testnet wallet app ..."
ledger-live app -o bitcoin_testnet

for((i=0; i<$dataLength; i++))
do
    currency=`echo $data | jq -r --argjson id $i '.[$id].currency'`
    echo $currency
    schemeSender=`echo $data | jq -r --argjson id $i '.[$id].schemeSender'`
    ...
    ledger-live send --device --currency $currency --scheme $schemeSender --index $indexSender --recipient $recipientAddress --amount $amount --feePerByte $feePerByte --format json |
