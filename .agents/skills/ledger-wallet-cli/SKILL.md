...
| 64        | `unknown_flag`, `invalid_flag_value`, `missing_required_flag`, `unknown_command`, `invalid_transport`, `device_ambiguous`, `ble_dependency_missing` | **your invocation is wrong**                             | Fix the command line — never retry by guessing flags. For `device_ambiguous`, pick from `error.details.candidates` and pass `--device <id>`. |
...
| BLE error naming `@abandonware/noble` (exit 64, code `ble_dependency_missing`)                       | optional BLE dependency not installed                                                                                                                  | Install the optional dependency, or use USB (`WALLET_CLI_TRANSPORT=usb` or unset).                                                                                                                                                                                                                                                                                                    |
