...
> **Diagnosing a USB failure:** with `--output json`, a failed device command carries
> `likely_cause` (`sandbox_blocking_usb`, `device_not_present`, `app_not_open`, `unknown`),
...
| `[✖] Timed out talking to the Ledger over USB…` / JSON `error.code: "USB_TIMEOUT"` | sandbox blocking USB, or device busy/locked. **Re-run with `--output json` and read `error.likely_cause` and `error.agent_hint` — they name the cause and the fix.** | When `likely_cause` is `sandbox_blocking_usb`, `agent_hint` tells you the bypass for your agent (`dangerouslyDisableSandbox: true` for Claude Code). **Ask the user for confirmation before re-running with the bypass.** The bypass is expected for device commands (`account discover`, `receive`, `send`, `genuine-check`, `swap execute`, `earn deposit`, `earn withdraw`, `ring init`); if this fires on any other command, investigate before bypassing rather than disabling the sandbox by reflex. For any other `likely_cause`, follow `user_hint` instead — do not bypass the sandbox. |
