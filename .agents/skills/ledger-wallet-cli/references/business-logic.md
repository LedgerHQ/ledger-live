...
- `agent_hint` is emitted whenever the cause is host-side. No environment sniffing is involved: the hint never consults `detectAgent()`, which the CLI uses only for the first-run nudge and its terminal-output decisions (spinners, hyperlinks). Its text is conditional ("If you are running this through Claude Code, Codex CLI, Cursor, or another sandboxed agent..."), so it reads correctly for a human as well - and an agent whose environment wallet-cli cannot recognise, which is the case that most needs the bypass instruction, still receives it. `user_hint` covers the terminal case alongside it.
