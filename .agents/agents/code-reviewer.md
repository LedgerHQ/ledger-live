...
- **`pnpm-lock.yaml` diff**: Flag unrelated version bumps, mass reformatting, or entries not explained by the PR's `package.json` changes. The lockfile diff should be entirely explainable by the stated dependency changes. **Hidden lockfile diff with a large stated line count**: if the diff view shows "No diff generated for this file." for `pnpm-lock.yaml` alongside a large reported add/delete count, this is not a formatting nit to skip past — it's a strong signal the branch is out of sync with the base branch. Flag it and recommend rebasing onto the base branch, resetting the lockfile to the base branch version (`git checkout origin/<base-branch> -- pnpm-lock.yaml`), then reinstalling to regenerate it, rather than resolving it by hand.
